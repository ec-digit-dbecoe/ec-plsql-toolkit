...
--
   ...
   -- 23.1  deboiph  08/05/2023  Added sensitive data discovery and masking capability
...
--         show_message('S','/'||CHR(10)||' ',TRUE);
   ...
   END;
   ---
   -- Get a mask for a table column if any
   ---
   FUNCTION get_mask (
   ...
   IS
      l_msk ds_utility_var.mask_rec_table;
      l_table_name ds_utility_var.table_name := UPPER(p_table_name);
      l_column_name ds_utility_var.column_name := UPPER(p_column_name);
   BEGIN
      IF ds_utility_var.g_msk.EXISTS(l_table_name) THEN
         IF ds_utility_var.g_msk(l_table_name).EXISTS(l_column_name) THEN
            RETURN ds_utility_var.g_msk(l_table_name)(l_column_name);
         END IF;
      END IF;
      RETURN NULL;
   END;
   ...
   BEGIN
      FOR r_msk IN c_msk LOOP
         ds_utility_var.g_msk(UPPER(r_msk.table_name))(UPPER(r_msk.column_name)) := r_msk;
      END LOOP;
   END;
--#begin public
   ---
...
--#end public
   ...
   BEGIN
      ...
      ELSE
         sys.dbms_random.seed(TO_CHAR(SYSTIMESTAMP,'HH24MISSFF'));
      END IF;
      ...
      ELSE
         ...
         l_row_idx := ds_masker_krn.random_integer(1,ds_utility_var.g_ds_tab(l_full_col_name).LAST,p_seed);
      END IF;
...
SELECT tmp.$col_name$ FROM rnd INNER JOIN tmp ON rnd.random_value >= tmp.lower_ratio_range AND rnd.random_value < tmp.upper_ratio_range'
         , '$col_name$', LOWER(p_col_name)), '$tab_name$', LOWER(p_tab_name)), '$weight$', LOWER(p_weight));
      ELSE
         l_sql := 'SELECT '||LOWER(p_col_name)||' FROM '||LOWER(p_tab_name)||CASE WHEN p_where IS NOT NULL THEN ' WHERE '|| p_where END || ' ORDER BY SYS.DBMS_RANDOM.VALUE';
      END IF;
      ...
      END IF;
      -- When seed is passed, only the first row is returned => close any previously open cursor
      IF p_seed IS NOT NULL AND l_cursor IS NOT NULL THEN
         close_cursor;
      END IF;
      -- Open cursor and prepare execution if not already done
      IF l_cursor IS NULL OR NOT sys.dbms_sql.is_open(l_cursor) THEN
         SYS.DBMS_RANDOM.SEED(CASE WHEN p_seed IS NOT NULL THEN p_seed ELSE TO_CHAR(SYSTIMESTAMP,'HH24MISSFF') END);
         l_cursor := sys.dbms_sql.open_cursor;
      ...
      IF l_count > 0 THEN
         ...
         END LOOP;
      END IF;
      IF l_count = 0 OR p_seed IS NOT NULL OR p_weight IS NOT NULL THEN
         close_cursor;
      END IF;
      IF l_count = 0 THEN
...
--#end public
   ...
   ---
   PROCEDURE refresh_dynamic_cache IS
   BEGIN
      ds_utility_var.g_ds_tab.DELETE;
      ds_utility_var.g_ds_tab2.DELETE;
      ds_utility_var.g_msk.DELETE;
      load_masks;
   END;
   ---
...
--#end public
   ...
   FUNCTION format_column_value (
    ...
    , p_relocate_ids IN VARCHAR2
    , p_db_link IN VARCHAR2
   )
   RETURN VARCHAR2
   IS
      l_relocate_ids VARCHAR2(1) := UPPER(NVL(SUBSTR(p_relocate_ids,1,1),'N'));
   ...
   BEGIN
      ...
      ELSIF p_col_type = 'NUMBER' THEN
         ...
         END IF;
         IF l_relocate_ids != 'N' THEN
            l_tab_col := LOWER(p_table_name||'.'||p_col_name);
            IF ds_utility_var.g_seq.EXISTS(l_tab_col) THEN
               r_seq := ds_utility_var.g_seq(l_tab_col);
               IF r_seq.id_shift_value IS NOT NULL THEN
--                  l_col_val := l_col_val || '+' || r_seq.id_shift_value;
                  NULL;
               ELSIF ds_utility_var.g_in_mem_seq_tab.EXISTS(r_seq.sequence_name) THEN
--                  l_seq := ds_utility_var.g_in_mem_seq_tab(r_seq.sequence_name);
--                  l_col_val := 'ds_utility_krn.set_identifier(' || r_seq.table_id || ',' || l_col_val || ',' || l_seq ||')';
--                  ds_utility_var.g_in_mem_seq_tab(r_seq.sequence_name) := l_seq + NVL(r_seq.in_mem_seq_increment_by,1);
                  NULL;
               ELSE
                  l_col_val := 'ds_utility_krn.set_identifier(' || r_seq.table_id || ',' || l_col_val || ',' || LOWER(r_seq.sequence_name) || '.nextval'||p_db_link||')';
               END IF;
...
--                  l_col_val := l_col_val || '+' || r_seq.id_shift_value;
                  NULL;
               ELSIF ds_utility_var.g_in_mem_seq_tab.EXISTS(r_seq.sequence_name) THEN
--                  l_col_val := 'ds_utility_krn.get_identifier(' || r_seq.table_id || ',' || l_col_val || ')';
                  NULL;
               ELSE
                  l_col_val := 'ds_utility_krn.get_identifier(' || r_seq.table_id || ',' || l_col_val || ')';
               END IF;
...
--#end public
   ...
   END;
   ---
   -- Check whether a column is forced
   ---
   FUNCTION is_forced (
      p_tab_name IN VARCHAR2 -- table name
    , p_col_name IN VARCHAR2 -- column name
   ...
   BEGIN
      ...
      WHILE l_name IS NOT NULL LOOP
         IF UPPER(p_tab_name)||'.'||UPPER(p_col_name)||'#'||p_oper LIKE l_name THEN
            RETURN TRUE;
         END IF;
   ...
   END;
   ---
   -- Check whether a column is masked
   ---
   FUNCTION is_masked (
      p_tab_name IN VARCHAR2 -- table name
    , p_col_name IN VARCHAR2 -- column name
   ...
   BEGIN
      r_msk := get_mask(p_tab_name,p_col_name);
      IF  r_msk.msk_id IS NOT NULL
      AND r_msk.msk_type = 'SQL'
      AND NOT ds_utility_var.g_fk_tab.EXISTS(LOWER(p_tab_name||'.'||p_col_name)) -- ignored FKs involved in PK propagation
      AND ds_utility_var.g_mask_data 
      THEN
         RETURN TRUE;
      END IF;
      RETURN FALSE;
   END;
   ---
   -- Check whether a column is relocated
   ---
   FUNCTION is_relocated (
      p_tab_name IN VARCHAR2 -- table name
    , p_col_name IN VARCHAR2 -- column name
   )
   RETURN BOOLEAN
   ...
   BEGIN
      ...
      ELSIF ds_utility_var.g_map.EXISTS(l_tab_col) THEN
         r_seq := ds_utility_var.g_map(l_tab_col);
      END IF;
      RETURN r_seq.id_shift_value IS NOT NULL OR (r_seq.sequence_name IS NOT NULL AND ds_utility_var.g_in_mem_seq_tab.EXISTS(r_seq.sequence_name) AND ds_utility_var.g_mask_data);
   END;
   ---
   ...
   FUNCTION get_forced_column_value (
    ...
    , p_relocate_ids IN VARCHAR2 -- Y/N
    , p_db_link IN VARCHAR2
   )
   RETURN VARCHAR2
   IS
   ...
   BEGIN
      ...
      WHILE l_name IS NOT NULL LOOP
         IF UPPER(p_tab_name)||'.'||UPPER(p_col_name)||'#'||p_oper LIKE l_name THEN
            ...
            ELSIF SUBSTR(l_col_val,1,1) = ':' THEN
               l_col_val := replace_string_ignore_case(l_col_val,':rowid','rowid');
               l_col_val := replace_string_ignore_case(l_col_val,':rownum','rownum');
            ELSE
               l_col_val := format_column_value(p_part,p_tab_name,p_col_name,p_col_rec.data_type,l_col_val,p_relocate_ids,p_db_link);
               p_col_rec.is_masked := FALSE;
            END IF;
--sys.dbms_output.put_line('forcing '||p_col_name||' to '||l_col_val);
            EXIT;
         END IF;
         l_name := ds_utility_var.g_rep.NEXT(l_name);
      END LOOP force_loop;
      IF NOT l_masking THEN
         l_tab_col := LOWER(p_tab_name||'.'||p_col_name);
         IF ds_utility_var.g_seq.EXISTS(l_tab_col) THEN
            r_seq := ds_utility_var.g_seq(l_tab_col);
            IF r_seq.id_shift_value IS NOT NULL THEN
               IF p_data IS NOT NULL THEN
                  IF l_col_val != 'NULL' THEN
                     l_col_val := TO_NUMBER(l_col_val) + r_seq.id_shift_value;
                  END IF;
               ELSE
                  l_col_val := l_col_val || '+' || r_seq.id_shift_value;
               END IF;
               l_masking := TRUE;
            ELSIF ds_utility_var.g_in_mem_seq_tab.EXISTS(r_seq.sequence_name) AND ds_utility_var.g_mask_data THEN
               l_seq := ds_utility_var.g_in_mem_seq_tab(r_seq.sequence_name);
               IF p_data IS NOT NULL THEN
                  l_col_val := get_identifier(r_seq.table_id,l_col_val);
-- For future use (SEQUENCE on non-PK/UK columns)
--               ELSIF r_seq.pk_name IS NOT NULL THEN
--                  l_col_val := 'ds_utility_krn.in_mem_seq_nextval('||LOWER(r_seq.table_name)||'.'||r_seq.column_name||')'; -- Sequence on non-pk column
               ELSE
                  l_col_val := 'ds_utility_krn.get_identifier(' || r_seq.table_id || ',' || l_col_val || ')'; -- Sequence on pk column => mapping needed
               END IF;
               l_masking := TRUE;
-- No more needed since creation of generate_identfiers()
--            ELSE
--               l_col_val := 'ds_utility_krn.set_identifier(' || r_seq.table_id || ',' || l_col_val || ',' || LOWER(r_seq.sequence_name) || '.nextval'||p_db_link||')';
            END IF;
         ELSIF ds_utility_var.g_map.EXISTS(l_tab_col) THEN
...
--               l_col_val := 'ds_utility_krn.get_identifier(' || r_seq.table_id || ',' || l_col_val || ')';
         ...
         END IF;
      END IF;
      IF NOT l_masking AND ds_utility_var.g_mask_data THEN
         -- no forced value => apply mask
         r_msk := get_mask(p_tab_name,p_col_name);
         IF r_msk.msk_id IS NOT NULL THEN
            IF  r_msk.msk_type = 'SQL'
            AND NOT ds_utility_var.g_fk_tab.EXISTS(LOWER(p_tab_name||'.'||p_col_name)) -- ignored FKs involved in PK propagation
            THEN
               l_masking := TRUE;
               l_col_val := r_msk.params;
--sys.dbms_output.put_line('Masking '||p_col_name||' to '||l_col_val);
            ELSE
               -- Other mask types to be implemented
               NULL; --TBD
            END IF;
         END IF;
      END IF;
      -- Bind variables if any
      IF l_masking AND ds_utility_var.g_mask_data THEN
         IF p_data IS NOT NULL THEN
            l_col_val := replace_string_ignore_case(l_col_val,'rowid',':rowid');
            l_col_val := replace_string_ignore_case(l_col_val,'rownum',':rownum');
         END IF;
   ...
   BEGIN
      ...
      IF p_sql IS NULL THEN
         RETURN NULL;
      END IF;
      l_beg := NVL(INSTR(l_sql,'''',1),0); --search nth quote
      IF l_beg > 0 THEN
         l_end := NVL(INSTR(l_sql,'''',l_beg+1,1),0);
         assert(l_end>0,'Unterminated string in expression: '||p_sql);
         l_sql := add_table_alias(SUBSTR(l_sql,1,l_beg-1), p_table_alias, p_col_names)
               || SUBSTR(l_sql,l_beg,l_end-l_beg+1)
               || add_table_alias(SUBSTR(l_sql,l_end+1), p_table_alias, p_col_names)
               ;
      ELSE
         ...
         FOR i IN l_idx..p_col_names.COUNT LOOP
            l_col_name := CASE WHEN i = 0 THEN 'rowid' ELSE p_col_names(i) END;
   ...
   BEGIN
      FOR i IN 1..p_col_names.COUNT LOOP
         l_sql := regexp_replace(l_sql,'([^A-Za-z0-9_]|^)('||p_table_alias||'\.'||p_col_names(i)||')([^A-Za-z0-9_]|$)'
                 ,'\1'||p_table_alias2||'.'||p_col_names(i)||'\3',1,0,'i');
      END LOOP;
      RETURN l_sql;
   END;
   ---
   ...
   BEGIN
      ...
      FOR i IN 1..l_col_names.COUNT LOOP
         ...
         l_col_rec.is_masked := l_col_rec.column_name NOT IN ('rowid','rownum') AND
            ((is_forced(p_table_name,l_col_rec.column_name,p_op)
          OR (is_masked(p_table_name,l_col_rec.column_name) AND p_part NOT IN ('I','U')))
          OR is_relocated(p_table_name,l_col_rec.column_name));
         l_is_masked := l_is_masked OR l_col_rec.is_masked;
         IF p_data IS NULL THEN
            l_col_rec.col_val := l_col_rec.column_name;
         ELSE
            l_col_rec.col_val := format_column_value(p_part,p_table_name,l_col_rec.column_name,l_col_rec.data_type,NVL(REPLACE(extract_string(p_data,i),'\~','~'),'NULL'),'Y',l_db_link);
         END IF;
      ...
      WHILE l_is_masked LOOP
         l_cnt := l_cnt + 1;
         assert(l_cnt<=20,'Loop detected in masks of '||p_table_name);
         l_is_masked := FALSE;
         FOR i IN 1..l_col_names.COUNT LOOP
            l_col_rec := ds_utility_var.g_col_tab(i);
            IF l_col_rec.is_masked AND l_col_rec.column_name NOT IN ('rowid','rownum') THEN
               l_col_rec.col_val := get_forced_column_value(p_part,p_table_name,l_col_rec.column_name,l_col_rec,p_op,p_data,p_relocate_ids,l_db_link);
               l_is_masked := l_is_masked OR l_col_rec.is_masked;
               ds_utility_var.g_col_tab(i) := l_col_rec;
            END IF;
         END LOOP;
...
--                  l_out := l_out || 'TRIM(TO_CHAR(' || l_col_val || '+' || TRIM(TO_CHAR(ds_utility_var.g_seq(l_tab_col).id_shift_value)) || '))';
--               ELSIF ds_utility_var.g_map.EXISTS(l_tab_col) AND ds_utility_var.g_map(l_tab_col).id_shift_value IS NOT NULL THEN
--                  l_out := l_out || 'TRIM(TO_CHAR(' || l_col_val || '+' || TRIM(TO_CHAR(ds_utility_var.g_map(l_tab_col).id_shift_value)) || '))';
--               ELSE
...
--               END IF;
   ...
   IS
   BEGIN
      RETURN build_sql_statement_part('S',p_table_name,p_table_alias,p_op,p_columns_list,p_left_tab,p_indent_first_line,p_columns_per_line,p_db_link,p_relocate_ids);
   END;
   ---
   ...
   IS
   BEGIN
      RETURN build_sql_statement_part('I',p_table_name,p_table_alias,'I',p_columns_list,p_left_tab,p_indent_first_line,p_columns_per_line,p_db_link,'Y',p_data,p_pk_size);
   END;
   ---
   ...
   IS
   BEGIN
      RETURN build_sql_statement_part('U',p_table_name,p_table_alias,'U',p_sel_columns,0,'N',1,NULL,'Y',p_data,p_pk_size,p_indent);
   END;
--
--#begin public
/**
...
--#end public
   ...
   BEGIN
      UPDATE ds_data_sets
       ...
       WHERE (p_set_id IS NULL OR set_id = p_set_id)
      ;
      assert(SQL%ROWCOUNT>0,'No data set property updated!');
   END;
   ---
...
--#end public
   ...
   BEGIN
      ...
      IF ds_utility_var.g_owner != USER THEN
   ...
   BEGIN
      UPDATE ds_tables
       ...
       WHERE (p_set_id IS NULL OR set_id = p_set_id)
         AND (p_table_name IS NULL OR table_name LIKE p_table_name ESCAPE '~')
      ;
--      assert(SQL%ROWCOUNT>0,'No table property updated!');
   END;
--
--#begin public
/**
   ...
   BEGIN
      UPDATE ds_constraints
       ...
       WHERE (p_set_id IS NULL OR set_id = p_set_id)
         ...
         AND (p_cardinality IS NULL OR cardinality = p_cardinality)
      ;
--      assert(SQL%ROWCOUNT>0,'No constraint property updated!');
   END;
--#begin public
/**
...
--#end public
   ...
   BEGIN
      ...
      ;
--      assert(SQL%ROWCOUNT>0,'No column property updated for table "'||p_table_name||'"!');
   END;
--#begin public
/**
         ...
         FOR r_tab IN c_tab(p_set_id,p_table_name) LOOP
            ...
            FOR r_fk IN c_fk(ds_utility_var.g_owner,r_tab.table_name,p_constraint_name) LOOP
               ...
               IF r_tab_det.table_name IS NULL THEN
                  ...
                  r_tab_det.set_id := p_set_id;
                  r_tab_det.table_name := r_fk.detail_table_name;
                  r_tab_det.extract_type := NVL(p_extract_type,'P');
               ...
               IF r_tab_mst.table_name IS NULL THEN
                  ...
                  r_tab_mst.extract_type := NVL(p_extract_type,'P');
...
--#end public
   ...
   BEGIN
      ...
      ;
      assert(SQL%ROWCOUNT>0,'No pattern property updated!');
   END;
--#begin public
   ---
   -- Delete pattern(s)
...
--#end public
   ...
   BEGIN
      ...
      SELECT ds_msk_seq.nextval, col.table_name, col.column_name
       ...
       WHERE col.owner = ds_utility_var.g_owner
...
DECLARE
   ...
   SELECT #'||l_partitioned_cols||q'#ds_rec.record_rowid
    FROM ds_records ds_rec, #'||l_table_name||q'# #'||r_tab.table_alias||q'#
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         FOR r_tab IN c_tab(r_set.set_id) LOOP
            ...
            THEN
               ...
               IF NVL(INSTR(l_export_mode,'I'),0) > 0 THEN
               ...
               END IF;
               IF NVL(INSTR(l_export_mode,'U'),0) > 0 THEN
...
--#end public
   ...
   END;
   ---
   -- Initialise pk masking propagation
   ---
   ...
   IS
      ...
      IS
         SELECT *
          ...
          ORDER BY set_id
      ;
      -- Cursor to get PKs whose masking must be propagated to FKs
      CURSOR c_fk (
         p_set_id IN ds_data_sets.set_id%TYPE
      ...
      IS
         SELECT ds_pk_con.constraint_name pk_name, LOWER(ds_pk_col.table_name) pk_tab_name, LOWER(ds_pk_col.column_name) pk_col_name
          ...
          INNER JOIN sys.all_constraints ds_pk_con
             ON ds_pk_con.owner = ds_utility_var.g_owner
   ...
   IS
      ...
      IS
         SELECT ds_tab.table_id, ds_tab.set_id
              , CASE WHEN ds_msk.msk_type IS NOT NULL THEN LOWER(ds_tab.table_alias||'_'||ds_msk.column_name) ELSE LOWER(ds_tab.sequence_name) END sequence_name
              , CASE WHEN ds_msk.msk_type IS NOT NULL THEN 'Y' ELSE '' END in_mem_seq_flag
              , CASE WHEN ds_msk.msk_type = 'SEQUENCE' AND REGEXP_REPLACE(ds_msk.params, '.*START WITH ([1-9][0-9]*).*', '\1') != ds_msk.params -- replace succeeded
              ...
              , ds_con.constraint_name pk_name
           FROM ds_tables ds_tab
          INNER JOIN sys.all_constraints ds_con
             ON ds_con.owner = ds_utility_var.g_owner
            AND ds_con.table_name = ds_tab.table_name
            AND ds_con.constraint_type IN ('P','U')
          ...
          INNER JOIN sys.all_tab_columns tcol
           ...
           LEFT OUTER JOIN ds_masks ds_msk
             ON ds_msk.table_name = ds_tab.table_name
            AND ds_msk.column_name = ccol.column_name
            AND ds_msk.msk_type = 'SEQUENCE'
            AND NVL(ds_msk.disabled_flag,'N') = 'N'
          WHERE ds_tab.set_id = p_set_id
            AND ds_tab.extract_type != 'N'
            AND ccol2.position IS NULL -- PK must have only one column
            AND (ds_tab.sequence_name IS NOT NULL OR ds_tab.id_shift_value IS NOT NULL OR ds_msk.msk_type IS NOT NULL)
            AND (ds_tab.sequence_name IS NULL OR seq.sequence_name IS NOT NULL) -- Named Oracle sequence exists
     ;
      CURSOR c_map (
         p_set_id IN ds_data_sets.set_id%TYPE
       , p_table_name IN sys.all_constraints.table_name%TYPE
      ...
      IS
         SELECT ds_tab.table_id, ds_tab.set_id, LOWER(ds_tab.sequence_name) sequence_name
         ...
         WHILE l_col_name IS NOT NULL LOOP
            ...
            UPDATE ds_masks
               SET msk_type = 'INHERIT'
                 ...
                 , values_sample = NULL
                 , sensitive_flag = 'Y'
                 , remarks = 'Inherit value from sequence '||r_map.sequence_name
             WHERE table_name = UPPER(r_map.table_name)
               AND column_name = UPPER(r_map.column_name)
            ;
            IF SQL%ROWCOUNT = 0 THEN
               insert_mask(p_table_name=>r_map.table_name, p_column_name=>r_map.column_name, p_msk_type=>'INHERIT', p_params=>r_map.sequence_name
                 ,p_remarks=>'Inherit value from sequence '||r_map.sequence_name
               );
            END IF;
   ...
   BEGIN
      ...
      IF p_extract_type IN ('F','N') THEN
         IF p_purpose = 'VIEW' THEN
            l_select := p_sel_columns;
         ELSIF p_purpose = 'SCRIPT' THEN
            l_select := build_sql_statement_part('X',p_table_name,NULL,p_op,p_sel_columns,7,'N');
         ELSIF p_purpose = 'DIRECT' THEN
            l_select := build_sql_statement_part('S',p_table_name,NULL,p_op,p_sel_columns,7,'N');
         END IF;
         l_sql :=
      ...
      IF ds_utility_var.g_mask_data THEN
         ...
         FOR c IN 1..t_pk_cols.COUNT LOOP
            ...
            WHILE ds_utility_var.g_fk_tab.EXISTS(r_fk.pk_tab_name||'.'||r_fk.pk_col_name) LOOP
               ...
               r_pk := ds_utility_var.g_pk_tab(r_fk.pk_tab_name||'.'||r_fk.pk_col_name);
            END LOOP;
            l_col_val := regexp_replace(r_pk.col_val,'([^A-Za-z0-9_]|^)('||r_pk.pk_col_name||')([^A-Za-z0-9_]|$)','\1'||r_pk.pk_tab_alias||'.'||r_pk.pk_col_name||'\3',1,0,'i');
            l_select := regexp_replace(l_select,'([^A-Za-z0-9_]|^)('||p_table_alias||'\.'||t_fk_cols(c)||')([^A-Za-z0-9_]|$)','\1'||l_col_val||'\3',1,0,'i');
            -- Shuffling of PK/UK
            FOR g IN 1..3 LOOP
               ...
               l_select := regexp_replace(l_select,'([^A-Za-z0-9_]|^)('||r_pk.pk_tab_alias||'\.'||r_pk.pk_col_name||')([^A-Za-z0-9_]|$)','\1'||l_table_alias3||'.'||r_pk.pk_col_name||'\3',1,0,'i');
            END LOOP;
         END LOOP;
      END IF; -- mask mode
      l_sql := '';
   ...
   --         END IF;
         ...
         FOR c IN 1..t_pk_cols.COUNT LOOP
            ...
            LOOP
               ...
               l_sql := l_sql
             ||l_ws||' INNER JOIN '||l_source_schema||LOWER(r_pk.pk_tab_name)||' '||r_pk.pk_tab_alias||CHR(10)
            ...
            FOR g IN 1..3 LOOP
               ...
               l_table_alias3 := r_pk.pk_tab_alias||'_rec'||TO_CHAR(g)||'_'||r_pk.table_alias;
               l_sql := l_sql
             ||l_ws||' INNER JOIN ds_records '||l_table_alias2||CHR(10)
             ||l_ws||'    ON '||r_pk.pk_tab_alias||'.rowid = '||l_table_alias2||'.record_rowid'||CHR(10)
             ||l_ws||' INNER JOIN '||l_source_schema||LOWER(r_pk.pk_tab_name)||' '||l_table_alias3||CHR(10)
      ...
      CURSOR c_set (
         ...
         SELECT *
          ...
          WHERE (p_set_id IS NULL OR set_id = p_set_id)
            AND set_type = 'SUB'
      ;
      -- Cursor to browse tables having an in-memory sequence
      CURSOR c_tab (
         p_set_id ds_data_sets.set_id%TYPE
      ) IS
         SELECT ds_tab.*, ds_msk.column_name
           FROM ds_tables ds_tab
          INNER JOIN ds_masks ds_msk
            ...
            AND NVL(ds_msk.disabled_flag,'N') = 'N'
          WHERE (p_set_id IS NULL OR ds_tab.set_id = p_set_id)
--            AND ds_tab.sequence_name IS NOT NULL
--            AND NVL(ds_tab.in_mem_seq_flag,'N') = 'Y'
          ORDER BY ds_tab.seq, ds_tab.table_name
         ;
      l_sql VARCHAR2(4000);
      l_pk_name sys.all_constraints.constraint_name%TYPE;
      l_src_table_name VARCHAR2(100);
   ...
   BEGIN
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         FOR r_tab IN c_tab(r_set.set_id) LOOP
            ...
            l_pk_columns := get_constraint_columns(l_pk_name);
--            l_src_table_name := gen_full_table_name(r_tab.table_name,r_tab.source_schema,r_tab.source_db_link);
            IF ds_utility_var.g_in_mem_seq_tab.EXISTS(LOWER(r_tab.table_alias||'_'||r_tab.column_name)) THEN
               l_sql :=
                  'INSERT INTO ds_identifiers (table_id, old_id, new_id)'||CHR(10)
                ||'SELECT ds_rec.table_id, '||r_tab.table_alias||'.'||l_pk_columns||', ds_utility_krn.in_mem_seq_nextval('''||LOWER(r_tab.table_name)||'.'||l_pk_columns||''')'||CHR(10)
...
--      t_fk_cols ds_utility_var.column_name_table;
   ...
   BEGIN
      ...
      l_sql := l_sql || '
DECLARE
   TYPE t_tgt_type IS TABLE OF '||l_table_name||'%ROWTYPE INDEX BY BINARY_INTEGER;
   ...
   r_tgt '||l_table_name||'%ROWTYPE; -- current target record
   r_lag '||l_table_name||'%ROWTYPE; -- previsous target record';
   ...
   l_level PLS_INTEGER; -- current hierarchic level';
      ...
      ELSE
         l_sql := l_sql ||'
   r_src '||l_table_name||'%ROWTYPE; -- current source/parent record';
      END IF;
      IF l_gen_view_name IS NOT NULL THEN
      ...
      IF r_con.cardinality = 'N-1' THEN
         ...
         FOR i IN 1..t_pk_cols.COUNT LOOP
            l_sql := l_sql || '
   TYPE t_'||t_pk_cols(i)||'_type IS TABLE OF '||l_table_name||'.'||t_pk_cols(i)||'%TYPE INDEX BY BINARY_INTEGER;
...
--   r_'||r_fk.constraint_name_lc||' '||r_fk.r_table_name||'%ROWTYPE;';
         l_select := 'SELECT fkt.* FROM '||r_fk.r_table_name || ' fkt';
   ...
   BEGIN
      ...
      FORALL i IN 1..t_tgt.COUNT
         UPDATE '||l_table_name||' SET ROW = t_tgt(i)
          WHERE ';
         FOR i IN 1..t_pk_cols.COUNT LOOP
            IF i > 1 THEN
               l_sql := l_sql||'
   ...
   BEGIN
      ...
      FORALL i IN 1..t_tgt.COUNT
         INSERT INTO '||l_table_name||' VALUES t_tgt(i)
         RETURNING ROWID BULK COLLECT INTO t_rowid;
      l_tot_count := l_tot_count + SQL%ROWCOUNT;
      t_tgt.DELETE;
      -- Prepare rowids
      t_rec.DELETE;
      l_count := 0;
...
BEGIN';
   ...
   WHILE l_level <= '||l_level_max||' LOOP -- for recursive FKs only, one iteration per hierarchy level (root level already processed)';
      ...
      END IF;
      l_sql := l_sql || q'#
            l_seed := TO_CHAR(SYSTIMESTAMP,'HH24MISSFF');
            r_tgt := NULL;#';
      FOR l_fk IN 1..t_fk.COUNT LOOP
         r_fk := t_fk(l_fk);
         l_sql := l_sql || '
   ...
   UPDATE ds_tables SET extract_count = l_tot_count WHERE table_id = '||r_tab.table_id||';';
     END IF;
      IF r_con.cardinality = '1-N' THEN
         l_sql := l_sql || '
   -- Update constraint statistics
   UPDATE ds_constraints SET extract_count = l_tot_count WHERE con_id = '||r_con.con_id||';';
      END IF;
      IF p_commit THEN
      l_sql := l_sql || '
   COMMIT;';
...
--#end public
      ...
      CURSOR c_con (
        ...
        ,p_table_name ds_constraints.src_table_name%TYPE
         ...
         SELECT *
          ...
          ORDER BY CASE WHEN src_table_name = dst_table_name THEN 1 ELSE con_seq END -- pig's ear first
      ;
   ...
   BEGIN
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         handle_data_set(p_set_id=>p_set_id,p_oper=>'DIRECT-EXECUTE',p_mode=>'D',p_mask_data=>FALSE);
         delete_data_set_rowids( p_set_id);
      ...
      IF NVL(p_final_commit,FALSE) THEN
         COMMIT;
      END IF;
   END;
--#begin public
...
SELECT DISTINCT '||r_tab_dst.table_id||','||l_dst_alias||'.rowid
 ...
 WHERE '||NVL(r_con.where_clause,'1=1');
   ...
   AND ds_rec2.table_id IS NULL';
      ...
      END IF;
      shuffle_records(p_set_id=>p_set_id);
      generate_identifiers(p_set_id=>p_set_id);
      show_message('D','<-'||l_plunit);
   END;
--#begin public
   ---
...
--#end public
   ...
   BEGIN
      ...
      DELETE ds_masks WHERE msk_type = 'INHERIT';
...
-- UI (Upsert) mode = R)refresh (for backward compatibility)
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         IF l_mode IN ('D') THEN
            l_order := -1;
         ELSE
            propagate_pk_masking(p_set_id=>r_set.set_id);
            l_order := 1;
         END IF;
         <<table_loop>>
...
--               l_in_mem_seq := r_tab.sequence_name IS NOT NULL AND ds_utility_var.g_in_mem_seq_tab.EXISTS(LOWER(r_tab.sequence_name));
               ...
               END IF;
               IF l_update_mode OR l_delete_mode /*OR l_in_mem_seq*/ THEN
                  l_pk_name := get_table_pk(r_tab.table_name);
                  assert(l_pk_name IS NOT NULL,'Table '||r_tab.table_name||' has no primary key');
                  l_pk_columns := get_constraint_columns(l_pk_name);
               ...
               FOR l_pass_index IN REVERSE 1..NVL(r_tab.group_count,1) LOOP
                  ...
                  ELSIF p_oper LIKE '%SCRIPT%' THEN
                     /* Select rows to extract */
                     l_sel_columns := l_sel_columns||',rowid,rownum'; -- to allow them in mask SQL expressions
                     ...
                     LOOP
                        ...
                        ELSE
                           ...
                           l_ws := NULL;
                        END IF;
                        IF p_oper IN ('PREPARE-SCRIPT','EXECUTE-SCRIPT') OR l_refresh_mode THEN
                           l_sep := ';'||CHR(10);
                        ELSE
                           l_sep := NULL;
                        END IF;
...
--                           execute_immediate(p_sql=>'BEGIN ddl_utility.execute_immediate'||CASE WHEN p_db_link IS NOT NULL THEN '@'||LOWER(p_db_link) END||'(:1); END;',p_using=>'BEGIN'||CHR(10)||l_sql||CHR(10)||'END;');
                           execute_immediate(p_sql=>'BEGIN ddl_utility.execute_immediate'||CASE WHEN p_db_link IS NOT NULL THEN '@'||LOWER(p_db_link) END||'(:1); END;',p_using=>'BEGIN'||CHR(10)||l_sql||'END;');
--                           execute_immediate(l_sql);
                        ELSE
                           IF l_refresh_mode THEN
                              l_sql := l_sql || '/' || CHR(10);
     ...
     FOR i IN p_pk_size+1..l_col_names.COUNT LOOP
       ...
       IF MOD(j-1,p_columns_per_line) = 0 THEN
         IF l_select IS NOT NULL THEN
            ...
            l_select := l_select || RPAD(' ',p_left_tab-3);
         ELSE
            IF p_indent_first_line = 'Y' THEN
       ...
       ELSE
         ...
         ELSIF l_col_type = 'NUMBER' THEN
            ...
            IF ds_utility_var.g_seq.EXISTS(l_tab_col) AND ds_utility_var.g_seq(l_tab_col).id_shift_value IS NULL THEN
              ...
              l_select := l_select || 'ds_utility_krn.set_identifier(' || ds_utility_var.g_seq(l_tab_col).table_id || ',' || l_col_val || ',' || LOWER(ds_utility_var.g_seq(l_tab_col).sequence_name) || '.nextval)';
            ELSIF ds_utility_var.g_map.EXISTS(l_tab_col) AND ds_utility_var.g_map(l_tab_col).id_shift_value IS NULL THEN
              l_select := l_select || 'ds_utility_krn.get_identifier(' || ds_utility_var.g_map(l_tab_col).table_id || ',' || l_col_val || ')';
            ELSE
              ...
              IF lower(get_table_alias(p_table_name, NULL) || '_rec_nbr') = lower(l_col_name) THEN -- SYSPER
               -- set record number the same as pk -- SYSPER
                 l_select := l_select || 'ds_utility_krn.get_identifier(' || ds_utility_var.g_seq(  LOWER(p_table_name||'.'||get_table_alias(p_table_name, NULL) || '_id') ).table_id  || ',' || l_col_val_pk || ')'; -- SYSPER
...
--#end public
       ...
       FOR r_tab IN c_tab(r_set.set_id,l_order) LOOP
         ...
         THEN
            ...
            FOR l_pass_index IN REVERSE 1..r_tab.group_count LOOP
              ...
              l_sql :=
   '
   SELECT '||build_select_clause(r_tab.table_name,r_tab.table_alias,l_sel_columns,7,'N')||'
   FROM '||l_src_table_name||'
   WHERE 1=1';
              IF r_tab.extract_type != 'F' THEN
                l_sql := l_sql ||
   ...
   '||build_values_clause_for_ver_tb(r_tab.table_name,r_tab.table_alias,l_sel_columns,l_result,0,3,'Y')||'
   );'; --SYSPER
                 show_message('S',RTRIM('BEGIN ddl_utility.execute_immediate(:1); END; ',CHR(10)));
...
--            ELSE
               l_sel_columns := normalise_columns_list(r_tab.table_name,NVL(r_tab.columns_list,'*'));
--            END IF;
            l_sel_columns := build_select_for_subquery(r_tab.table_name,NULL/*r_tab.table_alias*/,'I',l_sel_columns,17,'N',3,NULL); -- added for forced/masked columns
            IF r_tab.extract_type = 'F' THEN
               l_sql :=
'
...
--#end public
   ...
   PROCEDURE create_drop_objects (
     ...
     ,p_mask_data BOOLEAN := TRUE
      ...
      IS
         SELECT usr.table_name, ds_tab.table_alias
          ...
          GROUP BY usr.table_name, ds_tab.table_alias
...
--                  END IF;
                  IF NVL(ds_utility_var.g_owner,USER) != USER THEN
                     l_source_schema := LOWER(ds_utility_var.g_owner);
                  END IF;
...
--#end public
   ...
   BEGIN
      create_drop_objects(
        ...
        ,p_set_type=>p_set_type,p_mask_data=>p_mask_data
      );
   END;
--
--#begin public
...
--#end public
   ...
   BEGIN
      ...
      l_exp_capture := NVL(INSTR(UPPER(NVL(r_set.capture_mode,'XML')),'EXP'),0)>0;
      FOR r_tab IN c_tab(p_set_id) LOOP
