...
CREATE TABLE ds_data_sets (
  ...
  ,set_type VARCHAR2(10) NOT NULL CONSTRAINT ds_set_set_type_ck CHECK (set_type IN ('SUB','SQL','CSV','JSON','XML','GEN','CAP'))
  ,system_flag VARCHAR2(1) NULL CONSTRAINT ds_set_system_flag_ck CHECK (NVL(system_flag,'Y') IN ('Y','N'))
  ,disabled_flag VARCHAR2(1) NULL CONSTRAINT ds_set_disabled_flag_ck CHECK (NVL(disabled_flag,'Y') IN ('Y','N'))
  ,visible_flag VARCHAR2(1) NULL CONSTRAINT ds_set_visible_flag_ck CHECK (NVL(visible_flag,'Y') IN ('Y','N'))
  ,capture_flag VARCHAR2(1) NULL CONSTRAINT ds_set_capture_flag_ck CHECK (NVL(capture_flag,'Y') IN ('Y','N'))
  ,capture_mode VARCHAR2(10) NULL CONSTRAINT ds_set_capture_mode_ck CHECK (NVL(capture_mode,'NONE') IN ('NONE','ASYN','SYNC'))
...
CREATE TABLE ds_tables (
  ...
  ,extract_type VARCHAR2(1) NOT NULL CONSTRAINT ds_tab_extract_type_ck CHECK (extract_type IN ('B','F','P','N','R'))
...
CREATE TABLE ds_constraints (
  ...
  ,cardinality VARCHAR2(3) NOT NULL CONSTRAINT ds_con_cardinality_ck CHECK (CARDINALITY IN ('1-N','N-1'))
  ,extract_type VARCHAR2(1) NOT NULL CONSTRAINT ds_con_extract_type_ck CHECK (extract_type IN ('B','P','N')) 
  ,deferred VARCHAR2(9) NOT NULL CONSTRAINT ds_con_deferred_ck CHECK (DEFERRED IN ('IMMEDIATE','DEFERRED'))
...
CREATE TABLE ds_masks (
  ...
  ,sensitive_flag VARCHAR2(1) NULL CONSTRAINT ds_msk_sensitive_flag_ck CHECK (NVL(sensitive_flag,'Y') IN ('Y','N'))
  ,disabled_flag VARCHAR2(1) NULL CONSTRAINT ds_msk_disabled_flag_ck CHECK (NVL(disabled_flag,'Y') IN ('Y','N'))
  ,locked_flag VARCHAR2(1) NULL CONSTRAINT ds_msk_locked_flag_ck CHECK (NVL(locked_flag,'Y') IN ('Y','N'))
  ...
  ,msk_type VARCHAR2(30) NULL CONSTRAINT ds_msk_msk_type_ck CHECK (msk_type IS NULL OR msk_type IN ('SQL','SHUFFLE','INHERIT','SEQUENCE','TOKENIZE'))
...
CREATE TABLE ds_patterns (
  ...
  ,logical_operator VARCHAR2(3) NULL CONSTRAINT ds_pat_logical_operator_ck CHECK(NVL(logical_operator,'OR') IN ('AND','OR'))
  ,system_flag VARCHAR2(1) NULL CONSTRAINT ds_pat_system_flag_ck CHECK (NVL(system_flag,'Y') IN ('Y','N'))
  ,disabled_flag VARCHAR2(1) NULL CONSTRAINT ds_pat_disabled_flag_ck CHECK (NVL(disabled_flag,'Y') IN ('Y','N'))
  ,remarks VARCHAR2(4000) NULL
  ,msk_type VARCHAR2(30) NULL CONSTRAINT ds_pat_msk_type CHECK (msk_type IS NULL OR msk_type IN ('SQL','SHUFFLE'))
