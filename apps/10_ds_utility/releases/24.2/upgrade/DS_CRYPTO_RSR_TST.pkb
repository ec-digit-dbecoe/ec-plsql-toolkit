CREATE OR REPLACE PACKAGE BODY ds_crypto_rsr_tst AS
---
...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_decrypt_string_dataset''))')) x)
--@#end encrypt_decrypt_string_for_loop
--@
--@   -- Test #tst.seq - tst.name
--@   PROCEDURE test_encrypt_decrypt_string_tst.seq IS
--@   BEGIN
--@      ds_crypto_krn.set_encryption_algo('RSR');
...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_string_dataset''))')) x)
...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_decrypt_number_dataset''))')) x)
...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_number_dataset''))')) x)
...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_decrypt_date_dataset''))')) x)
...
/*
--@#begin encrypt_decrypt_integer_dataset
name;value;min_value;max_value;key;exception
encrypt/decrypt integer: value not int;1.1;1;20;NULL;-20018
encrypt/decrypt integer: missing upper limit;1;1;NULL;NULL;-20019
encrypt/decrypt integer: missing lower limit;1;NULL;20;NULL;-20020
encrypt/decrypt integer: upper limit not int;1;1;20.1;NULL;-20021
encrypt/decrypt integer: lower limit not int;1;1.1;20;NULL;-20022
encrypt/decrypt integer: invalid range;1;20;1;NULL;-20023
encrypt/decrypt integer: value not in range;0;1;20;NULL;-20024
encrypt/decrypt integer: null value;CAST(NULL AS NUMBER);1;20;NULL;0
encrypt/decrypt integer: positive range;1234;1;4999;NULL;0
encrypt/decrypt integer: neg/pos range;1234;-4999;+4999;NULL;0
encrypt/decrypt integer: neg/pos range;-1234;-4999;+4999;NULL;0
encrypt/decrypt integer: negative range;-1234;-4999;-1;NULL;0
--@#end encrypt_decrypt_integer_dataset
*/
--@/*--#delete
--@#begin encrypt_decrypt_integer_for_loop
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_decrypt_integer_dataset''))')) x)
--@#end encrypt_decrypt_integer_for_loop
--@
--@   -- Test #tst.seq - tst.name
--@   PROCEDURE test_encrypt_decrypt_integer_tst.seq IS
--@   BEGIN
--@      IF tst.key IS NULL THEN
--@         ds_crypto_krn.set_encryption_key(gk_key);
--@      END IF;
--@      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>tst.value,p_min_value=>tst.min_value,p_max_value=>tst.max_value,p_key=>tst.key)
--@               ,p_min_value=>tst.min_value,p_max_value=>tst.max_value,p_key=>tst.key)).to_equal(tst.value);
--@   END;
--@#endfor
--#if 0
 
   -- Test #1 - encrypt/decrypt integer: value not int
   PROCEDURE test_encrypt_decrypt_integer_1 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1.1,p_min_value=>1,p_max_value=>20,p_key=>NULL)
               ,p_min_value=>1,p_max_value=>20,p_key=>NULL)).to_equal(1.1);
   END;
 
   -- Test #2 - encrypt/decrypt integer: missing upper limit
   PROCEDURE test_encrypt_decrypt_integer_2 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1,p_min_value=>1,p_max_value=>NULL,p_key=>NULL)
               ,p_min_value=>1,p_max_value=>NULL,p_key=>NULL)).to_equal(1);
   END;
 
   -- Test #3 - encrypt/decrypt integer: missing lower limit
   PROCEDURE test_encrypt_decrypt_integer_3 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1,p_min_value=>NULL,p_max_value=>20,p_key=>NULL)
               ,p_min_value=>NULL,p_max_value=>20,p_key=>NULL)).to_equal(1);
   END;
 
   -- Test #4 - encrypt/decrypt integer: upper limit not int
   PROCEDURE test_encrypt_decrypt_integer_4 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1,p_min_value=>1,p_max_value=>20.1,p_key=>NULL)
               ,p_min_value=>1,p_max_value=>20.1,p_key=>NULL)).to_equal(1);
   END;
 
   -- Test #5 - encrypt/decrypt integer: lower limit not int
   PROCEDURE test_encrypt_decrypt_integer_5 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1,p_min_value=>1.1,p_max_value=>20,p_key=>NULL)
               ,p_min_value=>1.1,p_max_value=>20,p_key=>NULL)).to_equal(1);
   END;
 
   -- Test #6 - encrypt/decrypt integer: invalid range
   PROCEDURE test_encrypt_decrypt_integer_6 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1,p_min_value=>20,p_max_value=>1,p_key=>NULL)
               ,p_min_value=>20,p_max_value=>1,p_key=>NULL)).to_equal(1);
   END;
 
   -- Test #7 - encrypt/decrypt integer: value not in range
   PROCEDURE test_encrypt_decrypt_integer_7 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>0,p_min_value=>1,p_max_value=>20,p_key=>NULL)
               ,p_min_value=>1,p_max_value=>20,p_key=>NULL)).to_equal(0);
   END;
 
   -- Test #8 - encrypt/decrypt integer: null value
   PROCEDURE test_encrypt_decrypt_integer_8 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>CAST(NULL AS NUMBER),p_min_value=>1,p_max_value=>20,p_key=>NULL)
               ,p_min_value=>1,p_max_value=>20,p_key=>NULL)).to_equal(CAST(NULL AS NUMBER));
   END;
 
   -- Test #9 - encrypt/decrypt integer: positive range
   PROCEDURE test_encrypt_decrypt_integer_9 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1234,p_min_value=>1,p_max_value=>4999,p_key=>NULL)
               ,p_min_value=>1,p_max_value=>4999,p_key=>NULL)).to_equal(1234);
   END;
 
   -- Test #10 - encrypt/decrypt integer: neg/pos range
   PROCEDURE test_encrypt_decrypt_integer_10 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>1234,p_min_value=>-4999,p_max_value=>+4999,p_key=>NULL)
               ,p_min_value=>-4999,p_max_value=>+4999,p_key=>NULL)).to_equal(1234);
   END;
 
   -- Test #11 - encrypt/decrypt integer: neg/pos range
   PROCEDURE test_encrypt_decrypt_integer_11 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
   ...
   -- Test #12 - encrypt/decrypt integer: negative range
   PROCEDURE test_encrypt_decrypt_integer_12 IS
   BEGIN
      IF NULL IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      ut.expect(ds_crypto_krn.decrypt_integer(p_value=>ds_crypto_krn.encrypt_integer(p_value=>-1234,p_min_value=>-4999,p_max_value=>-1,p_key=>NULL)
               ,p_min_value=>-4999,p_max_value=>-1,p_key=>NULL)).to_equal(-1234);
   END;
--#endif 0
--@*/--#delete
 
/*
--@#begin encrypt_integer_dataset
name;value;min_value;max_value;key;result;exception
encrypt integer: null value;CAST(NULL AS NUMBER);NULL;NULL;'This is another private key';CAST(NULL AS NUMBER);0
encrypt integer: pos range;1234;1;4999;'This is another private key';1061;0
encrypt integer: neg range;-1234;-4999;-1;'This is another private key';-3000;0
encrypt integer: mix range pos val;1234;-4999;4999;'This is another private key';-4719;0
encrypt integer: mix range neg val;-1234;-4999;4999;'This is another private key';-3000;0
--@#end encrypt_integer_dataset
*/
--@/*--#delete
--@#begin encrypt_integer_for_loop
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_RSR_TST'',''encrypt_integer_dataset''))')) x)
--@#end encrypt_integer_for_loop
--@
--@   -- Test #tst.seq - tst.name
--@   PROCEDURE test_encrypt_integer_tst.seq IS
--@      l_result integer;
--@   BEGIN
--@      ds_crypto_krn.set_encryption_algo('RSR');
--@      IF tst.key IS NULL THEN
--@         ds_crypto_krn.set_encryption_key(gk_key);
--@      END IF;
--@      l_result := ds_crypto_krn.encrypt_integer(p_value=>tst.value,p_min_value=>tst.min_value,p_max_value=>tst.max_value,p_key=>tst.key);
--@      ut.expect(l_result).to_equal(tst.result);
--@#if "tst.value" != "CAST(NULL AS NUMBER)"
--@      ut.expect(l_result).to_be_greater_or_equal(tst.min_value);
--@      ut.expect(l_result).to_be_less_or_equal(tst.max_value);
--@#endif
--@   END;
--@#endfor
--#if 0
 
   -- Test #1 - encrypt integer: null value
   PROCEDURE test_encrypt_integer_1 IS
      l_result integer;
   BEGIN
      ds_crypto_krn.set_encryption_algo('RSR');
      IF 'This is another private key' IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      l_result := ds_crypto_krn.encrypt_integer(p_value=>CAST(NULL AS NUMBER),p_min_value=>NULL,p_max_value=>NULL,p_key=>'This is another private key');
      ut.expect(l_result).to_equal(CAST(NULL AS NUMBER));
   END;
 
   -- Test #2 - encrypt integer: pos range
   PROCEDURE test_encrypt_integer_2 IS
      l_result integer;
   BEGIN
      ds_crypto_krn.set_encryption_algo('RSR');
      IF 'This is another private key' IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      l_result := ds_crypto_krn.encrypt_integer(p_value=>1234,p_min_value=>1,p_max_value=>4999,p_key=>'This is another private key');
      ut.expect(l_result).to_equal(1061);
      ut.expect(l_result).to_be_greater_or_equal(1);
      ut.expect(l_result).to_be_less_or_equal(4999);
   END;
 
   -- Test #3 - encrypt integer: neg range
   PROCEDURE test_encrypt_integer_3 IS
      l_result integer;
   BEGIN
      ds_crypto_krn.set_encryption_algo('RSR');
      IF 'This is another private key' IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      l_result := ds_crypto_krn.encrypt_integer(p_value=>-1234,p_min_value=>-4999,p_max_value=>-1,p_key=>'This is another private key');
      ut.expect(l_result).to_equal(-3000);
      ut.expect(l_result).to_be_greater_or_equal(-4999);
      ut.expect(l_result).to_be_less_or_equal(-1);
   END;
 
   -- Test #4 - encrypt integer: mix range pos val
   PROCEDURE test_encrypt_integer_4 IS
      l_result integer;
   BEGIN
      ds_crypto_krn.set_encryption_algo('RSR');
      IF 'This is another private key' IS NULL THEN
         ds_crypto_krn.set_encryption_key(gk_key);
      END IF;
      l_result := ds_crypto_krn.encrypt_integer(p_value=>1234,p_min_value=>-4999,p_max_value=>4999,p_key=>'This is another private key');
      ut.expect(l_result).to_equal(-4719);
      ut.expect(l_result).to_be_greater_or_equal(-4999);
      ut.expect(l_result).to_be_less_or_equal(4999);
   END;
 
   -- Test #5 - encrypt integer: mix range neg val
   PROCEDURE test_encrypt_integer_5 IS
      l_result integer;
   BEGIN
      ds_crypto_krn.set_encryption_algo('RSR');
      IF 'This is another private key' IS NULL THEN
