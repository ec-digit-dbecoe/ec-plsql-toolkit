set scan off
CREATE OR REPLACE AND RESOLVE JAVA SOURCE NAMED "com/privacylogistics/FF3Cipher" AS
 ...
 *
 * Modified by deboiph (European Commission):
 * - Commented out logging to remove dependency to log4j framework
 * - Set minimum length of plaintext to 2 bytes (against NITS recommendation)
 */
...
//      this.minLen = (int) Math.ceil(Math.log(DOMAIN_MIN) / Math.log(radix));
        this.minLen = 2; // deboiph: allow small strings despite NITS recommendation
     ...
     * @throws IllegalBlockSizeException internal error
     */
    //@SuppressWarnings("unused") //deboiph: gives SQL*Plus error: SP2-0556: Invalid file name.
