...
CREATE TABLE ds_data_sets (
  ...
  ,set_type VARCHAR2(10) NOT NULL CHECK (set_type IN ('NATIVE','SQL','CSV','JSON','XML'))
  ,system_flag VARCHAR2(1) NULL CHECK (NVL(system_flag,'Y') IN ('Y','N'))
  ,disabled_flag VARCHAR2(1) NULL CHECK (NVL(disabled_flag,'Y') IN ('Y','N'))
  ,visible_flag VARCHAR2(1) NULL CHECK (NVL(visible_flag,'Y') IN ('Y','N'))
  ,capture_flag VARCHAR2(1) NULL CHECK (NVL(capture_flag,'Y') IN ('Y','N'))
...
CREATE TABLE ds_patterns (
  ...
  ,disabled_flag VARCHAR2(1) NULL CHECK (NVL(disabled_flag,'Y') IN ('Y','N'))
  ,remarks VARCHAR2(4000) NULL
  ,msk_type VARCHAR2(30) NULL CHECK (msk_type IS NULL OR msk_type IN ('SQL','SHUFFLE'))
