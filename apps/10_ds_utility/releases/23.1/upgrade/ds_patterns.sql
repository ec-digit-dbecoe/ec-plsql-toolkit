...
insert into ds_patterns (
  ...
  ,'(bban)|((bank|account).*(no|nr|number|num|#))', 'Bank account number', '^[0-9]{3}[ -]?[0-9]{7}[ -]?(0[1-9]|[1-8][0-9]|9[0-7])$', ''
  ,NULL, 'OR', NULL
...
insert into ds_patterns (
  ...
  ,'', '', '^(6011\d{2}([- ])?\d{4}\1?\d{4}\1?\d{4}|(65\d{2})\d{2}([- ])?\d{4}\1?\d{4}\1?\d{4}|(64[4-9]\d)\d{2}([- ])?\d{4}\1?\d{3}\1?\d{3}|(6011\d{2})\d{2}([- ])?\d{3}\1?\d{3}\1?\d{3})$', ''
  ,10, 'OR', NULL
...
insert into ds_patterns (
  ...
  ,'(identity|id).*(no|nr|number|num|#)', 'Identity card number', '^[0-9]{3}[ -]?[0-9]{7}[ -]?(0[1-9]|[1-8][0-9]|9[0-7])$', ''
  ,NULL, 'OR', NULL
...
insert into ds_patterns (
  ...
  ,'(phone|fax|gsm|mobile).*(no|nr|number|num|#)', 'Phone number', '', ''
  ,NULL, 'OR', NULL
  ,'SQL',  q'#ds_masker_krn.obfuscate_string(p_string=>:column_name,p_seed=>:column_name)#'
);
