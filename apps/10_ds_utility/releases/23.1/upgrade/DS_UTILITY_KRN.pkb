...
--#end public
   ...
   BEGIN
      FOR r_ds IN c_ds LOOP
         ...
         l_sql := CASE WHEN r_ds.set_type='SQL' THEN r_ds.params ELSE 'SELECT * FROM TABLE(ds_utility_krn.read_csv_clob('||r_ds.set_id||'))' END;
...
--#end public
   ...
   BEGIN
      ...
      l_pos := NVL(INSTR(p_set_col_name,'.'),0);
      assert(l_pos>0,'Error: Parameter "p_set_col_name" format must be "set-name.col-name"');
      RETURN random_value_from_data_set(SUBSTR(p_set_col_name,1,l_pos-1),SUBSTR(p_set_col_name,l_pos+1),p_col_len,p_seed);
   END;
--#begin public
   ---
...
*/
   FUNCTION create_data_set_def (
     ...
     ,p_set_type IN ds_data_sets.set_type%TYPE := 'NATIVE'
...
--#end public
   ...
   BEGIN
      ...
      r_set.set_type := NVL(p_set_type,'NATIVE');
...
--#end public
   ...
   IS
      ...
      IS
         SELECT ds_pk_con.constraint_name pk_name, LOWER(ds_pk_col.table_name) pk_tab_name, LOWER(ds_pk_col.column_name) pk_col_name
              , LOWER(ds_fk_con.constraint_name||ds_fk_col.position) pk_tab_alias, ds_tab.table_alias
   ...
   BEGIN
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         FOR r_fk IN c_fk(r_set.set_id, NULL, NULL) LOOP
            r_fk.join_clause := build_join_clause(r_fk.pk_tab_name, r_fk.fk_tab_name, r_fk.fk_name);
            ...
            WHILE t_pk.COUNT > 0 LOOP
               ...
               FOR r_map IN c_map(r_set.set_id, t_pk(l_idx).table_name, t_pk(l_idx).column_name) LOOP
                  IF NOT ds_utility_var.g_map.EXISTS(r_map.table_name||'.'||r_map.column_name) THEN
                     ...
                     r_seq.column_name := r_seq.column_name;
   ...
   --         END IF;
         ...
         FOR c IN 1..t_pk_cols.COUNT LOOP
            ...
            r_pk := ds_utility_var.g_pk_tab(r_fk.pk_tab_name||'.'||r_fk.pk_col_name);
            LOOP
               r_fk.join_clause := regexp_replace(r_fk.join_clause,'([^A-Za-z0-9_]|^)('||r_pk.pk_tab_name||')([^A-Za-z0-9_]|$)','\1'||r_pk.pk_tab_alias||'\3',1,0,'i');
               r_fk.join_clause := regexp_replace(r_fk.join_clause,'([^A-Za-z0-9_]|^)('||r_fk.fk_tab_name||')([^A-Za-z0-9_]|$)','\1'||p_table_alias||'\3',1,0,'i');
               l_sql := l_sql
...
--#end public
   ...
   PROCEDURE create_drop_objects (
     ...
     ,p_set_type IN ds_data_sets.set_type%TYPE := 'NATIVE' -- only this data set type
...
*/
   PROCEDURE create_views (
     ...
     ,p_mode IN VARCHAR2 := NULL -- (S)tatic or (D)ynamic
     ,p_set_id IN ds_data_sets.set_id%TYPE := NULL -- data set id (null for all)
     ,p_set_type IN ds_data_sets.set_type%TYPE := 'NATIVE' -- data set type (null for all)
     ,p_mask_data IN BOOLEAN := TRUE -- mask data?
   )
--#end public
   IS
...
--#end public
   ...
   BEGIN
      create_drop_objects(
       ...
       , p_mode=>p_mode, p_non_empty_only=>FALSE, p_set_id=>p_set_id,p_set_type=>'NATIVE'
      );
   END;
--
--#begin public
...
--#end public
   ...
   BEGIN
      create_drop_objects(
       ...
       , p_object_prefix=>p_policy_prefix, p_table_prefix=>p_table_prefix, p_full_schema=>p_full_schema
       , p_non_empty_only=>FALSE, p_set_id=>p_set_id,p_set_type=>'NATIVE'
      );
   END;
--
--#begin public
