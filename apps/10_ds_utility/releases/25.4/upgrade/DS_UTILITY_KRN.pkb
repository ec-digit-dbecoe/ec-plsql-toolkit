...
'UPDATE ds_records ds_rec
   SET remark = (';
         FOR r_col IN c_col(r_con.src_table_name) LOOP
            l_sql := l_sql || CHR(10)
                  || CASE WHEN r_col.position = 1 THEN '          SELECT ds_rec.remark || '' '||r_con.arrow||' ' ELSE '              || '',' END
                  || r_col.column_name || '=''||' ||r_col.column_name;
         END LOOP;
         l_sql := l_sql ||
'
...
--#end public
   ...
   ds_utility_krn.handle_data_set(p_set_id=>$set_id$,p_oper=>'DIRECT-EXECUTE',p_mode=>'D',p_mask_data=>FALSE, p_log_run=>FALSE);
