         ...
         IF c_msk%FOUND THEN
            UPDATE ds_masks
               SET sensitive_flag = 'Y'
                 ...
                 , gen_params = REPLACE(REPLACE(REPLACE(REPLACE(r_pat.gen_params,':column_name',LOWER(r_col.column_name)),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
             WHERE CURRENT OF c_msk
            ;
         ELSE
            INSERT INTO ds_masks (
             ...
             , gen_type, gen_params
               ...
               , NULL
               , r_pat.gen_type
               , REPLACE(REPLACE(REPLACE(REPLACE(r_pat.gen_params,':column_name',LOWER(r_col.column_name)),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
            );
         END IF;
         CLOSE c_msk;
      END;
