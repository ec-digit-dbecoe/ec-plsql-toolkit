...
--#end public
   IS
   BEGIN
      ds_utility_var.g_alias_like_pattern := p_alias_regexp_like_pattern;
   END;
--
--#begin public
   ---
...
--#end public
   IS
   BEGIN
      ds_utility_var.g_alias_replace_pattern := p_alias_regexp_replace_pattern;
   END;
--
--#begin public
   ---
...
--#end public
   IS
   BEGIN
      ds_utility_var.g_alias_constraint_type := p_alias_constraint_type;
   END;
--
--#begin public
/**
...
--#end public
      ...
      IS
         SELECT tok.token
          ...
          INNER JOIN ds_masks msk
             ON msk.msk_id = tok.msk_id
            AND msk.table_name = UPPER(p_table_name)
            AND msk.column_name = UPPER(p_column_name)
          WHERE tok.value = p_value
         ;
   ...
   BEGIN
      ...
      IF p_value IS NULL THEN
         RETURN NULL;
      END IF;
      OPEN c_tok(CASE WHEN ds_utility_var.g_encrypt_tokenized_values THEN ds_masker_krn.encrypt_string(p_value) ELSE p_value END);
...
--#end public
   ...
   BEGIN
      ...
      CLOSE c_tok;
      RETURN CASE WHEN ds_utility_var.g_encrypt_tokenized_values THEN ds_masker_krn.decrypt_string(l_value) ELSE l_value END;
   END;
   --
--#begin public
/**
   ...
   BEGIN
      ...
      IF r_set.set_id IS NOT NULL THEN
         ...
         update_data_set_def_properties(
           ...
           ,p_capture_user=>p_capture_user
           ,p_params=>p_params
           ,p_raise_error_when_no_update=>FALSE
         );
      ELSE
...
END;#','$msk_params$',replace_i(r_col.params,'SEED','rownum')),'$table_name$',r_col.table_name),'$column_name$',r_col.column_name)
      ...
      ,'$msk_options$',r_col.options),'$table_name_lc$',LOWER(r_col.table_name)),'$column_name_lc$',LOWER(r_col.column_name));
         execute_immediate(l_sql);
         reset_seed(p_seed);
      END LOOP;
      IF NVL(p_commit,FALSE) THEN
...
--#end public
   ...
   BEGIN
      ...
      -- Get make from DB (in case cache is not up to date)
      IF r_msk.msk_id IS NULL THEN
         OPEN c_msk(l_table_name, l_column_name);
         FETCH c_msk INTO r_msk;
         CLOSE c_msk;
         ds_utility_var.g_msk(l_table_name)(l_column_name) := r_msk;
      END IF;
      -- Create mask in DB and cache
      IF r_msk.msk_id IS NULL THEN
         INSERT INTO ds_masks (
         ...
         ds_utility_var.g_msk(r_msk.table_name)(r_msk.column_name) := r_msk;
      END IF;
