...
--#end public
   ...
   BEGIN
      ...
      IF r_msk.msk_id IS NULL THEN
         ...
         ds_utility_var.ga_msk(r_msk.table_name||'.'||r_msk.column_name) := r_msk;
         ds_utility_var.g_msk_date_time := SYSDATE;
      END IF;
      RETURN r_msk;
   END;
   ---
...
SELECT *
  FROM (';
         ...
         l_sql := l_sql ||
'
SELECT '||r_tab.table_id||' table_id, rowid record_rowid, 1 pass_count, 1 seq
       ...
       , p_pass_mult IN INTEGER
         ...
         SELECT *
          ...
          WHERE table_id = p_table_id
            AND NVL(deleted_flag,'N') = 'N'
            FOR UPDATE OF pass_count
          ORDER BY record_rowid, DECODE(con_id,NULL,1,2) /*to keep base records*/, pass_count * p_pass_mult /* +1 for ASC, -1 for DESC*/
      ;
...
--            FOR UPDATE OF pass_count
      ...
      CURSOR c_rec (
        ...
        ,p_pass_count ds_records.pass_count%TYPE
          ...
          SELECT ROWIDTOCHAR(record_rowid) record_rowid, seq
            FROM ds_records
           WHERE table_id = p_table_id
             AND pass_count = p_pass_count
             AND NVL(deleted_flag,'N') = 'N'
         ...
         WHILE l_count > 0 LOOP
            ...
            FOR r_tab_src IN c_tab(p_set_id, CASE WHEN i=1 THEN 1/*ASC*/ ELSE -1/*DESC*/ END) LOOP
               ...
               FOR r_con IN c_con(p_set_id,r_tab_src.table_name,l_cardinality,l_extract_type) LOOP
                  ...
                  l_row_count := 0;
                  l_seq_mult := 1; --CASE WHEN r_con.cardinality = '1-N' THEN -1 ELSE 1 END;
...
SELECT '||l_distinct||r_tab_dst.table_id||','||l_dst_alias||'.rowid
               ...
               ,'||CASE WHEN r_tab_src.extract_type != 'F' THEN NVL(r_rec.seq,1) + 1 * l_seq_mult ELSE 1/*TBD*/ END||'
               ,'''||r_rec.record_rowid||'''
...
SELECT DISTINCT '||r_tab_dst.table_id||','||l_dst_alias||'.rowid
               ,'||TO_CHAR(r_tab_dst.pass_count+1)||','||r_con.con_id||'
               ,'||CASE WHEN r_tab_src.extract_type != 'F' THEN 'NVL(ds_rec.seq,1) + 1 * '||l_seq_mult ELSE '1/*TBD*/' END||'
               ,ds_rec.record_rowid
