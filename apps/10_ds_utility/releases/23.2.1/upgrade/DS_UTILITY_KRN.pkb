   ...
   BEGIN
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         FOR r_seq IN c_seq(r_set.set_id) LOOP
            show_message('D','Using '||CASE WHEN r_seq.in_mem_seq_flag = 'Y' THEN 'in-memory' ELSE 'Oracle' END
                       ||' sequence '||r_seq.sequence_name||' to generate value for '||r_seq.table_Name||'.'||r_seq.column_name);
...
--print_key(r_pk);
      ...
      IF p_target_db_link IS NOT NULL AND (instr_ignore_case(l_select,'.encrypt')>0 OR instr_ignore_case(l_select,'.decrypt')>0) THEN
         l_sql := l_sql
...
--            AND NVL(ds_tab.in_mem_seq_flag,'N') = 'Y'
   ...
   BEGIN
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         FOR r_tab IN c_tab(r_set.set_id) LOOP
            show_message('D','Generating identifiers for table '||r_tab.table_name);
            l_pk_name := get_table_pk(r_tab.table_name);
            assert(l_pk_name IS NOT NULL,'Table '||r_tab.table_name||' has no primary key');
            l_pk_columns := get_constraint_columns(l_pk_name);
--            l_src_table_name := gen_full_table_name(r_tab.table_name,r_tab.source_schema,r_tab.source_db_link);
...
--#end public
   ...
   BEGIN
      ...
      DELETE ds_masks WHERE deleted_flag = 'Y';
   END;
--#begin public
/**
