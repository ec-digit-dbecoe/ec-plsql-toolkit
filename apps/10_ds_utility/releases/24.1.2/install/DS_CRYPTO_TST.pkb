...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_TST'',''encrypt_decrypt_integer_dataset''))')) x)
...
--@#for tst IN (SELECT rownum "seq", x.* FROM TABLE(readcsv('SELECT * FROM TABLE(gen_utility.get_custom_code(''PACKAGE BODY'',''DS_CRYPTO_TST'',''encrypt_integer_dataset''))')) x)
