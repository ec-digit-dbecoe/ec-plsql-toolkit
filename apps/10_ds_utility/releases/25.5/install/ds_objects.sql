...
COMMENT ON COLUMN ds_tab_columns.gen_params IS 'Generation parameters';
...
COMMENT ON COLUMN ds_records.remarks IS 'Comment';
...
CREATE TABLE ds_masks (
  ...
  ,gen_params VARCHAR2(4000 CHAR) NULL
  ,tde_type VARCHAR2(30 CHAR) NULL CONSTRAINT ds_msk_tde_type_ck CHECK (tde_type IS NULL OR tde_type IN ('SQL','INHERIT'))
...
COMMENT ON COLUMN ds_masks.msk_params IS 'Mask parameters';
...
COMMENT ON COLUMN ds_masks.tde_params IS 'Encryption type';
