...
--#begin public
   ...
   FUNCTION graph_data_set (
    ...
    , p_set_type IN ds_data_sets.set_type%TYPE := NULL -- set type when set_id is NULL
    , p_log_run IN VARCHAR2 := 'Y'           -- log run?
   )
   ...
   IS
      ...
      IS
         ...
         SELECT col.column_id col_seq, col.column_name col_name, col.nullable
              ...
              , CASE NVL(p_set_type,'SUB') WHEN 'SUB' THEN ds_msk.msk_type WHEN 'TDE' THEN ds_msk.tde_type END col_type
              , ds_msk.msk_id, ds_msk.sensitive_flag, ds_msk.locked_flag
              , ds_msk.dependent_flag, ds_msk.disabled_flag, ds_msk.deleted_flag
              , CASE NVL(p_set_type,'SUB') WHEN 'SUB' THEN ds_msk.msk_params WHEN 'TDE' THEN ds_msk.tde_params END params
              , ds_msk.shuffle_group, ds_msk.partition_bitmap
              ...
              , ds_msk.msk_type, ds_msk.msk_params
              , ds_msk.gen_type, ds_msk.gen_params
              , ds_msk.tde_type, ds_msk.tde_params
         ...
         SELECT col.column_id col_seq, col.column_name col_name, col.nullable
              ...
              , ds_col.gen_params params, NULL shuffle_group, NULL partition_bitmap
              , NULL options, NULL pat_cat, NULL pat_name
              , NULL remarks, NULL values_sample
              , ds_col.null_value_pct, ds_col.null_value_condition
              , CASE WHEN con.column_name IS NOT NULL THEN 'Y' END has_cons
              , CASE WHEN ind.column_name IS NOT NULL THEN 'Y' END has_ind
              , NULL msk_type, NULL msk_params
              , ds_col.gen_type, ds_col.gen_params
              , NULL tde_type, NULL tde_params
           FROM sys.all_tab_columns col
           LEFT OUTER JOIN (
                  SELECT DISTINCT ccol.table_name, ccol.column_name
                    FROM sys.all_constraints con
      ...
      l_delim VARCHAR2(2);
      l_del BOOLEAN;
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'graph_data_set';
...
--          END IF;
       ...
       IS
          ...
          END;
       BEGIN
--          add_param('gen_type',pr_col.gen_type,TRUE);
--          add_param('gen_params',pr_col.gen_params,FALSE,TRUE);
       ...
       BEGIN
          ...
          IF r_set.set_type = 'GEN' OR p_set_type = 'GEN' THEN
             add_param('gen_type',pr_col.gen_type,FALSE,TRUE);
             add_param('gen_params',pr_col.gen_params,FALSE,TRUE);
          ELSIF p_set_type = 'TDE' THEN
             add_param('tde_type',pr_col.tde_type,FALSE,TRUE);
             add_param('tde_params',pr_col.tde_params,FALSE,TRUE);
          ELSE
             add_param('msk_type',pr_col.msk_type,TRUE);
             add_param('msk_params',pr_col.msk_params,FALSE,TRUE);
          END IF;
   ...
   BEGIN
      IF p_log_run = 'Y' THEN
         -- Start run
         l_params := 'SELECT * FROM TABLE(ds_utility_ext.'||k_routine_name||'(';
         ...
         l_params := l_params || ', p_full_schema=>'||CASE WHEN p_full_schema IS NULL THEN 'NULL' ELSE ''''||p_full_schema||'''' END;
         l_params := l_params || ', p_show_aliases=>'||CASE WHEN p_show_aliases IS NULL THEN 'NULL' ELSE ''''||p_show_aliases||'''' END;
         l_params := l_params || ', p_show_legend=>'||CASE WHEN p_show_legend IS NULL THEN 'NULL' ELSE ''''||p_show_legend||'''' END;
         l_params := l_params || ', p_show_conf_columns=>'||CASE WHEN p_show_conf_columns IS NULL THEN 'NULL' ELSE ''''||p_show_conf_columns||'''' END;
         l_params := l_params || ', p_show_cons_columns=>'||CASE WHEN p_show_cons_columns IS NULL THEN 'NULL' ELSE ''''||p_show_cons_columns||'''' END;
         l_params := l_params || ', p_show_ind_columns=>'||CASE WHEN p_show_ind_columns IS NULL THEN 'NULL' ELSE ''''||p_show_ind_columns||'''' END;
         l_params := l_params || ', p_show_all_columns=>'||CASE WHEN p_show_all_columns IS NULL THEN 'NULL' ELSE ''''||p_show_all_columns||'''' END;
         l_params := l_params || ', p_hide_dis_columns=>'||CASE WHEN p_hide_dis_columns IS NULL THEN 'NULL' ELSE ''''||p_hide_dis_columns||'''' END;
         l_params := l_params || ', p_column_name=>'||CASE WHEN p_column_name IS NULL THEN 'NULL' ELSE ''''||p_column_name||'''' END;
         l_params := l_params || ', p_show_column_keys=>'||CASE WHEN p_show_column_keys IS NULL THEN 'NULL' ELSE ''''||p_show_column_keys||'''' END;
         l_params := l_params || ', p_show_column_types=>'||CASE WHEN p_show_column_types IS NULL THEN 'NULL' ELSE ''''||p_show_column_types||'''' END;
         l_params := l_params || ', p_show_stats=>'||CASE WHEN p_show_stats IS NULL THEN 'NULL' ELSE ''''||p_show_stats||'''' END;
         l_params := l_params || ', p_show_config=>'||CASE WHEN p_show_config IS NULL THEN 'NULL' ELSE ''''||p_show_config||'''' END;
         l_params := l_params || ', p_show_constraints=>'||CASE WHEN p_show_constraints IS NULL THEN 'NULL' ELSE ''''||p_show_constraints||'''' END;
         l_params := l_params || ', p_show_indexes=>'||CASE WHEN p_show_indexes IS NULL THEN 'NULL' ELSE ''''||p_show_indexes||'''' END;
         l_params := l_params || ', p_show_triggers=>'||CASE WHEN p_show_triggers IS NULL THEN 'NULL' ELSE ''''||p_show_triggers||'''' END;
         l_params := l_params || ', p_show_all_props=>'||CASE WHEN p_show_all_props IS NULL THEN 'NULL' ELSE ''''||p_show_all_props||'''' END;
         l_params := l_params || ', p_show_prop_regexp=>'||CASE WHEN p_show_prop_regexp IS NULL THEN 'NULL' ELSE ''''||p_show_prop_regexp||'''' END;
         l_params := l_params || ', p_hide_prop_regexp=>'||CASE WHEN p_hide_prop_regexp IS NULL THEN 'NULL' ELSE ''''||p_hide_prop_regexp||'''' END;
         l_params := l_params || ', p_graph_att=>'||CASE WHEN p_graph_att IS NULL THEN 'NULL' ELSE ''''||p_graph_att||'''' END;
         l_params := l_params || ', p_node_att=>'||CASE WHEN p_node_att IS NULL THEN 'NULL' ELSE ''''||p_node_att||'''' END;
         l_params := l_params || ', p_edge_att=>'||CASE WHEN p_edge_att IS NULL THEN 'NULL' ELSE ''''||p_edge_att||'''' END;
         l_params := l_params || ', p_main_att=>'||CASE WHEN p_main_att IS NULL THEN 'NULL' ELSE ''''||p_main_att||'''' END;
         l_params := l_params || ', p_legend_att=>'||CASE WHEN p_legend_att IS NULL THEN 'NULL' ELSE ''''||p_legend_att||'''' END;
         l_params := l_params || ', p_set_type=>'||CASE WHEN p_set_type IS NULL THEN 'NULL' ELSE ''''||p_set_type||'''' END;
         l_params := l_params || '));';
         l_run_id := ds_utility_krn.start_run(p_routine_name=>k_routine_name, p_set_id=>p_set_id, p_params=>l_params);
      END IF;
      ...
      IF p_set_id IS NULL THEN
         IF p_set_type = 'TDE' THEN
            PIPE ROW(line(6, 'label=<<b>'||CASE WHEN p_full_schema='Y' THEN 'All tables and encrypted columns' ELSE 'Encrypted tables and columns' END||'</b>>'));
         ELSE
...
--                                                  || CASE WHEN r_col.col_type IS NOT NULL AND NVL(r_set.set_type,'SUB')='SUB' THEN '⧞' END
         ...
         IF p_show_stats = 'Y' THEN
            ...
            ELSIF r_tab.extract_type IS NOT NULL THEN
               IF r_tab.extract_count IS NOT NULL THEN --r_tab.extract_count != r_tab.source_count THEN
                  l_tab_label := r_tab.extract_count || ' / ' || r_tab.source_count
                              || l_tab_label || plural(' row',r_tab.extract_count)
                              || CASE r_set.set_type WHEN 'SUB' THEN ' extracted' WHEN 'GEN' THEN ' generated' END;
               ELSE
                  l_tab_label := r_tab.source_count
                              || plural(' row',r_tab.source_count);
               END IF;
            END IF;
         END IF;
         IF l_tab_label IS NOT NULL THEN
            l_node_att := l_node_att || '|Stats: ' || l_tab_label;
         END IF;
         ...
         FOR r_con IN c_con(r_tab.ora_table_name) LOOP
            ...
            l_con_hlabel := '';
            IF p_show_stats = 'Y' AND r_con.extract_type IN ('B','P') AND r_con.extract_count > 0 THEN
               l_con_hlabel := ' \n' || r_con.extract_count || '\n ';
            END IF;
      ...
      IF NVL(p_show_legend,'Y') = 'Y' THEN
         l_col_leg := '';
         l_conf_type := CASE NVL(r_set.set_type,'SUB') WHEN 'SUB' THEN CASE WHEN p_set_type = 'TDE' THEN 'encryption' ELSE 'mask' END WHEN 'GEN' THEN 'gen' END;
         IF l_columns THEN
         ...
         IF NVL(r_set.set_type,'SUB') = 'SUB' THEN
            ...
            ELSE
               ...
               ELSE
                  PIPE ROW(line(6, 'XTABLE [label="{X-TABLE\n  '||CASE p_set_type WHEN 'TDE' THEN 'Encrypted' ELSE 'Masked' END ||' table   '||l_col_leg||'}" fillcolor=grey fontcolor=black color=black]'));
               END IF;
            END IF;
            IF p_set_id IS NOT NULL THEN
      ...
      PIPE ROW(line(0,'}'));
      IF p_log_run = 'Y' THEN
         ds_utility_krn.stop_run(p_routine_name=>k_routine_name, p_run_id=>l_run_id);
      END IF;
--   EXCEPTION
--      WHEN OTHERS THEN
--         IF p_log_run = 'Y' THEN
--            IF SQLCODE = -6548 THEN --ORA-06548: no more rows needed
--               ds_utility_krn.stop_run(p_routine_name=>k_routine_name, p_run_id=>l_run_id);
--            ELSE
--               ds_utility_krn.stop_run(p_routine_name=>k_routine_name, p_run_id=>l_run_id, p_exit_code=>SQLCODE, p_error_msg=>SQLERRM||CHR(10)||SYS.DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
--            END IF;
--         END IF;
--         RAISE;
   END graph_data_set;
--#begin public
   ...
   PROCEDURE execute_degpl (
    ...
    , p_commit IN BOOLEAN := FALSE -- commit?
...
--      ;
      ...
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'execute_degpl';
      l_params ds_runs.params%TYPE;
      l_run_id ds_runs.run_id%TYPE;
      l_tde_updated BOOLEAN := FALSE;
...
--         l_set_id := NVL(p_set_id,l_set_id);
      ...
      BEGIN
         ...
         get_col_value_from_param('gen_params', r_col.gen_params, l_update);
      ...
      BEGIN
         ...
         get_msk_value_from_param('msk_params', r_msk.msk_params, l_update);
         ...
         get_msk_value_from_param('gen_type', r_msk.gen_type, l_update);
         get_msk_value_from_param('gen_params', r_msk.gen_params, l_update);
         get_msk_value_from_param('tde_type', r_msk.tde_type, l_update);
         l_tde_updated := l_tde_updated OR l_update;
         get_msk_value_from_param('tde_params', r_msk.tde_params, l_update);
         l_tde_updated := l_tde_updated OR l_update;
      ...
      BEGIN
         ...
         add_col_param('gen_p)arams','OXC4000');
         add_col_param('gen_t)ype','OUC3');
         ...
         add_msk_param('gen_t)ype','OUC30');
         add_msk_param('gen_p)arams','OXC4000');
         add_msk_param('l)ocked_flag','OUF1');
         add_msk_param('msk_p)arams','OXC4000');
         add_msk_param('msk_t)ype','OUC30');
         ...
         add_msk_param('tde_p)arams','OXC4000');
         add_msk_param('tde_t)ype','OUC30');
   ...
   BEGIN
      IF p_log_run THEN
         -- Start run
         l_params := 'exec ds_utility_ext.'||k_routine_name||'(';
         l_params := l_params || 'p_code=>''[NOTLOGGED]''';
         l_params := l_params || ', p_table_name=>'||CASE WHEN p_table_name IS NULL THEN 'NULL' ELSE ''''||p_table_name||'''' END;
         l_params := l_params || ', p_commit=>'||CASE WHEN p_commit IS NULL THEN 'NULL' WHEN p_commit THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ')';
         l_run_id := ds_utility_krn.start_run(p_routine_name=>k_routine_name, p_set_id=>NULL, p_params=>l_params);
      END IF;
      initially;
      -- Copy path
      l_line := TRIM(p_code);
