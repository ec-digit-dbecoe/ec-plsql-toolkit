...
--
   ...
   END;
   ---
   --  Send CLOB content to dbms_output
   ---
   ...
   IS
       ...
       l_last_nl    INTEGER;  -- Position of last newline in the chunk
   BEGIN
       -- Print the CLOB in chunks
       WHILE l_offset <= DBMS_LOB.GETLENGTH(p_clob) LOOP
           ...
           l_last_nl := INSTR(l_chunk, CHR(10), -1); 
           IF l_last_nl > 0 THEN
               -- Print only up to the last newline
               DBMS_OUTPUT.PUT_LINE(SUBSTR(l_chunk, 1, l_last_nl));
               -- Move offset to start from after the newline
               l_offset := l_offset + l_last_nl;
           ELSE
               -- No newline found, print the whole chunk and move offset normally
...
--#end public
   IS
   BEGIN
      ds_utility_var.g_log_mask := UPPER(SUBSTR(p_log_mask,1,5));
   END;
--#begin public
/**
   ...
   PROCEDURE insert_log (
     ...
     ,p_new_line BOOLEAN := FALSE
      ...
      CURSOR c_out IS
         SELECT NVL(MAX(line),1)
           FROM ds_run_logs
          WHERE run_id = ds_utility_var.g_run_id
      ;
   BEGIN
      IF ds_utility_var.g_run_id IS NULL THEN
         RETURN;
      END IF;
      -- Get last line number
      IF ds_utility_var.g_log_line IS NULL THEN
         OPEN c_out;
         FETCH c_out INTO ds_utility_var.g_log_line;
         CLOSE c_out;
      END IF;
      assert(ds_utility_var.g_log_line IS NOT NULL,'Cannot guess line number');
      -- Try to append first
      UPDATE ds_run_logs
         SET text = text || p_text
       WHERE run_id = ds_utility_var.g_run_id
         AND line = ds_utility_var.g_log_line;
      -- Insert if not found
      IF SQL%NOTFOUND THEN
         INSERT INTO ds_run_logs (
            run_id, line, text
         ) VALUES (
            ds_utility_var.g_run_id, ds_utility_var.g_log_line, p_text
         );
      END IF;
      -- Increment line number if required
      IF p_new_line THEN
         ds_utility_var.g_log_line := ds_utility_var.g_log_line + 1;
      END IF;
      -- Save work
      COMMIT;
   END;
...
--#end public
      ...
      ELSE
         ...
         ELSE
            ...
            ELSIF p_output = 'DS_OUTPUT' THEN
               insert_output(p_text,p_new_line);
            ELSIF p_output = 'DS_RUN_LOGS' THEN
               insert_log(p_text,p_new_line);
            ELSE
   ...
   IS
      ...
      k_output CONSTANT VARCHAR2(11) := 'DS_RUN_LOGS';
   BEGIN
      IF NVL(INSTR(ds_utility_var.g_log_mask,l_type),0) <= 0 AND l_type != 'T' THEN
         -- Do not log
         RETURN;
      END IF;
      IF l_type = 'I' THEN
         put('Info: ',FALSE,k_output);
      ELSIF l_type = 'W' THEN
         put('Warning: ',FALSE,k_output);
      ELSIF l_type = 'E' THEN
         put('Error: ',FALSE,k_output);
      END IF;
      IF l_type = 'D' THEN
        IF ds_utility_var.g_show_time AND ds_utility_var.g_time_mask IS NOT NULL THEN
           put(TO_CHAR(SYSDATE,ds_utility_var.g_time_mask)||' '||p_text, p_new_line,k_output);
        ELSE
           put(p_text, p_new_line,k_output);
        END IF;
      ELSE
         put(p_text, p_new_line,k_output);
      END IF;
...
--#end public
   IS
      l_type VARCHAR2(1) := UPPER(SUBSTR(p_type,1,1));
   BEGIN
      log_message(p_type, p_text, p_new_line);
...
--#end public
   IS
      PRAGMA autonomous_transaction;
      -- Cursor to get the name of a data set using its id
      CURSOR c_set (
         p_set_id ds_runs.set_id%TYPE
      )
      IS
         SELECT set_name
           FROM ds_data_sets
      ...
      ;
      -- Cursor to generate a run id from sequence
      CURSOR c_run IS
         SELECT ds_run_seq.nextval run_id
           FROM dual
      ;
      l_set_name ds_data_sets.set_name%TYPE;
      l_run_id ds_runs.run_id%TYPE;
      l_found BOOLEAN;
   BEGIN
      assert(p_routine_name IS NOT NULL, 'Internal error: routine name is mandatory!');
      -- Check set id and get its name (if specified)
      IF p_set_id IS NOT NULL THEN
         OPEN c_set(p_set_id);
         FETCH c_set INTO l_set_name;
         l_found := c_set%FOUND;
         CLOSE c_set;
         assert(l_found,'Error: set with id "'||p_set_id||'" does not exist!');
      END IF;
      -- Generate run id
      OPEN c_run;
      ...
      INSERT INTO ds_runs (
       ...
       , params
         ...
         l_run_id, p_routine_name
       , p_set_id, l_set_name
       , SYSDATE, 'ONGOING'
       , p_params
      );
      -- Save into cache
      ds_utility_var.g_run_id := l_run_id;
      ds_utility_var.g_routine_name := p_routine_name;
      COMMIT;
      RETURN l_run_id;
   END;
--#begin public
...
--#end public
   IS
      PRAGMA autonomous_transaction;
      l_error_msg ds_runs.error_msg%TYPE;
   BEGIN
      IF ds_utility_var.g_routine_name = p_routine_name THEN
         l_error_msg := NVL(p_error_msg, CASE WHEN NVL(p_exit_code,0)!=0 THEN SQLERRM(p_exit_code) END);
         UPDATE ds_runs
            SET end_time = SYSDATE
              , exit_code = p_exit_code
              , error_msg = l_error_msg
              , status = CASE p_exit_code WHEN 0 THEN 'SUCCESS' ELSE 'FAILURE' END
          WHERE run_id = NVL(p_run_id, ds_utility_var.g_run_id)
...
--#end public
   ...
   BEGIN
      ...
      FOR i IN 1..l_col_names.COUNT LOOP
         ...
         ELSE
            l_col_rec.col_val := format_column_value(p_part,p_table_name,l_col_rec.column_name,l_col_rec.data_type,REPLACE(extract_string(p_data,i),'\~','~'));
         END IF;
...
--         END IF;
      ...
      FOR i IN 1..l_col_names.COUNT LOOP
         ...
         IF p_part IN ('S','I','X') THEN
            ...
            l_idx := i - p_pk_size;
            IF p_columns_per_line>0 AND MOD(l_idx-1,p_columns_per_line) = 0 THEN
               IF l_out IS NOT NULL THEN
                  l_out := l_out || CHR(10);
                  l_out := l_out || RPAD(' ',p_left_tab-3);
               ELSE
         ...
         ELSIF p_part = 'X' THEN
            ...
            END IF;
         END IF;
         IF p_column_alias THEN
            l_out := l_out || ' ' || l_col_rec.column_name;
         END IF;
         <<next_col>>
         NULL;
      END LOOP;
   ...
   FUNCTION build_values_clause (
     ...
     ,p_columns_per_line IN INTEGER := 3
     ,p_db_link IN VARCHAR2 := NULL -- for remote script execution
     ,p_column_alias IN BOOLEAN := FALSE
   )
   RETURN VARCHAR2
   IS
   BEGIN
      RETURN build_sql_statement_part('I',p_table_name,p_table_alias,'I',p_columns_list,p_left_tab,p_indent_first_line,p_columns_per_line,p_db_link,p_data,p_pk_size,0,p_column_alias);
   END;
   ---
   ...
   FUNCTION build_set_and_where_clauses (
     ...
     ,p_pk_size IN INTEGER
     ,p_indent IN INTEGER := 0
     ,p_columns_per_line IN INTEGER := 1
   )
   RETURN VARCHAR2
   IS
   BEGIN
      RETURN build_sql_statement_part('U',p_table_name,p_table_alias,'U',p_sel_columns,0,'N',p_columns_per_line,NULL,p_data,p_pk_size,p_indent);
   END;
--
--#begin public
/**
...
--#end public
   ...
   BEGIN
      INSERT INTO ds_tab_columns (
       ...
       , gen_type, gen_params, null_value_pct
      ...
      SELECT tab.table_id table_id, tab.table_name tab_name, tcol.column_name col_name, tcol.column_id col_seq
           , CASE WHEN ccol.constraint_name IS NOT NULL THEN 'FK' ELSE NVL(msk.gen_type,p_gen_type) END gen_type
           , CASE WHEN ccol.constraint_name IS NOT NULL THEN ccol.constraint_name ELSE msk.gen_params END params
...
--                AND ds_con.extract_type IN ('B','P')
        ...
        LEFT OUTER JOIN ds_masks msk
         ...
         AND msk.gen_type IS NOT NULL
         AND msk.gen_params IS NOT NULL
...
*/
   PROCEDURE update_table_column_properties (
    ...
    , p_gen_params IN ds_tab_columns.gen_params%TYPE := '~'
   ...
   BEGIN
      UPDATE ds_tab_columns
         SET col_seq = CASE WHEN p_col_seq = -1 THEN col_seq ELSE p_col_seq END
           ...
           , gen_params = CASE WHEN p_gen_params = '~' THEN gen_params ELSE p_gen_params END
   ...
   BEGIN
      FOR r_tab IN c_tab LOOP
         UPDATE ds_tab_columns col_tgt
            SET (col_seq, gen_type, gen_params, null_value_pct, null_value_condition) = (
                   SELECT col_seq, gen_type, gen_params, null_value_pct, null_value_condition
...
--#begin public
   ...
   PROCEDURE insert_mask (
    ...
    , p_msk_params ds_masks.msk_params%TYPE := NULL
    ...
    , p_gen_type ds_masks.gen_type%TYPE := NULL
    , p_gen_params ds_masks.gen_params%TYPE := NULL
    , p_tde_type ds_masks.tde_type%TYPE := NULL
    , p_tde_params ds_masks.tde_params%TYPE := NULL
...
--#end public
   ...
   BEGIN
      ...
      SELECT ds_msk_seq.nextval, col.table_name, col.column_name
           ...
           , p_pat_cat, p_pat_name, p_msk_params
           , p_options, p_remarks, p_values_sample
           , p_gen_type, p_gen_params
           , p_tde_type, p_tde_params
        FROM sys.all_tab_columns col
        LEFT OUTER JOIN ds_masks msk
...
--#begin public
   ...
   PROCEDURE update_mask_properties (
    ...
    , p_msk_params ds_masks.msk_params%TYPE := '~'
    , p_options ds_masks.options%TYPE := '~'
    , p_remarks ds_masks.remarks%TYPE := '~'
    , p_gen_type ds_masks.gen_type%TYPE := '~'
    , p_gen_params ds_masks.gen_params%TYPE := '~'
    , p_tde_type ds_masks.tde_type%TYPE := '~'
    , p_tde_params ds_masks.tde_params%TYPE := '~'
    , p_raise_error_when_no_update BOOLEAN := TRUE
   )
--#end public
   IS
   BEGIN
      UPDATE ds_masks
         SET sensitive_flag = CASE WHEN p_sensitive_flag = '~' THEN sensitive_flag ELSE p_sensitive_flag END
           ...
           , gen_type = CASE WHEN p_gen_type = '~' THEN gen_type ELSE p_gen_type END
           , gen_params = CASE WHEN p_gen_params = '~' THEN gen_params ELSE p_gen_params END
           , tde_type = CASE WHEN p_tde_type = '~' THEN tde_type ELSE p_tde_type END
           , tde_params = CASE WHEN p_tde_params = '~' THEN tde_params ELSE p_tde_params END
...
--#end public
   IS
      l_rec_id ds_records.rec_id%TYPE;
      CURSOR c_rec IS
         SELECT ds_rec_seq.nextval
           FROM dual
      ;
   BEGIN
      OPEN c_rec;
      FETCH c_rec INTO l_rec_id;
      CLOSE c_rec;
      RETURN l_rec_id;
   END;
--#begin public
...
'
INSERT INTO ds_records (
   table_id, record_rowid, pass_count, seq, distance
)';
...
SELECT *
  FROM (';
         ...
         l_sql := l_sql ||
'
SELECT '||r_tab.table_id||' table_id, rowid record_rowid, 1 pass_count, 0 seq, 0 distance
       ...
       , p_pass_mult IN INTEGER
         ...
         SELECT *
          ...
          WHERE table_id = p_table_id
            AND NVL(deleted_flag,'N') = 'N'
            FOR UPDATE OF pass_count
          ORDER BY record_rowid, pass_count * p_pass_mult /* +1 for ASC, -1 for DESC*/, distance, rec_id
      ;
      r_rec_prv ds_records%ROWTYPE := NULL;
      l_count INTEGER := 0;
   BEGIN
      FOR r_rec IN c_rec(p_table_id, p_pass_mult) LOOP
         -- Keep record with highest sequence if constraint is immediate, lowest if deferred
   ...
   IS
      ...
      IS
         SELECT ds_con.*, ds_tab.source_schema, ds_tab.source_db_link
              , CASE ds_con.cardinality WHEN '1-N' THEN '>-' WHEN 'N-1' THEN '-<' ELSE '--' END arrow
           FROM ds_constraints ds_con
          INNER JOIN ds_tables ds_tab
             ON ds_tab.set_id = ds_con.set_id
...
'UPDATE ds_records ds_rec
   SET remarks = (';
         FOR r_col IN c_col(r_con.src_table_name) LOOP
            l_sql := l_sql || CHR(10)
                  || CASE WHEN r_col.position = 1 THEN '          SELECT ds_rec.remarks || '' '||r_con.arrow||' ' ELSE '              || '',' END
                  || r_col.column_name || '=''||' ||r_col.column_name;
         END LOOP;
         l_sql := l_sql ||
'
...
*/
   ---
   -- Propagate masking of PKs to FKs
   ...
   IS
      ...
      -- Covers only SHUFFLE and SQL (with column dependencies only) masking.
      CURSOR c_fk (
         p_set_id IN ds_data_sets.set_id%TYPE
      ...
      IS
         SELECT ds_pk_con.constraint_name pk_name, LOWER(ds_pk_col.table_name) pk_tab_name, LOWER(ds_pk_col.column_name) pk_col_name
           ...
           LEFT OUTER JOIN ds_masks ds_msk
            ...
            AND (ds_msk.msk_params IS NOT NULL OR ds_msk.msk_type = 'SHUFFLE')
      ...
      WHILE l_fk_col_name IS NOT NULL LOOP
         ...
         UPDATE ds_masks
            SET msk_type = 'INHERIT'
              , msk_params = r_fk2.pk_tab_name||'.'||r_fk2.pk_col_name
              , pat_cat = NULL
              , pat_name = NULL
              , values_sample = NULL
              , sensitive_flag = 'Y'
         ...
         ;
         IF SQL%ROWCOUNT = 0 THEN
            insert_mask(p_table_name=>r_fk2.fk_tab_name, p_column_name=>r_fk2.fk_col_name, p_msk_type=>'INHERIT', p_msk_params=>r_fk2.pk_tab_name||'.'||r_fk2.pk_col_name
         ...
         l_fk_col_name := ds_utility_var.g_fk_tab.NEXT(l_fk_col_name);
      END LOOP fk3_loop;
--      dbms_output.put_line('init_pk_masking_propagation(): pk_tab.count='||ds_utility_var.g_pk_tab.COUNT);
--      dbms_output.put_line('init_pk_masking_propagation(): fk_tab.count='||ds_utility_var.g_fk_tab.COUNT);
      load_masks;
   END;
/*
   ---
...
--#end public
   IS
      CURSOR c_msk IS
         SELECT table_name, column_name, msk_params, dependent_flag
           FROM ds_masks
          WHERE msk_type = 'SQL'
            AND NVL(deleted_flag,'N') = 'N'
            AND NVL(disabled_flag,'N') = 'N'
   ...
   BEGIN
      FOR r_msk IN c_msk LOOP
         ...
         END IF;
         l_dependent_flag := CASE WHEN is_dependent(r_msk.msk_params, r_msk.column_name, l_columns) THEN 'Y' ELSE NULL END;
      ...
      IS
         SELECT ds_tab.table_id, ds_tab.set_id
              , CASE WHEN ds_msk.msk_type = 'SEQUENCE' THEN 
                          CASE WHEN NVL(INSTR(ds_msk.msk_params,'@'),0)<=0 AND all_seq.sequence_name IS NULL
                               THEN LOWER(ds_tab.table_alias||'_'||ds_msk.column_name)
                               ELSE ds_msk.msk_params
                           END
                     ELSE NULL
                 END sequence_name
              , CASE WHEN ds_msk.msk_type = 'SEQUENCE' AND NVL(INSTR(ds_msk.msk_params,'@'),0)<=0 AND all_seq.sequence_name IS NULL THEN 'Y' ELSE '' END in_mem_seq_flag
              , CASE WHEN ds_msk.msk_type = 'SEQUENCE'
                     THEN CASE WHEN REGEXP_REPLACE(ds_msk.msk_params, '.*START WITH ([1-9][0-9]*).*', '\1') != ds_msk.msk_params -- replace succeeded
                               THEN TO_NUMBER(REGEXP_REPLACE(ds_msk.msk_params, '.*START WITH ([1-9][0-9]*).*', '\1'))
                               ELSE 1
                           END
                     ELSE NULL
                 END in_mem_seq_start_number
              , CASE WHEN ds_msk.msk_type = 'SEQUENCE' 
                     THEN CASE WHEN REGEXP_REPLACE(ds_msk.msk_params, '.*INCREMENT BY ([1-9][0-9]*).*', '\1') != ds_msk.msk_params -- replace succeeded
                               THEN TO_NUMBER(REGEXP_REPLACE(ds_msk.msk_params, '.*INCREMENT BY ([1-9][0-9]*).*', '\1'))
                               ELSE 1
                           END
                     ELSE NULL
          ...
          INNER JOIN all_tab_columns all_col
           ...
           LEFT OUTER JOIN sys.all_sequences all_seq
            ...
            AND all_seq.sequence_name = UPPER(ds_msk.msk_params)
          WHERE ds_tab.set_id = p_set_id
      ;
...
--                     print_seq('*** MAP ***',r_seq);
      ...
      IF ds_utility_var.g_mask_data THEN
         ...
         WHILE l_col_name IS NOT NULL LOOP
            ...
            UPDATE ds_masks
               SET msk_type = 'INHERIT'
                 , msk_params = NVL(r_map.sequence_name,r_map.pk_tab_name||'.'||r_map.pk_col_name)
                 , pat_cat = NULL
                 , pat_name = NULL
                 , values_sample = NULL
                 , sensitive_flag = 'Y'
            ...
            IF SQL%ROWCOUNT = 0 THEN
               insert_mask(p_table_name=>r_map.table_name, p_column_name=>r_map.column_name, p_msk_type=>'INHERIT'
                 ,p_msk_params=>r_map.sequence_name, p_remarks=>l_remarks, p_deleted_flag=>NULL
               );
            END IF;
      ...
      CURSOR c_col IS
         SELECT ds_msk.*, ds_tab.table_id, ds_tab.extract_type
          ...
          WHERE ds_msk.msk_type = 'TOKENIZE'
            AND ds_msk.msk_params IS NOT NULL -- SQL expression needed
            AND NVL(ds_msk.disabled_flag,'N') = 'N'
            AND NVL(ds_msk.deleted_flag,'N') = 'N'
...
--         dbms_output.put_line('value='||r_src.value||', token='||r_src.token||': '||CASE WHEN l_ok THEN 'ok' ELSE 'ko' END);
   ...
   END IF;
END;#','$msk_params$',replace_i(r_col.msk_params,'SEED','rownum')),'$table_name$',r_col.table_name),'$column_name$',r_col.column_name)
...
--      t_fk_cols ds_utility_var.column_name_table;
      ...
      CURSOR c_col IS
         SELECT LOWER(ds_tab.table_name) tab_name
              ...
              , ds_col.gen_params
              , CASE WHEN ds_col.gen_params = r_con.constraint_name AND r_con.cardinality = '1-N' THEN 'DFK' /*Driving FK*/ ELSE ds_col.gen_type END gen_type
          ...
          INNER JOIN sys.all_tab_columns col
           ...
           LEFT OUTER JOIN ds_constraints ds_con
            ...
            AND ds_con.constraint_name = ds_col.gen_params
            AND ds_con.cardinality = 'N-1'
           LEFT OUTER JOIN sys.all_constraints con
             ON con.owner = col.owner
            AND con.constraint_name = ds_col.gen_params
...
--   r_'||r_fk.constraint_name_lc||' '||r_fk.r_table_name||'%ROWTYPE;';
   ...
   BEGIN
      ...
      FOR i IN 1..t_rowid.COUNT LOOP
         l_count := l_count + 1;
         r_rec.rec_id := ds_utility_krn.gen_rec_id;
      ...
      ELSE
         ...
         FOR i IN 1..t_col.COUNT LOOP
            ...
            IF r_col.gen_type = 'SEQ' THEN
               r_col.col_val := LOWER(r_col.gen_params) || '.nextval';
            ...
            ELSIF r_col.gen_type = 'SQL' THEN
               r_col.col_val := replace_kw(r_col.gen_params);
               r_col.is_final := 'N';
...
--#end public
   IS
      ...
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'generate_data_set';
      l_params ds_runs.params%TYPE;
   BEGIN
      IF p_log_run THEN
         -- Start run
         l_params := 'exec ds_utility_krn.'||k_routine_name||'(';
         l_params := l_params || 'p_set_id=>'||CASE WHEN p_set_id IS NULL THEN 'NULL' ELSE TO_CHAR(p_set_id) END;
         l_params := l_params || ', p_middle_commit=>'||CASE WHEN p_middle_commit IS NULL THEN 'NULL' WHEN p_middle_commit THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_final_commit=>'||CASE WHEN p_final_commit IS NULL THEN 'NULL' WHEN p_final_commit THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ')';
         start_run(p_routine_name=>k_routine_name, p_set_id=>p_set_id, p_params=>l_params);
      END IF;
      <<set_loop>>
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         l_sql := 'CREATE OR REPLACE PROCEDURE '||USER||'.'||l_proc_name||' IS';
         l_sql := l_sql || CHR(10) || 'BEGIN';
         l_sql := l_sql || REPLACE(q'#
   -- Delete records and rowids generated by any previous run
   ds_utility_krn.transport_data_set(p_set_id=>$set_id$,p_method=>'DIRECT-EXECUTE',p_mode=>'D',p_mask_data=>FALSE,p_log_run=>FALSE);
   ...
   UPDATE ds_constraints
    ...
    ;';
         ...
         IF NVL(INSTR(ds_utility_var.g_msg_mask,'S'),0)>0 THEN
            output_clob(l_sql);
         END IF;
         EXECUTE IMMEDIATE l_sql;
         show_message('D','Procedure '||l_proc_name||' created');
         execute_immediate('BEGIN '||USER||'.'||l_proc_name||'; END;');
         ...
         IF NOT ds_utility_var.g_test_mode AND NOT NVL(INSTR(ds_utility_var.g_msg_mask,'D'),0)>0 THEN
            execute_immediate('DROP PROCEDURE '||USER||'.'||l_proc_name);
      ...
      END IF;
      IF p_log_run THEN
         stop_run(p_routine_name=>k_routine_name);
      END IF;
   EXCEPTION
      WHEN OTHERS THEN
         IF p_log_run THEN
            stop_run(p_routine_name=>k_routine_name, p_exit_code=>SQLCODE, p_error_msg=>SQLERRM||CHR(10)||SYS.DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
         END IF;
         RAISE;
   END;
--#begin public
...
--            FOR UPDATE OF pass_count
      ...
      CURSOR c_rec (
        ...
        ,p_pass_count ds_records.pass_count%TYPE
          ...
          SELECT ROWIDTOCHAR(record_rowid) record_rowid, seq, distance, rec_id
            FROM ds_records
           WHERE table_id = p_table_id
             AND pass_count = p_pass_count
             AND NVL(deleted_flag,'N') = 'N'
      ...
      l_seq_mult INTEGER;
      l_pass_mult INTEGER;
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'extract_data_set';
      l_params ds_runs.params%TYPE;
   BEGIN
      -- Check mandatory parameters
      assert(p_set_id IS NOT NULL,'Missing mandatory parameter: p_set_id');
...
'
INSERT INTO ds_records (
   table_id, record_rowid, pass_count, con_id, seq, distance, source_rowid, src_rec_id
)';
...
SELECT '||l_distinct||r_tab_dst.table_id||','||l_dst_alias||'.rowid
      ...
      ,'||CASE WHEN r_tab_src.extract_type != 'F' THEN r_rec.seq + 1 * l_seq_mult ELSE 0 END||'
      ,'||CASE WHEN r_tab_src.extract_type != 'F' THEN r_rec.distance + 1 ELSE 0 END||'
      ,CHARTOROWID('''||r_rec.record_rowid||''')
      ,'||r_rec.rec_id||'
...
SELECT DISTINCT '||r_tab_dst.table_id||','||l_dst_alias||'.rowid
               ...
               ,'||CASE WHEN r_tab_src.extract_type != 'F' THEN 'ds_rec.distance + 1' ELSE '0' END||'
               ,ds_rec.record_rowid
               ,ds_rec.rec_id
...
--#begin public
   ---
   -- Propagate encryption of PKs to FKs
   ---
   PROCEDURE propagate_encryption (
      p_commit IN BOOLEAN := FALSE
   )
--#end public
   IS
      -- Cursor to recursively find fk columns that derive from encrypted pk columns
      CURSOR c_msk IS
         SELECT msk.sensitive_flag, msk.tde_type, msk.tde_params, msk.remarks, col.*
           FROM (
            SELECT
                LEVEL AS hierarchy_level
              , CONNECT_BY_ROOT pk.owner AS root_owner
              , CONNECT_BY_ROOT pk.table_name AS root_table_name
              , CONNECT_BY_ROOT pk.column_name AS root_column_name
             ...
             INNER JOIN all_constraints pk_cons 
                ON pk.constraint_name = pk_cons.constraint_name
               AND pk.owner = pk_cons.owner
             INNER JOIN all_constraints fk_cons 
                ON pk_cons.constraint_name = fk_cons.r_constraint_name
                AND pk_cons.owner = fk_cons.r_owner
             INNER JOIN all_cons_columns fk
                ON fk.constraint_name = fk_cons.constraint_name
               AND fk.owner = fk_cons.owner
               AND fk.position = pk.position -- Ensure correct column mapping in composite keys
             WHERE pk_cons.constraint_type IN ('P', 'U') -- Primary keys and unique keys
               AND fk_cons.constraint_type = 'R' -- Foreign keys
             START WITH (pk.owner, pk.table_name, pk.column_name) IN (
                      SELECT NVL(ds_utility_var.g_owner,USER), table_name, column_name
                        FROM ds_masks
                       WHERE tde_type = 'SQL'
                         AND tde_params IS NOT NULL
                )
           CONNECT BY PRIOR fk.owner = pk.owner -- Include owner in hierarchy traversal
               AND PRIOR fk.table_name = pk.table_name 
               AND PRIOR fk.column_name = pk.column_name
               AND PRIOR fk.position = pk.position -- Maintain column order in composite keys
             ORDER BY hierarchy_level, fk_owner, fk_table_name, fk.position
             ) col
          INNER JOIN ds_masks msk
            ON msk.table_name = col.root_table_name
           AND msk.column_name = col.root_column_name
         ;
   BEGIN
      UPDATE ds_masks SET tde_type = NULL, tde_params=NULL WHERE tde_type = 'INHERIT';
      FOR r_msk IN c_msk LOOP
         r_msk.tde_type := 'INHERIT';
         r_msk.tde_params := REGEXP_REPLACE(r_msk.tde_params,'([^A-Za-z0-9_]|^)('||r_msk.root_column_name||')([^A-Za-z0-9_]|$)','\1'||LOWER(r_msk.fk_column_name)||'\3',1,0,'i');
         r_msk.remarks := 'Inherited encryption from '||LOWER(r_msk.root_table_name)||'.'||LOWER(r_msk.root_column_name);
         UPDATE ds_masks
            SET tde_type = r_msk.tde_type
              , tde_params = r_msk.tde_params
              , remarks = CASE WHEN NVL(INSTR(remarks,'encryption'),0)>0 THEN remarks ELSE RTRIM(r_msk.remarks || ' / ' || remarks) END
          WHERE table_name = r_msk.fk_table_name
            AND column_name = r_msk.fk_column_name
         ;
         IF SQL%NOTFOUND THEN
            insert_mask(p_table_name=>r_msk.fk_table_name, p_column_name=>r_msk.fk_column_name, p_tde_type=>r_msk.tde_type, p_tde_params=>r_msk.tde_params
              ,p_sensitive_flag=>r_msk.sensitive_flag, p_remarks=>r_msk.remarks, p_deleted_flag=>NULL
            );
         END IF;
      END LOOP;
      IF p_commit THEN
...
--#end public
   ...
   BEGIN
      ...
      UPDATE ds_masks SET msk_type = NULL, msk_params=NULL WHERE msk_type = 'INHERIT';
...
--#begin public
/**
* Transport a data set (copy/delete via direct execution or prepare/execution script)
...
*/
   PROCEDURE transport_data_set (
    ...
    , p_log_run IN BOOLEAN := TRUE -- log run?
    , p_rows_per_statement IN PLS_INTEGER := 50 -- nber of rows per statement
    , p_remote_delete IN BOOLEAN := FALSE -- local delete by default
    , p_ignore_existing IN BOOLEAN := FALSE -- insert ignore?
   )
--#end public
   IS
      -- Cursor to browse data sets
      ...
      l_refresh_mode BOOLEAN; -- update first then insert if not found?
      l_delete_mode BOOLEAN;
      l_move_mode BOOLEAN; -- insert than delete?
      l_merge_mode BOOLEAN; -- use MERGE INTO syntax?
      l_ongoing_merge BOOLEAN := FALSE;
      l_bulk_mode BOOLEAN; -- process multiple rows per statement?
      l_pass_count INTEGER;
      l_order INTEGER;
      l_src_table_name VARCHAR2(100);
      ...
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'transport_data_set';
      l_params ds_runs.params%TYPE;
      l_header BOOLEAN;
      -- Add subquery to filter selected record based on rowids
      ...
      BEGIN
         ...
         RETURN CHR(10)
            ...
            ||l_spaces||'       )';
      END;
      -- Generate WHEN MATCHED and WHEN NOT MATCHED clauses if a MERGE INTO is ongoing
      PROCEDURE finalize_merge (
         p_semi_colon BOOLEAN := TRUE
      )
      IS
         l_cols ds_utility_var.column_name_table;
      BEGIN
         IF NOT l_ongoing_merge THEN
            RETURN;
         END IF;
         l_sql := l_sql || ') q ON (';
         l_cols := tokenize_columns_list(l_pk_columns);
         FOR i IN 1..l_cols.COUNT LOOP
            l_sql := l_sql || CASE WHEN i>1 THEN ' AND ' END|| 't.' || l_cols(i) || '=q.' ||l_cols(i);
         END LOOP;
         l_sql := l_sql || ')' || CHR(10);
         IF l_insert_mode THEN
            l_sql := l_sql ||  'WHEN NOT MATCHED THEN INSERT (';
            l_cols := tokenize_columns_list(l_ins_columns);
            FOR i IN 1..l_cols.COUNT LOOP
               l_sql := l_sql ||CASE WHEN i>1 THEN ', ' END || 't.' || l_cols(i);
            END LOOP;
            l_sql := l_sql || ') VALUES (';
            FOR i IN 1..l_cols.COUNT LOOP
               l_sql := l_sql ||CASE WHEN i>1 THEN ', ' END || 'q.' || l_cols(i);
            END LOOP;
            l_sql := l_sql || ')';
            l_sql := l_sql || CHR(10);
         END IF;
         IF l_update_mode THEN
            l_sql := l_sql || 'WHEN MATCHED THEN UPDATE ';
            assert(l_cols.COUNT>=1,'Table must have at least one non-PK column for merge update');
            l_cols := tokenize_columns_list(l_upd_columns);
            FOR i IN 1..l_cols.COUNT LOOP
               l_sql := l_sql || CASE i WHEN 1 THEN 'SET ' ELSE ', ' END || 't.' || l_cols(i) || '=q.' ||l_cols(i);
            END LOOP;
            l_sql := l_sql || CHR(10);
         END IF;
/* merge delete not used/needed
         IF l_delete_mode THEN
--          l_sql := l_sql || 'WHEN MATCHED THEN DELETE'; --works only as of release 19.11 (an update is required before!)
            l_cols := tokenize_columns_list(l_upd_columns);
            assert(l_cols.COUNT>=1,'Table must have at least one non-PK column for merge update');
            l_sql := l_sql || 'WHEN MATCHED THEN UPDATE SET ' || l_cols(1) || '=q.' ||l_cols(1) || CHR(10); -- mandatory before 19.11!
            l_sql := l_sql || 'DELETE WHERE 1=1';
            l_sql := l_sql || CHR(10);
         END IF;
*/
         IF p_semi_colon THEN
            l_sql := l_sql || ';';
         END IF;
         l_ongoing_merge := FALSE;
      END;
      -- Finalise and execute a SQL statement
      PROCEDURE execute_script IS
      BEGIN
         IF l_sql IS NULL THEN
            RETURN;
         END IF;
         IF l_merge_mode THEN
            finalize_merge;
         ELSIF l_bulk_mode THEN
            l_sql := l_sql ||';'; -- terminate last statement
         END IF;
         execute_immediate(p_sql=>'BEGIN ds_utility_krn.execute_immediate'||CASE WHEN p_db_link IS NOT NULL THEN '@'||LOWER(p_db_link) END||'(:1); END;',p_using=>'BEGIN'||CHR(10)||l_sql||CHR(10)||'END;');
         l_sql := NULL;
      END;
   BEGIN
      ...
      assert(NOT (p_mode = 'M' AND p_remote_delete), 'Move (remote insert + local delete) is not compatible with remote delete!');
      IF p_log_run THEN
         -- Start run
         l_params := 'exec ds_utility_krn.'||k_routine_name||'(';
         l_params := l_params || 'p_set_id=>'||CASE WHEN p_set_id IS NULL THEN 'NULL' ELSE TO_CHAR(p_set_id) END;
         l_params := l_params || ', p_method=>'||CASE WHEN p_method IS NULL THEN 'NULL' ELSE ''''||p_method||'''' END;
         l_params := l_params || ', p_mode=>'||CASE WHEN p_mode IS NULL THEN 'NULL' ELSE ''''||p_mode||'''' END;
         l_params := l_params || ', p_db_link=>'||CASE WHEN p_db_link IS NULL THEN 'NULL' ELSE ''''||p_db_link||'''' END;
         l_params := l_params || ', p_output=>'||CASE WHEN p_output IS NULL THEN 'NULL' ELSE ''''||p_output||'''' END;
         l_params := l_params || ', p_middle_commit=>'||CASE WHEN p_middle_commit IS NULL THEN 'NULL' WHEN p_middle_commit THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_final_commit=>'||CASE WHEN p_final_commit IS NULL THEN 'NULL' WHEN p_final_commit THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_mask_data=>'||CASE WHEN p_mask_data IS NULL THEN 'NULL' WHEN p_mask_data THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ')';
         start_run(p_routine_name=>k_routine_name, p_set_id=>p_set_id, p_params=>l_params);
      END IF;
      ...
      FOR r_set IN c_set(p_set_id) LOOP
         ...
         FOR r_tab IN c_tab(r_set.set_id,l_order) LOOP
            ...
            THEN
               ...
               l_bulk_mode := p_rows_per_statement > 1;
               l_merge_mode := l_bulk_mode AND (l_refresh_mode OR l_update_mode OR (l_insert_mode AND p_ignore_existing));
               l_ins_columns := normalise_columns_list(r_tab.table_name,NVL(r_tab.columns_list,'*'));
               l_pk_size := 0;
               l_sel_columns := l_ins_columns;
               assert(NOT l_insert_mode OR l_ins_columns IS NOT NULL,'List of columns to insert is empty for table '||r_tab.table_name);
               IF l_update_mode OR l_delete_mode OR l_merge_mode THEN
               ...
               END IF;
               IF l_update_mode OR (l_delete_mode AND l_merge_mode) THEN
               ...
               FOR l_pass_index IN REVERSE 1..NVL(r_tab.group_count,1) LOOP
                  ...
                  IF p_method = 'DIRECT-EXECUTE' THEN
                     IF l_update_mode AND NOT l_merge_mode THEN
                        ...
                        l_row_count := execute_immediate(l_sql);
                     END IF;
                     IF l_insert_mode AND NOT l_merge_mode THEN
                        l_sql :=
                        ...
                        /* In refresh mode, insert only missing records */
--                        IF l_refresh_mode THEN
--                           l_sql := l_sql ||
...
--                        END IF;
                        ...
                        l_row_count := execute_immediate(l_sql);
                     END IF;
                     IF l_delete_mode AND NOT l_merge_mode THEN
                        l_sql :=
                           'DELETE '||CASE WHEN p_remote_delete THEN l_tgt_table_name ELSE LOWER(r_tab.table_name) END||CHR(10);
                        IF p_remote_delete THEN
                           l_sql := l_sql || 
                              ' WHERE (' || l_pk_columns || ') IN ('||CHR(10)
                           || '    SELECT ' || l_pk_columns||CHR(10) 
                           || '      FROM ' || LOWER(r_tab.table_name)||CHR(10)
                           || '     WHERE 1=1'
                           || rowid_subquery(r_tab.extract_type,r_tab.table_id,l_pass_count,4)||CHR(10)
                           || ')';
                        ELSE
                           l_sql := l_sql ||' WHERE 1=1'||rowid_subquery(r_tab.extract_type,r_tab.table_id,l_pass_count,0)||CHR(10);
                        END IF;
                        l_row_count := execute_immediate(l_sql);
                     END IF;
                     IF l_merge_mode THEN
                        l_sql := 'MERGE INTO '||l_tgt_table_name||' t USING ('||CHR(10)
                         ||build_join_statement (
                              p_purpose => 'DIRECT'
                            , p_op => 'I'
                            , p_extract_type => r_tab.extract_type
                  ...
                  ELSIF p_method LIKE '%SCRIPT%' THEN
                     ...
                     LOOP
                        ...
                        IF l_delete_mode THEN
                           IF l_bulk_mode THEN
                              l_header := MOD(l_row_count-1, p_rows_per_statement)=0;
                              IF l_header THEN
                                 l_sql := l_sql
                                        ||CASE WHEN l_row_count > 1 AND p_method = 'PREPARE-SCRIPT' THEN ';'||CHR(10) END --terminate previous statement
                                        ||'DELETE '||LOWER(r_tab.table_name)||CHR(10);
                              END IF;
                              l_sql := l_sql
                                 ||CASE WHEN l_header THEN ' WHERE ' ELSE '    OR ' END
                                 ||'('
                                 ||REPLACE(REPLACE(REPLACE(build_set_and_where_clauses(l_tgt_table_name,r_tab.table_alias,l_sel_columns,l_result,l_pk_size,0,0),' WHERE '),'   AND ',' AND '),CHR(10))
                                 ||')';
                           ELSE
                              l_sql := l_sql ||
                        ...
                        END IF;
                        IF l_insert_mode OR l_merge_mode THEN
                           IF l_bulk_mode OR l_merge_mode THEN
                              l_header := MOD(l_row_count-1, p_rows_per_statement)=0;
                              IF l_header THEN
                                 IF l_merge_mode THEN
                                    finalize_merge;
                                    l_sql := l_sql || CHR(10) ||'MERGE INTO '||l_tgt_table_name||' t USING ('||CHR(10);
                                    l_ongoing_merge := TRUE;
                                 ELSIF l_insert_mode THEN
                                    l_sql := l_sql
                                       ||CASE WHEN l_row_count > 1 AND p_method = 'PREPARE-SCRIPT' THEN ';'||CHR(10) END -- terminate previous statement
                                       ||l_ws||l_ws||'INSERT INTO '||l_tgt_table_name||' ('||l_ins_columns||')'||CHR(10);
                                 END IF;
                              END IF;
                              l_sql := l_sql ||
                                 l_ws||l_ws
                               ||CASE WHEN NOT l_header THEN 'UNION ALL ' END
                               ||'SELECT '
                               ||build_values_clause(r_tab.table_name,r_tab.table_alias,l_sel_columns,l_result,0/*l_pk_size*/,0,'N',0,NULL,l_header)
                               ||' FROM dual';
                           ELSE
                              l_sql := l_sql ||
                        ...
                        END IF;
                        IF l_refresh_mode THEN
                           l_sql := l_sql || l_ws || 'END IF;' || CHR(10) || 'END;' || CHR(10);
                        END IF;
                        IF p_method = 'EXECUTE-SCRIPT' THEN
                           IF MOD(l_row_count, p_rows_per_statement)=0 -- rows per statement reached
                           THEN
                              execute_script;
                           ELSE
                              l_sql := l_sql || CHR(10);
                     ...
                     IF (l_bulk_mode OR l_merge_mode) AND l_row_count > 0 THEN
                        IF p_method = 'EXECUTE-SCRIPT' THEN
                           execute_script;
                        ELSE
                           IF l_merge_mode THEN
...
--#end public
   ...
   BEGIN
      transport_data_set (
        ...
        ,p_log_run=>p_log_run
        ,p_rows_per_statement=>p_rows_per_statement
        ,p_ignore_existing=>p_ignore_existing
      );
   END;
--#begin public
/**
...
--#end public
   ...
   BEGIN
      ...
      -- Initialize the policy context session.
      DBMS_SESSION.SET_IDENTIFIER(ds_utility_var.g_policy_ctx_session);
...
'EXISTS (
    ...
    OR ';
      ...
      || '      ' || l_sql || ' '
      || '   ) '
      || '   OR USER != COALESCE(SYS_CONTEXT(''' || ds_utility_var.g_policy_ctx_name || ''', ''FILTER_ACTIVE''), ''X'') '
      || '   OR ( '
      || '         COALESCE(SYS_CONTEXT(''' || ds_utility_var.g_policy_ctx_name || ''', ''FILTER_DATA_PUMP''), ''N'') = ''Y'' '
      || '         AND COALESCE(LOWER(SYS_CONTEXT(''USERENV'', ''MODULE'')), ''X'') NOT IN (''data pump worker'', ''data pump master'') '
      || '   ) '
      || ') ';
...
--#end public
   ...
   END;
   ---
   -- Return decrypt columns
   ---
   FUNCTION build_decrypted_select (
      p_owner IN VARCHAR2
    , p_table_name IN VARCHAR2
    , p_include_rowid IN BOOLEAN
   ) 
   RETURN VARCHAR2
   IS
      -- Cursor to browse all table columns
      CURSOR c_col (
         p_owner sys.all_tables.owner%TYPE
       , p_table_name sys.all_tables.table_name%TYPE
       , p_include_rowid VARCHAR2
      )
      IS
         SELECT usr.column_id
              , LOWER(usr.column_name) column_name, LOWER(usr.column_name) column_alias
              , usr.data_type, usr.data_length
              , usr.data_precision, usr.data_scale, usr.char_length, usr.char_used
              , msk.tde_type, msk.tde_params
           FROM sys.all_tab_columns usr
           LEFT OUTER JOIN ds_masks msk
             ON msk.table_name = usr.table_name
            AND msk.column_name = usr.column_name
          WHERE usr.owner = UPPER(p_owner)
            AND usr.table_name = UPPER(p_table_name)
          UNION ALL
         SELECT 0
              , 'rowid', 'rowid#'
              , 'ROWID', NULL
              ...
              , NULL, NULL
           FROM dual
          WHERE p_include_rowid = 'Y'
          ORDER BY 1
      ;
      l_sql VARCHAR2(4000);
   BEGIN
      FOR r_col IN c_col(p_owner,p_table_name,CASE WHEN p_include_rowid THEN 'Y' ELSE 'N' END) LOOP
         l_sql := l_sql
         || CASE WHEN l_sql IS NULL THEN 'SELECT ' ELSE ', ' END 
         || CASE WHEN r_col.tde_params IS NOT NULL THEN 'CAST(' || REPLACE(r_col.tde_params,'encrypt','decrypt')
         || ' AS ' || CASE WHEN r_col.data_type LIKE '%CHAR%' THEN r_col.data_type || '(' || r_col.char_length || CASE r_col.char_used WHEN 'C' THEN ' CHAR' WHEN 'B' THEN ' BYTE' END || ')' 
                           WHEN r_col.data_type = 'NUMBER' THEN r_col.data_type || CASE WHEN r_col.data_precision IS NOT NULL THEN '(' || r_col.data_precision|| ',' || r_col.data_scale || ')' END
                           ELSE r_col.data_type
                       END
         || ')' ELSE r_col.column_name END
         || ' ' ||r_col.column_alias
         ;
      END LOOP;
      l_sql := l_sql || ' FROM ' || LOWER(p_table_name);
      RETURN l_sql;
   END;
   ---
   ...
   PROCEDURE create_drop_objects (
     ...
     ,p_table_name IN VARCHAR2 := NULL   -- table name pattern for full schema
     ...
     ,p_db_link IN VARCHAR2 := NULL -- for RPC
      ...
      CURSOR c_tab (
         p_owner sys.all_tables.owner%TYPE
        ,p_set_id ds_data_sets.set_id%TYPE
        ,p_set_type ds_data_sets.set_type%TYPE
        ,p_table_name sys.all_tables.table_name%TYPE
      )
      IS
         SELECT usr.table_name, ds_tab.table_alias, ds_tab.target_db_link
          ...
          WHERE usr.owner = p_owner
            AND (p_set_id IS NULL OR ds_tab.set_id = p_set_id)
            AND (NVL(p_set_type,'XXX') != 'TDE' OR usr.table_name IN (SELECT DISTINCT table_name FROM ds_masks) OR p_table_name IS NOT NULL)
            AND (p_table_name IS NULL OR usr.table_name LIKE p_table_name OR REGEXP_LIKE(usr.table_name, p_table_name))
...
'||l_sql;
      ...
      END IF;
      <<set_loop>>
      IF p_set_type = 'SUB' THEN
         FOR r_set IN c_set(p_set_id) LOOP
            define_walk_through_strategy(p_set_id=>r_set.set_id);
         END LOOP set_loop;
      END IF;
      <<table_loop>>
      FOR r_tab IN c_tab(NVL(ds_utility_var.g_owner,USER),p_set_id,p_set_type,p_table_name) LOOP
         l_sql := NULL;
         -- Filter table
...
SELECT '||l_sel_columns||'
 ...
 WHERE 1=0';
               ...
               ELSIF p_object_type = 'VIEW' THEN
                  ...
                  END IF;
                  IF p_set_type = 'TDE' THEN
                     l_sql := 'CREATE OR REPLACE VIEW '||l_object_name||' AS'||CHR(10)||build_decrypted_select(NVL(ds_utility_var.g_owner,USER),r_tab.table_name,p_include_rowid);
                  ELSE
         ...
         ELSIF p_object_type = 'POLICY'
           AND r_tab.table_name NOT LIKE 'DS~_%' ESCAPE '~' -- records of DS tables cannot be hidden/encrypted!!!
          THEN
            IF p_operation = 'DROP' THEN
               l_sql := 'BEGIN
...
END;';
            ELSE
               assert(p_set_type='SUB', 'Unsupported set type for triggers: '||p_set_type);
...
--#end public
   IS
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'create_views';
      l_params ds_runs.params%TYPE;
   BEGIN
      IF p_log_run THEN
         -- Start run
         l_params := 'exec ds_utility_krn.'||k_routine_name||'(';
         l_params := l_params || 'p_view_suffix=>'||CASE WHEN p_view_suffix IS NULL THEN 'NULL' ELSE ''''||p_view_suffix||'''' END;
         l_params := l_params || ', p_view_prefix=>'||CASE WHEN p_view_prefix IS NULL THEN 'NULL' ELSE ''''||p_view_prefix||'''' END;
         l_params := l_params || ', p_table_prefix=>'||CASE WHEN p_table_prefix IS NULL THEN 'NULL' ELSE ''''||p_table_prefix||'''' END;
         l_params := l_params || ', p_full_schema=>'||CASE WHEN p_full_schema IS NULL THEN 'NULL' WHEN p_full_schema THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_table_name=>'||CASE WHEN p_table_name IS NULL THEN 'NULL' ELSE ''''||p_table_name||'''' END;
         l_params := l_params || ', p_non_empty_only=>'||CASE WHEN p_non_empty_only IS NULL THEN 'NULL' WHEN p_non_empty_only THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_mode=>'||CASE WHEN p_mode IS NULL THEN 'NULL' ELSE ''''||p_mode||'''' END;
         l_params := l_params || ', p_set_id=>'||CASE WHEN p_set_id IS NULL THEN 'NULL' ELSE TO_CHAR(p_set_id) END;
         l_params := l_params || ', p_set_type=>'||CASE WHEN p_set_type IS NULL THEN 'NULL' ELSE ''''||p_set_type||'''' END;
         l_params := l_params || ', p_mask_data=>'||CASE WHEN p_mask_data IS NULL THEN 'NULL' WHEN p_mask_data THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_include_rowid=>'||CASE WHEN p_include_rowid IS NULL THEN 'NULL' WHEN p_include_rowid THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ')';
         start_run(p_routine_name=>k_routine_name, p_set_id=>p_set_id, p_params=>l_params);
      END IF;
      create_drop_objects(
        ...
        ,p_table_name=>p_table_name,p_non_empty_only=>p_non_empty_only,p_mode=>p_mode,p_set_id=>p_set_id
...
--#end public
   ...
   BEGIN
      create_drop_objects(
        ...
        ,p_object_prefix=>p_view_prefix, p_table_prefix=>p_table_prefix, p_full_schema=>p_full_schema
        ,p_table_name=>p_table_name,p_non_empty_only=>p_non_empty_only,p_set_id=>p_set_id,p_set_type=>p_set_type
      );
   END;
--

   /**
   * Create the policies context.
   *
   * @param p_context_name: context name
   * @throws -20000: context creation failure
   */
   PROCEDURE create_policies_context(
      p_context_name IN VARCHAR2 := 'DS_POLICY_CONTEXT'
   ) IS
   ...
   BEGIN
      ...
      -- Check whether the context name is valid.
      IF TRIM(p_context_name) IS NULL THEN
         RAISE_APPLICATION_ERROR(-20000, 'Invalid policy context name.');
      END IF;
      ds_utility_var.g_policy_ctx_name := p_context_name;

      -- Create the context.
      sql_stmt := replace(
         ds_utility_var.gk_policy_ctx_creation
      , '{ctx_name}'
      , p_context_name
      );
      EXECUTE IMMEDIATE sql_stmt;

   END create_policies_context;

   /**
   * Drop the policies context.
   *
   * @param p_context_name: context_name
   */
   PROCEDURE drop_policies_context(p_context_name VARCHAR2 := NULL) IS
   ...
   BEGIN
      ...
      IF TRIM(p_context_name) IS NOT NULL THEN
         ctx_name := TRIM(p_context_name);
      ELSE
         ctx_name := ds_utility_var.g_policy_ctx_name;
      END IF;

      -- Drop the context.
      EXECUTE IMMEDIATE
         REPLACE(ds_utility_var.gk_policy_ctx_removal, '{ctx_name}', ctx_name);

   END drop_policies_context;

   /**
   * Activate the policies.
   *
   * @param p_user: user the policies must be activated for, current user by default
   * @param p_data_pump_only: whether the policies are only activated for
   * data pump sessions only
   */
   PROCEDURE activate_policies(
      p_user               VARCHAR2 := NULL
    , p_data_pump_only     BOOLEAN  := TRUE
   ) IS
   
      -- whether the policy is only activate for data pump sessions
      data_pump_only       CHAR(1)  := 'Y';
      
   BEGIN
      DBMS_SESSION.SET_CONTEXT(
         namespace         => ds_utility_var.g_policy_ctx_name
       , attribute         => 'FILTER_ACTIVE'
       , value             => COALESCE(TRIM(p_user), USER)
       , client_id         => ds_utility_var.g_policy_ctx_session
      );
      IF p_data_pump_only THEN
         data_pump_only := 'Y';
      ELSE
         data_pump_only := 'N';
      END IF;
      DBMS_SESSION.SET_CONTEXT(
         namespace         => ds_utility_var.g_policy_ctx_name
       , attribute         => 'FILTER_DATA_PUMP'
       , value             => data_pump_only
       , client_id         => ds_utility_var.g_policy_ctx_session
      );
   END activate_policies;

   /**
   * Deactivate the policies.
   */
   PROCEDURE deactivate_policies IS
   BEGIN
      DBMS_SESSION.SET_CONTEXT(
         namespace         => ds_utility_var.g_policy_ctx_name
       , attribute         => 'FILTER_ACTIVE'
...
* @param p_mode S=Static(quicker), D=Dynamic(slower)
* @param p_set_id data set id (null for all)
* @param p_set_type data set type
*/
...
--#end public
   ...
   BEGIN
      create_drop_objects(
       ...
       , p_table_name=>p_table_name, p_mode=>p_mode, p_non_empty_only=>FALSE, p_set_id=>p_set_id,p_set_type=>p_set_type
      );
   END;
--
--#begin public
...
--#end public
   ...
   BEGIN
      create_drop_objects(
       ...
       , p_object_prefix=>p_policy_prefix, p_table_prefix=>p_table_prefix, p_full_schema=>p_full_schema
       , p_table_name=>p_table_name, p_non_empty_only=>FALSE, p_set_id=>p_set_id,p_set_type=>p_set_type
      );
   END;
--
--#begin public
...
--#end public
   ...
   BEGIN
      create_drop_objects(
       ...
       , p_table_name=>p_table_name, p_non_empty_only=>p_non_empty_only, p_set_id=>p_set_id,p_object_options=>p_table_options
      );
   END;
--
--#begin public
...
--         IF NVL(INSTR(UPPER(NVL(r_set.capture_mode,'XML')),'FWD'),0)>0 THEN
   ...
   END;
   ---
   -- Create or drop triggers for TDE
   ---
   PROCEDURE create_drop_tde_triggers (
      p_operation IN VARCHAR2 -- CREATE or DROP
    , p_view_suffix IN VARCHAR2 := NULL  -- suffix to add to view name
    , p_view_prefix IN VARCHAR2 := NULL  -- prefix to add to view name
    , p_table_prefix IN VARCHAR2 := NULL -- prefix to remove from table name
   ...
   IS
      -- Cursor to browse masked tables
      CURSOR c_tab IS
         SELECT DISTINCT table_name
           FROM ds_masks
          WHERE tde_params IS NOT NULL
          ORDER BY 1
      ;
      -- Cursor to browse all table columns
      CURSOR c_col (
      ...
      IS
         SELECT LOWER(usr.column_name) column_name, usr.data_type, usr.data_length
              , usr.data_precision, usr.data_scale
              , msk.tde_params
           FROM sys.all_tab_columns usr
           LEFT OUTER JOIN ds_masks msk
             ON msk.table_name = usr.table_name
      ...
      l_view_suffix_len PLS_INTEGER;
      t_pk_cols ds_utility_var.column_name_table;
      TYPE a_tde_params_type IS TABLE OF ds_masks.tde_params%TYPE INDEX BY ds_utility_var.column_name;
      a_tde a_tde_params_type;
      TYPE a_pk_cols_type IS TABLE OF VARCHAR2(1) INDEX BY ds_utility_var.column_name;
      a_pk_cols a_pk_cols_type;
      l_tde_params ds_masks.tde_params%TYPE;
   BEGIN
      assert(p_operation IN ('CREATE','DROP'), 'Invalid operation for tde triggers: '||p_operation);
      FOR r_tab IN c_tab LOOP
         l_view_suffix := CASE WHEN p_view_suffix IS NULL AND p_view_prefix IS NULL AND p_table_prefix IS NULL THEN '_TDE' ELSE p_view_suffix END;
         l_view_suffix_len := NVL(LENGTH(l_view_suffix),0);
         l_view_name := SUBSTR(p_view_prefix||r_tab.table_name,1,30);
         IF l_view_suffix IS NOT NULL THEN
            l_view_name := SUBSTR(l_view_name,1,30-l_view_suffix_len)||l_view_suffix;
         END IF;
         l_trigger_name := SUBSTR(l_view_name,1,30-4)||'_TRG';
         IF p_operation = 'DROP' THEN
            l_sql := 'DROP TRIGGER ' || l_trigger_name;
         ELSIF p_operation = 'CREATE' THEN
            l_insert := NULL;
            l_update := NULL;
            l_delete := NULL;
            ...
            t_pk_cols := tokenize_columns_list(get_pk_columns(r_tab.table_name));
            FOR i IN 1..t_pk_cols.COUNT LOOP
               a_pk_cols(t_pk_cols(i)) := 'X';
            END LOOP;
            l_delete := 'DELETE '||LOWER(r_tab.table_name);
            FOR r_col IN c_col(USER, r_tab.table_name) LOOP
               l_tde_params := CASE WHEN r_col.tde_params IS NULL THEN ':NEW.'||r_col.column_name
                                    ELSE REGEXP_REPLACE(r_col.tde_params,'(^|[^a-zA-Z0-9_])'||r_col.column_name||'([^a-zA-Z0-9_]|$)', '\1:NEW.'||r_col.column_name||'\2',1,0,'i')
                               END;
               l_insert := l_insert || CASE WHEN l_insert IS NULL THEN 'INSERT INTO '||LOWER(r_tab.table_name)||' (' ELSE ', ' END || r_col.column_name;
               l_values := l_values || CASE WHEN l_values IS NULL THEN 'VALUES (' ELSE ', ' END || l_tde_params;
               IF NOT a_pk_cols.EXISTS(r_col.column_name) THEN -- do not update pk columns
                  l_update := l_update || CASE WHEN l_update IS NULL THEN 'UPDATE '||LOWER(r_tab.table_name)||' SET ' ELSE ', ' END || r_col.column_name || ' = ' || l_tde_params;
               END IF;
               a_tde(r_col.column_name) := REPLACE(l_tde_params,':NEW.',':OLD.'); -- save for later
            END LOOP;
            l_insert := l_insert || ')';
            l_values := l_values || ');';
            l_where := NULL;
            FOR i IN 1..t_pk_cols.COUNT LOOP
               l_where := l_where || CASE WHEN l_where IS NULL THEN 'WHERE ' ELSE ' AND ' END || t_pk_cols(i) || ' = ' || a_tde(t_pk_cols(i));
            END LOOP;
            l_where := l_where ||';';
            l_sql := 
'CREATE OR REPLACE TRIGGER '||LOWER(l_trigger_name)||'
INSTEAD OF INSERT OR UPDATE OR DELETE ON '||LOWER(l_view_name)||'
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        '||l_insert||'
...
';
         ...
         EXCEPTION
            WHEN OTHERS THEN
               show_message('E', 'Error while creating tde trigger on '||LOWER(r_tab.table_name));
...
--#end public
   IS
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'create_tde_triggers';
      l_params ds_runs.params%TYPE;
   BEGIN
      IF p_log_run THEN
         -- Start run
         ...
         l_params := l_params || ', p_view_prefix=>'||CASE WHEN p_view_prefix IS NULL THEN 'NULL' ELSE ''''||p_view_prefix||'''' END;
         l_params := l_params || ', p_table_prefix=>'||CASE WHEN p_table_prefix IS NULL THEN 'NULL' ELSE ''''||p_table_prefix||'''' END;
         l_params := l_params || ', p_ignore_errors=>'||CASE WHEN p_ignore_errors IS NULL THEN 'NULL' ELSE ''''||p_ignore_errors||'''' END;
         l_params := l_params || ')';
         start_run(p_routine_name=>k_routine_name, p_set_id=>NULL, p_params=>l_params);
      END IF;
      create_drop_tde_triggers(
         p_operation=>'CREATE', p_view_suffix=>p_view_suffix
        ,p_view_prefix=>p_view_prefix, p_table_prefix=>p_table_prefix
      );
      IF p_log_run THEN
         stop_run(p_routine_name=>k_routine_name);
      END IF;
...
--#end public
   IS
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'drop_tde_triggers';
      l_params ds_runs.params%TYPE;
   BEGIN
      IF p_log_run THEN
         -- Start run
      ...
      END IF;
      create_drop_tde_triggers(
         p_operation=>'DROP', p_view_suffix=>p_view_suffix
        ,p_view_prefix=>p_view_prefix, p_table_prefix=>p_table_prefix
      );
      IF p_log_run THEN
         stop_run(p_routine_name=>k_routine_name);
...
--#end public
   IS
      ...
      r_set ds_data_sets%ROWTYPE;
      l_columns_list ds_tables.columns_list%TYPE;
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'create_triggers';
      l_params ds_runs.params%TYPE;
   BEGIN
...
--#end public
   IS
      ...
      l_full_table_name VARCHAR2(100);
      k_routine_name CONSTANT ds_runs.routine_name%TYPE := 'discover_sensitive_data';
      l_params ds_runs.params%TYPE;
         ...
         IF c_msk%FOUND THEN
            UPDATE ds_masks
               SET sensitive_flag = 'Y'
                 ...
                 , msk_params = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r_pat.msk_params,':column_name',LOWER(r_col.column_name)),':col_char_length',r_col.char_length),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
                 , pat_cat = r_pat.pat_cat
                 , pat_name = r_pat.pat_name
                 , remarks = p_remarks
                 , values_sample = p_values_sample
                 , deleted_flag = NULL
                 , gen_type = r_pat.gen_type
                 , gen_params = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r_pat.gen_params,':column_name',LOWER(r_col.column_name)),':col_char_length',r_col.char_length),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
                 , tde_type = r_pat.tde_type
                 , tde_params = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r_pat.tde_params,':column_name',LOWER(r_col.column_name)),':col_char_length',r_col.char_length),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
             WHERE CURRENT OF c_msk
            ;
         ELSE
            INSERT INTO ds_masks (
               table_name, column_name, sensitive_flag
             , msk_type, msk_params, pat_cat
             ...
             , tde_type, tde_params
               ...
               , r_pat.msk_type
               , REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r_pat.msk_params,':column_name',LOWER(r_col.column_name)),':col_char_length',r_col.char_length),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
               , r_pat.pat_cat, r_pat.pat_name
               , p_remarks, p_values_sample
               , NULL
               , r_pat.gen_type
               , REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r_pat.gen_params,':column_name',LOWER(r_col.column_name)),':col_char_length',r_col.char_length),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
               , r_pat.tde_type
               , REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r_pat.tde_params,':column_name',LOWER(r_col.column_name)),':col_char_length',r_col.char_length),':col_data_length',r_col.data_length),':col_data_precision',r_col.data_precision),':col_data_scale',r_col.data_scale)
            );
         END IF;
         CLOSE c_msk;
      END;
   ...
   BEGIN
      IF p_log_run THEN
         ...
         l_params := 'exec ds_utility_krn.'||k_routine_name||'(';
         l_params := l_params || 'p_set_id=>'||CASE WHEN p_set_id IS NULL THEN 'NULL' ELSE TO_CHAR(p_set_id) END;
         l_params := l_params || ', p_full_schema=>'||CASE WHEN p_overwrite IS NULL THEN 'NULL' WHEN p_full_schema THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_table_name=>'||CASE WHEN p_table_name IS NULL THEN 'NULL' ELSE ''''||p_table_name||'''' END;
         l_params := l_params || ', p_rows_sample_size=>'||CASE WHEN p_rows_sample_size IS NULL THEN 'NULL' ELSE TO_CHAR(p_rows_sample_size) END;
         l_params := l_params || ', p_col_data_min_pct=>'||CASE WHEN p_col_data_min_pct IS NULL THEN 'NULL' ELSE TO_CHAR(p_col_data_min_pct) END;
         l_params := l_params || ', p_col_data_min_cnt=>'||CASE WHEN p_col_data_min_cnt IS NULL THEN 'NULL' ELSE TO_CHAR(p_col_data_min_cnt) END;
         l_params := l_params || ', p_values_sample_size=>'||CASE WHEN p_rows_sample_size IS NULL THEN 'NULL' ELSE TO_CHAR(p_values_sample_size) END;
         l_params := l_params || ', p_column_name=>'||CASE WHEN p_column_name IS NULL THEN 'NULL' ELSE ''''||p_column_name||'''' END;
         l_params := l_params || ', p_overwrite=>'||CASE WHEN p_overwrite IS NULL THEN 'NULL' WHEN p_overwrite THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ', p_commit=>'||CASE WHEN p_commit IS NULL THEN 'NULL' WHEN p_commit THEN 'TRUE' ELSE 'FALSE' END;
         l_params := l_params || ')';
         start_run(p_routine_name=>k_routine_name, p_set_id=>p_set_id, p_params=>l_params);
      END IF;
      delete_data_set_cache;
...
--                  dbms_output.put_line('Unsupported data type ('||t_desc(c).col_type||') for column '||t_desc(c).col_name);
      ...
      ;
      propagate_masking(p_set_id=>p_set_id,p_commit=>p_commit);
      propagate_encryption(p_commit=>p_commit);
      -- Commit if requested
      IF p_commit THEN
         COMMIT;
      END IF;
