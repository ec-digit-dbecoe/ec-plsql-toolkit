CREATE OR REPLACE PACKAGE ds_utility_var
AUTHID CURRENT_USER
AS
---
...
---
   ...
   gk_policy_ctx_creation     CONSTANT VARCHAR2(4000) :=
      'CREATE OR REPLACE CONTEXT {ctx_name} USING DS_UTILITY_KRN ACCESSED GLOBALLY';

   -- Context removal code.
   gk_policy_ctx_removal      CONSTANT VARCHAR2(4000) :=
      'DROP CONTEXT {ctx_name}';

   -- policy context name
   g_policy_ctx_name                   VARCHAR2(100)  :=
      'DS_POLICY_CONTEXT';

   -- policy context session name
   g_policy_ctx_session                VARCHAR2(100)  := 'DS_SESSION';
   ...
   TYPE seq_record_type IS RECORD (
    ...
    , msk_params ds_masks.msk_params%TYPE
   ...
   g_log_mask  VARCHAR2(10) := 'IWE'; -- Log filter
   g_owner sys.all_objects.owner%TYPE := USER; -- Object owner (session user by default)
   ...
   g_run_id ds_runs.run_id%TYPE;
   g_routine_name ds_runs.routine_name%TYPE;
   g_log_line INTEGER;
