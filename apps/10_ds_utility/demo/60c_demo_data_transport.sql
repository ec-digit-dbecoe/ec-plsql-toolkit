...
exec ds_utility_krn.set_source_schema('APP_DBCC_D');
exec ds_utility_krn.transport_data_set(p_set_id=>ds_utility_krn.get_data_set_def_by_name('DEMO_DATA_SUB'),p_method=>'DIRECT-EXECUTE',p_mode=>'I',p_db_link=>'DBCC_DIGIT_01_T.CC.CEC.EU.INT',p_final_commit=>TRUE);
--exec ds_utility_krn.set_test_mode(FALSE);
--exec ds_utility_krn.set_message_filter('EWI');
PAUSE End of scenario 1
select * from demo_persons@DBCC_DIGIT_01_T.CC.CEC.EU.INT order by per_id;
select * from demo_org_entities@DBCC_DIGIT_01_T.CC.CEC.EU.INT order by oen_cd;
...
--exec ds_utility_krn.set_test_mode(TRUE);
exec ds_utility_krn.set_source_schema('APP_DBCC_D');
exec ds_utility_krn.transport_data_set(p_set_id=>ds_utility_krn.get_data_set_def_by_name('DEMO_DATA_SUB'),p_method=>'EXECUTE-SCRIPT',p_output=>'DS_OUTPUT',p_mode=>'I',p_db_link=>'DBCC_DIGIT_01_T.CC.CEC.EU.INT',p_final_commit=>TRUE);
--exec ds_utility_krn.set_test_mode(FALSE);
--exec ds_utility_krn.set_message_filter('EWI');
