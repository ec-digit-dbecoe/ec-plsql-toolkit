...
--#end public
   ...
   BEGIN
      ...
      IF NOT INSTR(qc_utility_var.g_msg_mask,l_type) <= 0 AND l_type != 'T' THEN
   ...
   BEGIN
      ...
      r_msg.object_owner := qc_utility_var.g_object_owner;
   ...
   BEGIN
      ...
      r_stat.object_owner := qc_utility_var.g_object_owner;
         ...
         IF l_found THEN
            ...
            IF l_len > 0 THEN
               ...
               ELSIF SUBSTR(l_qualifier,0-l_len) = l_pk_alias THEN
                  l_qualifier := RTRIM(SUBSTR(l_qualifier,1,LENGTH(l_qualifier)-l_len),'_');
               END IF;
            END IF;
         END IF;
      END IF;
...
--#end public
   ...
   BEGIN
      ...
      IF qc_utility_var.g_cache(p_owner).t_tab_con.EXISTS(p_table_name) THEN
         ...
         WHILE l_constraint_name IS NOT NULL LOOP
            r_con := qc_utility_var.g_cache(p_owner).t_tab_con(p_table_name)(l_constraint_name);
            IF INSTR(','||UPPER(r_con.constraint_cols)||',',','||p_column_name||',')>0 THEN
               IF r_con.constraint_type = 'P' THEN
                  -- P takes precedence
                  l_constraint_type := 'P';
   ...
   BEGIN
      ...
      l_len := LENGTH(p_var)-2;
      <<var_loop>>
      FOR i IN 1..4 LOOP
         l_ret := INSTR(p_str, '{' || CASE WHEN i IN (2,4) THEN '_' END || SUBSTR(p_var,2,l_len) || CASE WHEN i IN (3,4) THEN '_' END || '}');
...
--      assert(qc_utility_var.g_app_alias IS NOT NULL,'Application alias is not defined!');
      ...
      ELSE
         ...
         END IF;
         IF INSTR(p_pattern,k_pk_column)>0 OR INSTR(p_pattern,k_pk_columns)>0 THEN -- only if keyword found in pattern
            l_pk_columns := get_tab_pk_cols(p_owner, l_table_name); -- in cache so at no cost
            IF INSTR(l_pk_columns,',') = 0 THEN
      ...
      ELSIF p_object_type = 'CONSTRAINT' OR p_object_type LIKE 'CONSTRAINT:%' THEN
         ...
         IF INSTR(l_cons_columns,',') = 0 THEN
      ...
      ELSIF p_object_type = 'TRIGGER' THEN
         ...
         IF varinstr(p_pattern, k_trg_type)>0 THEN
            IF r_trg.trigger_name IS NULL THEN
               r_trg := get_trigger(p_owner,l_object_name);
            END IF;
            IF r_trg.trigger_type IS NOT NULL AND INSTR(p_pattern, k_trg_type)>0 THEN
...
--#end public
   ...
   BEGIN
      ...
      l_pos := INSTR(l_fix_name,k_seq_nr);
      IF l_pos > 0 THEN
      ...
      END IF;
      --
      IF (LENGTH(l_fix_name) > qc_utility_var.gk_identifier_name) THEN
...
--sys.dbms_output.put_line('any_string='||l_any_string);
      ...
      IF l_any_string IS NOT NULL THEN
         ...
         l_dict_name := UPPER(SUBSTR(l_any_string, 6+l_prefix, LENGTH(l_any_string)-(6+l_prefix+l_suffix)));
...
--sys.dbms_output.put_line('looking in DICT('||l_dict_name||')');
            ...
            FOR i IN 1..qc_utility_var.g_rdict(l_dict_name).COUNT LOOP
               ...
               IF REGEXP_INSTR(l_pattern2,'{_?any ') > 0 THEN
   ...
   IS
      ...
      IS
         SELECT *
          ...
          ORDER BY 1, length(dict_value) DESC, 3
      ;
      l_count INTEGER;
   BEGIN
   ...
   BEGIN
      ...
      FOR r_tab IN c_tab LOOP
         l_entity_name := CASE WHEN p_singularify = 1
                               THEN CASE WHEN SUBSTR(l_entity_name,-3) = 'IES' THEN SUBSTR(l_entity_name,1,LENGTH(l_entity_name)-3)||'Y' -- CITIES => CITY
                                         WHEN SUBSTR(l_entity_name,-1) = 'S' THEN SUBSTR(l_entity_name,1,LENGTH(l_entity_name)-1) -- CARS => CAR
                                         ELSE l_entity_name
                                    END
   ...
   BEGIN
      ...
      ELSIF p_op = 'CREATE' THEN
         ...
         ELSIF l_object_type = 'TABLE COLUMN' THEN
            ...
            ELSIF p_fix_type = 'TABLE COLUMN' THEN
               l_sql := 'ALTER TABLE '||p_owner||'.'||p_table_name||' ADD '||SUBSTR(p_object_name,INSTR(p_object_name,'.',-1/*backward search from the end*/)+1)||' '||p_extra_name;
            ELSE
      ...
      ELSIF p_op = 'MODIFY' THEN
         IF l_object_type = 'TABLE COLUMN' THEN
            l_sql := 'ALTER TABLE '||p_owner||'.'||p_table_name||' MODIFY '||SUBSTR(p_object_name,INSTR(p_object_name,'.')+1)||' '||p_extra_name;
         ELSE
            assert(FALSE,'Unsupported data type (:1) for attempted operation (:2)', k_where, l_fix_type, p_op);
         END IF;
   ...
   IS
      ...
      CURSOR c_obj IS
         SELECT object_type||'.'||object_name name
           FROM all_objects
          WHERE owner = qc_utility_var.g_object_owner
      ...
      CURSOR c_dep IS
         SELECT referenced_type||'.'||referenced_name referencee
          ...
          WHERE owner = qc_utility_var.g_object_owner
            AND referenced_owner = qc_utility_var.g_object_owner
      ...
      WHILE l_del > 0 AND l_max > 0 LOOP
         ...
         WHILE l_idx IS NOT NULL LOOP
            ...
            IF NOT t_dep.EXISTS(l_idx) THEN
               -- List object
               l_ord := l_ord + 1;
               r_res.object_type := SUBSTR(t_obj(l_idx),1,INSTR(t_obj(l_idx),'.')-1);
               r_res.object_name := SUBSTR(t_obj(l_idx),INSTR(t_obj(l_idx),'.')+1);
      ...
      IF NOT (sys.dbms_db_version.version = 11 AND sys.dbms_db_version.version = 1) AND UPPER(p_compile_synonyms) = 'ALL' THEN
         ...
         FOR R IN (
            SELECT synonym_name
              FROM all_synonyms
             WHERE owner = 'PUBLIC'
               AND table_owner = qc_utility_var.g_object_owner
         ) LOOP
      ...
      IF UPPER(p_compile_code) = 'ALL' THEN
         ...
         FOR r IN (
            SELECT o.object_type, o.object_name, count(d.name) AS priority
             ...
             WHERE o.owner = qc_utility_var.g_object_owner
         ...
         sys.dbms_utility.compile_schema(
            schema         => qc_utility_var.g_object_owner,
            compile_all    => TRUE,
   ...
   IS
      CURSOR c_obj IS
         ...
         SELECT 10 sort_order, obj.object_type, obj.owner, obj.object_name
          ...
          WHERE obj.owner != 'SYS'
            AND obj.owner = qc_utility_var.g_object_owner
         ...
         INNER JOIN qc_patterns pat2
          ...
          WHERE con.owner = qc_utility_var.g_object_owner
...
--                     ON dict_own.dict_name = 'OBJECT OWNER'
--                    AND dict_own.dict_key = 'TABLE'
                  WHERE ind.owner = qc_utility_var.g_object_owner
         ...
         SELECT 50 sort_order, pat.object_type, vw.owner, vw.view_name object_name
          ...
          WHERE vw.owner != 'SYS' -- workaround for what seems to be an Oracle bug (SYS constraints are returned by this query!)
            AND vw.owner = qc_utility_var.g_object_owner
         ...
         SELECT 51 sort_order, pat.object_type, mvw.owner, mvw.mview_name object_name
          ...
          WHERE mvw.owner != 'SYS' -- workaround for what seems to be an Oracle bug (SYS constraints are returned by this query!)
            AND mvw.owner = qc_utility_var.g_object_owner
         ...
         SELECT 60 sort_order, obj.object_type||' COLUMN', col.owner, col.table_name||'.'||col.column_name object_name
          ...
          WHERE col.owner = qc_utility_var.g_object_owner
      ...
      CURSOR c_obj IS
         SELECT obj.object_type, obj.object_name
          ...
          INNER JOIN qc_patterns pat
             ON pat.object_type = obj.object_type
            AND pat.app_alias IN ('ALL', qc_utility_var.g_app_alias)
          WHERE obj.owner = qc_utility_var.g_object_owner
   ...
   IS
      CURSOR c_obj IS
         SELECT pat.object_type, tab.table_name object_name, INITCAP(pat.object_type) object_desc, pk.constraint_name
          ...
          WHERE tab.owner = qc_utility_var.g_object_owner
            AND SUBSTR(tab.table_name,1,4) != 'BIN$'
            AND (pat.include_pattern IS NULL OR qc_utility_krn.ext_regexp_like(pat.object_type,tab.owner,tab.table_name,pat.include_pattern)=1)
            AND (pat.exclude_pattern IS NULL OR qc_utility_krn.ext_regexp_like(pat.object_type,tab.owner,tab.table_name,pat.exclude_pattern)=0)
          ORDER BY 1, 2
...
--                       ON dict_own.dict_name = 'OBJECT OWNER'
--                      AND dict_own.dict_key = 'TABLE'
                    WHERE fk.owner = qc_utility_var.g_object_owner
...
--                       ON dict_own.dict_name = 'OBJECT OWNER'
--                      AND dict_own.dict_key = 'TABLE'
                    WHERE pk.owner = qc_utility_var.g_object_owner
                      AND pk.constraint_type IN ('P','U')
                )
         ORDER BY 1, 2
      ;
   ...
   IS
      CURSOR c_obj IS
         ...
         SELECT pat.object_type, mvw.mview_name object_name, INITCAP(pat.object_type) object_desc, com.comments
          ...
          WHERE mvw.owner = qc_utility_var.g_object_owner
            AND (pat.include_pattern IS NULL OR qc_utility_krn.ext_regexp_like(pat.object_type,mvw.owner,mvw.mview_name,pat.include_pattern)=1)
            AND (pat.exclude_pattern IS NULL OR qc_utility_krn.ext_regexp_like(pat.object_type,mvw.owner,mvw.mview_name,pat.exclude_pattern)=0)
          UNION
...
--                    AND dict_own.dict_key = 'TABLE'
         ...
         SELECT 5 sort_order, REPLACE(pat.object_type,NULL/*' ALIAS'*/) object_type, qc_utility_var.g_object_owner
              ...
              , qc_utility_krn.replace_vars(REPLACE(pat.object_type,NULL/*' ALIAS'*/),qc_utility_var.g_object_owner,dict.dict_key||'.'||dict.dict_value,CASE WHEN p_anti_pattern='Y' THEN pat.anti_pattern ELSE pat.check_pattern END) check_pattern2
              , qc_utility_krn.ext_regexp_like(REPLACE(pat.object_type,NULL/*' ALIAS'*/),qc_utility_var.g_object_owner,dict.dict_key||'.'||dict.dict_value,CASE WHEN p_anti_pattern='Y' THEN pat.anti_pattern ELSE pat.check_pattern END) check_res
              , pat.msg_type
          ...
          WHERE dict.dict_name = 'TABLE ALIAS'
            ...
            AND (pat.include_pattern IS NULL OR qc_utility_krn.ext_regexp_like(REPLACE(pat.object_type,' ALIAS'),qc_utility_var.g_object_owner,dict.dict_key||'.'||dict.dict_value,pat.include_pattern)=1)
            AND (pat.exclude_pattern IS NULL OR qc_utility_krn.ext_regexp_like(REPLACE(pat.object_type,' ALIAS'),qc_utility_var.g_object_owner,dict.dict_key||'.'||dict.dict_value,pat.exclude_pattern)=0)
          UNION
         ...
         SELECT 50 sort_order, pat.object_type, trg.owner
          ...
          INNER JOIN qc_patterns pat2
             ON pat2.object_type = 'TABLE'
            AND pat2.app_alias IN ('ALL', qc_utility_var.g_app_alias)
          WHERE trg.owner = qc_utility_var.g_object_owner
         ...
         SELECT 70 sort_order, pat.object_type, com.owner, 'Comment on table' object_desc, com.table_name object_name
          ...
          INNER JOIN qc_patterns pat2
             ON pat2.object_type = 'TABLE'
            AND pat2.app_alias IN ('ALL', qc_utility_var.g_app_alias)
          WHERE com.owner = qc_utility_var.g_object_owner
...
--Replaced with checking of PL/SQL identifiers via PL/SCOPE
         ...
         SELECT 100 sort_order, pat.object_type, arg.owner
          ...
          INNER JOIN qc_patterns pat
            ...
            AND (p_anti_pattern = 'Y' OR pat.check_pattern IS NOT NULL)
            AND (p_anti_pattern = 'N' OR pat.anti_pattern IS NOT NULL)
          WHERE arg.owner = qc_utility_var.g_object_owner
           ...
           FROM (
               SELECT con.table_name pk_table_name, ccol.column_name pk_column_name, con.constraint_name pk_name
                ...
                INNER JOIN qc_dictionary_entries dict
                   ON dict.dict_name = CASE WHEN INSTR(pat_fk.check_pattern,' alias}')>0 THEN 'TABLE ALIAS'
                                            WHEN INSTR(pat_fk.check_pattern,' entity}')>0 THEN 'TABLE ENTITY'
                                            ELSE NULL
                                       END
                  AND dict.app_alias = qc_utility_var.g_app_alias
...
--                  AND dict_own.dict_key = 'TABLE'
                INNER JOIN all_cons_columns ccol
                  ...
                  AND INSTR(ccol.column_name,dict.dict_value) > 0 -- column name contains table alias or table entity
                  AND ccol.position = 1
          ...
          INNER JOIN qc_patterns pat_fk
           ...
           LEFT OUTER JOIN qc_dictionary_entries dict1
             ON dict1.dict_name = CASE WHEN INSTR(pat_fk.check_pattern,'{parent alias}')>0 THEN 'TABLE ALIAS'
                                       WHEN INSTR(pat_fk.check_pattern,'{parent entity}')>0 THEN 'TABLE ENTITY'
                                       ELSE NULL
                                  END
          ...
          INNER JOIN qc_patterns pat1
           ...
           LEFT OUTER JOIN qc_dictionary_entries dict2
             ON dict2.dict_name = CASE WHEN INSTR(pat_fk.check_pattern,'{table alias}')>0 THEN 'TABLE ALIAS'
                                       WHEN INSTR(pat_fk.check_pattern,'{child alias}')>0 THEN 'TABLE ALIAS'
                                       WHEN INSTR(pat_fk.check_pattern,'{table entity}')>0 THEN 'TABLE ENTITY'
                                       WHEN INSTR(pat_fk.check_pattern,'{child entity}')>0 THEN 'TABLE ENTITY'
                                       ELSE NULL
                                  END
      ...
      CURSOR c_obj IS
         ...
         SELECT DISTINCT 'INDEX' object_type, 'Index' object_desc, ind1.table_name||'.'||ind1.index_name object_name
          ...
          INNER JOIN ind ind2
            ...
            AND SUBSTR(ind2.ind_columns||',',1,LENGTH(ind1.ind_columns)+1) = ind1.ind_columns||','
      ...
      CURSOR c_obj IS
         SELECT * FROM (
            SELECT tcol.table_name||'.'||tcol.column_name object_name
                ...
                || CASE WHEN INSTR(dompat.check_pattern,'\(')>0 THEN
                        ...
                        END
                   END
                || CASE WHEN INSTR(dompat.check_pattern,'NULL')>0 THEN
   ...
   IS
      ...
      CURSOR c_obj IS
         SELECT con.table_name||'.'||con.constraint_name||'.'||ccol.column_name object_name
           ...
           FROM dual
          INNER JOIN all_constraints con
             ON con.owner = qc_utility_var.g_object_owner
            AND con.constraint_type = 'R'
          INNER JOIN all_tables tab
             ON tab.owner = qc_utility_var.g_object_owner
            AND tab.table_name = con.table_name -- to exclude views
            AND SUBSTR(tab.table_name,1,4) != 'BIN$'
          INNER JOIN qc_patterns pat
             ON pat.object_type = 'TABLE'
...
--                      AND src.line = tree.line
            ...
            , filtered AS (
               -- references towards a declaration
               SELECT *
                 FROM prepared
                WHERE usage='REFERENCE'
                  AND parent_usage = 'DECLARATION'
                  AND parent_type != 'FUNCTION'
                  AND INSTR(type_path,'RECORD/VARIABLE') = 0
                  AND type NOT IN ('PACKAGE','PACKAGE BODY','PROCEDURE','FUNCTION')
                UNION
               -- declarations without references
               SELECT p.* FROM prepared p
                 LEFT OUTER JOIN prepared c
                   ON c.owner = p.owner
                  AND c.object_type = p.object_type
                  AND c.object_name = p.object_name
                  AND c.parent_usage_id = p.usage_id
                  AND c.usage = 'REFERENCE'
                WHERE p.usage = 'DECLARATION'
                  AND c.object_type IS NULL --no child
                  AND p.type NOT IN ('PACKAGE','PACKAGE BODY','PROCEDURE','FUNCTION')
            ), categorized AS (
               SELECT CASE WHEN parent_type LIKE 'FORMAL%' THEN 'Parameter'
                           WHEN type_path NOT LIKE '/PACKAGE/PROCEDURE/%'
                            ...
                            AND type_path NOT LIKE '/FUNCTION/FUNCTION/%' THEN 'Global'
                           ELSE 'Local'
                       END dbcc_scope
                    , CASE WHEN parent_type = 'FORMAL IN' THEN 'In'
                           WHEN parent_type = 'FORMAL OUT' THEN 'Out'
                           WHEN parent_type = 'FORMAL IN OUT' THEN 'In Out'
                           WHEN parent_type = 'CONSTANT' THEN 'Constant'
                           ELSE 'Variable'
                       END dbcc_modifiability
                    , CASE WHEN parent_type IN ('ASSOCIATIVE ARRAY','INDEX TABLE','VARRAY','NESTED TABLE') THEN
                                CASE WHEN parent_usage = 'DECLARATION' THEN 'Table Type'
                                ELSE 'TBD-Table'
                                 END
                           WHEN parent_type = 'SUBTYPE' THEN
                                CASE WHEN parent_usage = 'DECLARATION' THEN 'SubType'
                                ELSE 'TBD-Subtype'
                                 END
                           WHEN type = 'CURSOR' THEN
                                CASE WHEN usage = 'DECLARATION' THEN 'Cursor'
                                     WHEN usage = 'REFERENCE' THEN 'Record'
                                     ELSE 'TBD-Cursor' --%ROWWTYPE
                                 END
                           WHEN type = 'RECORD' THEN
                                CASE WHEN usage = 'DECLARATION' THEN 'Record Type'
                                     WHEN usage = 'REFERENCE' THEN 'Record'
                                     ELSE 'TBD-Record'
                                 END
                           WHEN type = 'EXCEPTION' THEN
                                CASE WHEN usage = 'DECLARATION' THEN 'Exception'
                                     ELSE 'TBD-Exception'
                                 END
                           WHEN type IN ('ASSOCIATIVE ARRAY','INDEX TABLE','VARRAY','NESTED TABLE') THEN
                                CASE WHEN usage = 'REFERENCE' THEN 'Table'
                                     ELSE 'TBD-Table'
                                 END
                           WHEN type = 'OBJECT' THEN
                                CASE WHEN usage = 'REFERENCE' THEN 'Object'
                                     ELSE 'TBD-Object'
                                 END
                           WHEN type LIKE '%TYPE' THEN 'Scalar'
                           ELSE 'TBD-type'
                       END dbcc_type
                    , CASE WHEN usage = 'DECLARATION' THEN name ELSE parent_name END dbcc_name
                    , CASE WHEN usage = 'DECLARATION' THEN line ELSE parent_line END dbcc_line
                    , CASE WHEN usage = 'DECLARATION' THEN col  ELSE parent_col  END dbcc_col
               , f.*
                from filtered f
            )
            , default_patterned AS (
               ...
               SELECT c.*
                    ...
                    , SUBSTR(src.text,ids.col,LENGTH(ids.name)) src_name
                 FROM checked c
                INNER JOIN all_identifiers ids
                   ON ids.owner = qc_utility_var.g_object_owner
      ...
      CURSOR c_obj IS
         SELECT conpat.object_type, conx.owner
          ...
          WHERE NOT tab.table_name LIKE 'BIN$%' --necessary?
            AND tab.owner = qc_utility_var.g_object_owner
            AND (tabpat.include_pattern IS NULL OR qc_utility_krn.ext_regexp_like(tabpat.object_type,tab.owner,tab.table_name,tabpat.include_pattern)=1)
            AND (tabpat.exclude_pattern IS NULL OR qc_utility_krn.ext_regexp_like(tabpat.object_type,tab.owner,tab.table_name,tabpat.exclude_pattern)=0)
   ...
   IS
      ...
      CURSOR c_src IS
         SELECT src.owner, src.TYPE, src.NAME, UPPER(src.text) text
           FROM all_source src
          WHERE src.owner = qc_utility_var.g_object_owner
--          WHERE src.owner IN (
--               SELECT qc_utility_var.g_object_owner
...
--                   AND dict_own.dict_key LIKE dep.TYPE||'%'
                 WHERE dep.owner = qc_utility_var.g_object_owner
                   AND referenced_type = DECODE(INSTR(msg.object_type,':'),0,msg.object_type,SUBSTR(msg.object_type,1,INSTR(msg.object_type,':')-1))
                   AND referenced_name = DECODE(INSTR(msg.object_name,'.'),0,msg.object_name,SUBSTR(msg.object_name,INSTR(msg.object_name,'.')+1))
      ...
      IF t_msg.COUNT > 0 THEN
         ...
         FOR r_src IN c_src LOOP
            ...
            FOR i IN 1..t_msg.COUNT LOOP
               ...
               THEN
                  IF  INSTR(r_src.text,t_msg(i).object_name)>0 /*avoid expensive regexp operation here after*/
   ...
   BEGIN
      ...
      FOR r_msg IN c_msg(l_run_id) LOOP
         IF r_msg.msg_type IN ('E','W') THEN
            l_count := l_count + 1;
            pipe ROW('  '||CASE WHEN l_count > 1 THEN ',' END || ' {');
            pipe ROW('      "engineId": "QC_UTILITY"');
            pipe ROW('     ,"ruleId": "'||r_msg.qc_code||' :'||r_msg.descr||'"');
            pipe ROW('     ,"severity": "'||CASE r_msg.msg_type WHEN 'E' THEN 'BLOCKER' WHEN 'W' THEN 'MINOR' END||'"');
            pipe ROW('     ,"type": "BUG"');
...
--                OR (p_end_run_id IS NOT NULL AND run_id = p_end_run_id)
         ...
         SELECT REPLACE(NEW.object_type, ' COLUMN') object_type2, NEW.object_name, NEW.object_type
              , INITCAP(CASE WHEN INSTR(NEW.object_type,': ')>0 THEN SUBSTR(NEW.object_type,INSTR(NEW.object_type,': ')+2)||' '||SUBSTR(NEW.object_type,1,INSTR(NEW.object_type,': ')-1) ELSE NEW.object_type END)||' '||NEW.object_name
          ...
          INNER JOIN qc_run_msgs NEW
            ...
            AND NEW.object_owner = qc_utility_var.g_object_owner
           LEFT OUTER JOIN qc_run_msgs OLD
            ...
            AND OLD.object_owner = qc_utility_var.g_object_owner
           LEFT OUTER JOIN qc_run_msgs oldt -- get master object
            ...
            AND oldt.object_owner = qc_utility_var.g_object_owner
         ...
         SELECT REPLACE(OLD.object_type,' COLUMN') object_type2, OLD.object_name, OLD.object_type
              , INITCAP(CASE WHEN INSTR(OLD.object_type,': ')>0 THEN SUBSTR(OLD.object_type,INSTR(OLD.object_type,': ')+2)||' '||SUBSTR(OLD.object_type,1,INSTR(OLD.object_type,': ')-1) ELSE OLD.object_type END)||' '||OLD.object_name
          ...
          INNER JOIN qc_run_msgs OLD
           ...
           LEFT OUTER JOIN qc_run_msgs newt -- get master object
            ...
            AND newt.object_owner = qc_utility_var.g_object_owner
   ...
   BEGIN
      ...
      FOR r_chk IN c_chk(p_qc_code) LOOP
         -- If QC is included and not excluded according to the pattern
          ...
          OR ((r_pat.include_pattern IS NULL OR ext_regexp_like('QUALITY CHECK',qc_utility_var.g_object_owner,r_chk.qc_code, r_pat.include_pattern) = 1)
         AND  (r_pat.exclude_pattern IS NULL OR ext_regexp_like('QUALITY CHECK',qc_utility_var.g_object_owner,r_chk.qc_code, r_pat.exclude_pattern) = 0))
         THEN
            ...
            BEGIN
               ...
               qc_utility_msg.load_msg(t_msg=>t_msg,p_qc_code=>r_chk.qc_code,p_app_alias=>qc_utility_var.g_app_alias,p_object_owner=>qc_utility_var.g_object_owner,p_run_id=>l_run_id);
               <<msg_loop>>
               FOR i IN 1..t_msg.count LOOP
                  r_msg := t_msg(i);
...
--dbms_output.put_line('messages updated: '||qc_utility_var.t_msg_upd.COUNT);
            ...
            qc_utility_msg.save_msg(t_msg=>qc_utility_var.t_msg,p_qc_code=>r_chk.qc_code,p_app_alias=>qc_utility_var.g_app_alias,p_object_owner=>qc_utility_var.g_object_owner,p_run_id=>l_run_id);
            -- Save statistics
            qc_utility_stat.save_stat(t_stat=>qc_utility_var.t_stat,p_qc_code=>r_chk.qc_code,p_app_alias=>qc_utility_var.g_app_alias,p_object_owner=>qc_utility_var.g_object_owner,p_run_id=>l_run_id);
         END IF;
         <<next_qc>>
         NULL;
      END LOOP chk_loop;
      END LOOP own_loop;
      END LOOP app_loop;
      l_msg_count := send_report(l_run_id,p_app_alias);
...
--#end public
           ...
           FROM (
               SELECT DISTINCT -- distinct because there are 2 possible simultaneous usages: REFERENCE and ASSIGNMENT
                ...
                INNER JOIN all_identifiers usage
                   ON usage.owner = SUBSTR(decl.object_name,1,INSTR(decl.object_name,'.')-1)
   --               AND usage.name = SUBSTR(decl.object_name,INSTR(decl.object_name,'.')+1,INSTR(decl.object_name,'#')-INSTR(decl.object_name,'.')-1)
                  AND usage.signature = SUBSTR(decl.object_name,INSTR(decl.object_name,'#')+1)
                WHERE decl.app_alias = qc_utility_var.g_app_alias
                  AND decl.object_owner = qc_utility_var.g_object_owner
   ...
   --               AND decl.type NOT IN ('PACKAGE', 'PROCEDURE', 'FUNCTION') -- ???
               ...
               SELECT CASE WHEN proc_call.object_type IN ('PACKAGE','TYPE') THEN 1 WHEN proc_call.object_type = 'TRIGGER' THEN 3 ELSE 2 END seq
                    ...
                    , regexp_instr(COALESCE(src0.text,src1.text,src2.text,src3.text,src4.text), param.name||'[ '||CHR(9)||']*=>',1,1,0,'i') col
                    , decl.fix_name
                ...
                INNER JOIN all_identifiers param -- parameter declaration
                   ON param.owner = SUBSTR(decl.object_name,1,INSTR(decl.object_name,'.')-1)
                  AND param.signature = SUBSTR(decl.object_name,INSTR(decl.object_name,'#')+1)
...
--                 , fix_ddl = r_msg.fix_ddl
   ...
   BEGIN
      ...
      FOR r_fix IN c_fix LOOP
         ...
         l_col := l_col - l_len + LENGTH(r_fix.fix_name);
         -- Update fix info
      END LOOP fix_loop;
      -- Force last break
      compile_source(l_owner,l_type,l_name);
...
--#end public
   ...
   BEGIN
      ...
      ;
      IF SQL%ROWCOUNT <= 0 THEN
         IF INSTR(p_app_alias,'%')>0 OR INSTR(p_app_alias,'_')>0 THEN
            raise_application_error(-20000,k_where||'wildcards not allowed in application alias when inserting');
         END IF;
         IF INSTR(p_object_type,'%')>0 OR INSTR(p_object_type,'_')>0 THEN
   ...
   BEGIN
      ...
      IF SQL%ROWCOUNT <= 0 THEN
         IF INSTR(p_app_alias,'%')>0 OR INSTR(p_app_alias,'_')>0 THEN
            raise_application_error(-20000,k_where||'wildcards not allowed in application alias when inserting');
         END IF;
         IF INSTR(p_dict_name,'%')>0 OR INSTR(p_dict_name,'_')>0 THEN
            raise_application_error(-20000,k_where||'wildcards not allowed in dictionary name when inserting');
         END IF;
         IF INSTR(p_dict_key,'%')>0 THEN -- undercore is a valid character for key
