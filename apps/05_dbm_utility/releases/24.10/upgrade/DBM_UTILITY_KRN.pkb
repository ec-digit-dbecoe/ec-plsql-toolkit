         ...
         IF l_verbose THEN
            ...
            WHILE l_text IS NOT NULL LOOP
               IF NOT la_priv_res(l_text) THEN
                  print_line('Info: '||l_prv_desc||' privilege "'||l_text||'" is granted');
               END IF;
               l_text := la_priv_res.NEXT(l_text);
            END LOOP;
         END IF;
...
--         output_line(pr_cmd.cmd_id,'IN','prompt Migrating application "'||r_app.app_code||'"...');
   ...
   IS
      ...
      IS
         SELECT app.*, ver.ver_code next_ver_code, ver.next_op_type next_ver_op_type
           FROM dbm_all_applications app
           LEFT OUTER JOIN dbm_versions ver
             ON ver.app_code = app.app_code
   ...
   BEGIN
      ...
      FOR r_app IN c_app(p_owner=>CASE WHEN l_all THEN NULL ELSE USER END) LOOP
         ...
         l_version := 'has '||CASE WHEN r_app.next_ver_code IS NULL THEN 'latest' ELSE 'old' END||' version "'||r_app.ver_code||'" installed';
         IF r_app.app_code != 'all' THEN
            ...
            l_status := ', status is "'||NVL(r_app.ver_status,'UNKNOWN')||'"';
         ...
         ELSE
            ...
            IF l_obj_count = 0 AND r_app.owner = USER THEN
               dbms_output.put_line('Application "'||r_app.app_code||'" is not installed'||CASE WHEN l_all THEN ' locally' END||l_next);
            ELSE
            dbms_output.put_line('Application "'||r_app.app_code||'" '||l_owned_by||l_version||l_status||l_accessible||l_next);
            END IF;
         END IF;
