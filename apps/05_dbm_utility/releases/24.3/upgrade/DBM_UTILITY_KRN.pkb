...
END;
   ...
   -- Output line to the in/out/err stream of a command
   ---
   PROCEDURE output_whenever_sqlerror_exit (
      p_cmd_id IN dbm_streams.cmd_id%TYPE
...
--#end public
   ...
   BEGIN
      FOR x IN 1..2 LOOP
         l_app_code := CASE x WHEN 1 THEN p_app_code ELSE 'all' END;
         FOR y IN 1..2 LOOP
            l_ver_code := CASE y WHEN 1 THEN p_ver_code ELSE 'all' END;
   ...
   IS
   BEGIN
      RETURN get_par_value('dbm_utility', 'all', 'get_file_extensions','sql, pks, pkb, pls, plb, java');
   END;
   ---
   ...
   BEGIN
      ...
      FOR r_fil IN c_fil LOOP
         ...
         END IF;
         output_line(p_cmd_id, 'IN', 'host bin\get-hashes '||l_apps_dir||r_fil.dir||' ' ||p_cmd_id||' chk-hashes '||l_apps_dir||' >'||get_tmp_dir(TRUE)||'get-hashes.sql');
         output_line(p_cmd_id, 'IN', '@@'||get_tmp_dir(TRUE)||'get-hashes.sql');
      END LOOP;
...
--#end public
   ...
   BEGIN
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         IF NOT p_from_other_op THEN
            ...
            output_whenever_sqlerror_exit(p_cmd_id, 'IN');
   ...
   BEGIN
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         assert(l_found, 'Application "'||r_app.app_code||'" is not installed!');
         assert(lt_ver.EXISTS(r_ver.ver_nbr),'Version "'||r_ver.ver_code||'" of application "'||r_ver.app_code||'" not found on file system!');
         l_label := CASE p_command WHEN 'setup' THEN 'Setting up' WHEN 'expose' THEN 'Exposing' WHEN 'conceal' THEN 'Concealing' END;
         assert(l_label IS NOT NULL, 'Unimplemented command: '||p_command);
         IF p_command IN ('expose','conceal') THEN
            define_variables(p_cmd_id, r_ver, 'IN');
            output_line(p_cmd_id,'IN','define grantee="'||NVL(p_params,'public')||'"');
            output_whenever_sqlerror_exit(p_cmd_id, 'IN');
         END IF;
         output_line(p_cmd_id,'IN','prompt '||l_label||' application "'||r_ver.app_code||'" version "'||r_ver.ver_code||'"...');
         output_whenever_sqlerror_exit(p_cmd_id, 'IN');
         output_line(p_cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_ver(p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'",p_last_op_type=>"'||UPPER(p_command)||'", p_last_op_status=>"ONGOING")','"',''''));
         ...
         FOR k IN 1..3 LOOP
            ...
            IF dbm_utility_var.ga_app.EXISTS(l_app_code) THEN
               ...
               IF lt_ver.EXISTS(l_ver_nbr) THEN
                  lr_ver := lt_ver(l_ver_nbr);
                  lt_files := CASE p_command WHEN 'setup' THEN lr_ver.t_set_files WHEN 'expose' THEN lr_ver.t_exp_files WHEN 'conceal' THEN lr_ver.t_con_files END;
                  FOR i IN NVL(lt_files.FIRST,1)..NVL(lt_files.LAST,0) LOOP
                     l_path := lt_files(i);
                     IF lr_ver.a_fil.EXISTS(l_path) AND NOT file_condition_met(l_app_code, l_ver_nbr, l_path, lr_ver.a_fil(l_path)) THEN
                        output_line(p_cmd_id,'IN','prompt Skipping script "'||get_apps_dir(TRUE)||l_path||'" (condition not met)...');
                     ELSE
                        output_line(p_cmd_id,'IN','prompt Executing script "'||get_apps_dir(TRUE)||l_path||'"...');
                        output_line(p_cmd_id,'IN','@@'||get_apps_dir(TRUE)||l_path);
            ...
            END IF;
         END LOOP;
         IF r_ver.app_code = 'dbm_utility' AND p_command = 'conceal' THEN
         ...
         IF l_files_count = 0 THEN
            output_line(p_cmd_id, 'IN', 'prompt No file was found to '||p_command||' application "'||r_ver.app_code||'" version "'||r_ver.ver_code||'"');
         ELSE
            output_line(p_cmd_id,'IN','prompt '||INITCAP(p_command)||' of application "'||r_ver.app_code||'" version "'||r_ver.ver_code||'" was successful');
         END IF;
         IF r_app.app_code = 'dbm_utility' AND p_command = 'conceal' THEN
            -- Reload cache (twice for "ERROR-04068: Existing state of packages has been discarded" + hide errors)
   ...
   IS
      ...
      IS
         ...
         SELECT seq, app_code, USER owner, ver_code, home_dir
           FROM dbm_applications 
          WHERE (NVL(p_app_code,'all')='all' OR app_code = p_app_code)
            AND ver_code IS NOT NULL -- exclude not installed apps
            AND app_code != 'all'
          UNION
         -- Applications installed centrally
         SELECT cen.seq, cen.app_code, cen.owner, cen.ver_code, cen.home_dir
   ...
   BEGIN
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         IF NOT l_found AND r_app.owner != sys_context('USERENV','SESSION_USER') THEN
            ...
            r_ver.ver_code := r_app.ver_code;
            r_ver.ver_nbr := get_version_nbr(r_app.ver_code);
         ELSE
            assert(l_found, 'Application "'||r_app.app_code||'" is not installed!');
         END IF;
         assert(lt_ver.EXISTS(r_ver.ver_nbr),'Version "'||r_ver.ver_code||'" of application "'||r_ver.app_code||'" not found on file system!');
...
--         output_line(pr_cmd.cmd_id,'IN','prompt Migrating application "'||r_app.app_code||'"...');
         ...
         FOR r_ver IN c_ver(r_app.app_code, pr_ver.ver_status) LOOP
            ...
            -- Check dependencies
            output_whenever_sqlerror_exit(pr_cmd.cmd_id, 'IN');
            IF l_dependencies THEN
            ...
            FOR k IN 1..2 LOOP
               ...
               FOR i IN NVL(lt_files.FIRST,1)..NVL(lt_files.LAST,0) LOOP
                  l_path := lt_files(i);
                  IF lr_ver.a_fil.EXISTS(l_path) AND NOT file_condition_met(r_ver.app_code, r_ver.ver_nbr, l_path, lr_ver.a_fil(l_path)) THEN
                     output_line(pr_cmd.cmd_id,'IN','prompt Skipping pre-check script "'||get_apps_dir(TRUE)||l_path||'"(condition not met)...');
                  ELSE
                     output_line(pr_cmd.cmd_id,'IN','prompt Executing pre-check script "'||get_apps_dir(TRUE)||l_path||'"...');
                     output_line(pr_cmd.cmd_id,'IN','@@'||get_apps_dir(TRUE)||l_path);
                  END IF;
                  EXIT WHEN lt_files.EXISTS(0);
               END LOOP;
            END LOOP;
            ...
            IF lt_files.FIRST IS NOT NULL THEN
               output_line(pr_cmd.cmd_id, 'IN', 'host bin\get-hashes '||l_apps_dir||get_dir(lt_files(lt_files.FIRST))||' ' ||pr_cmd.cmd_id||' set-hashes '||l_apps_dir||' >'||get_tmp_dir(TRUE)||'get-hashes.sql');
            ...
            FOR i IN NVL(lt_files.FIRST,1)..NVL(lt_files.LAST,0) LOOP
               ...
               IF get_file_run_status(l_path) = 'SUCCESS' THEN
                  output_line(pr_cmd.cmd_id,'IN','prompt Skipping already executed script "'||l_path||'"...');
               ELSE
                  IF lr_ver.a_fil.EXISTS(l_path) AND NOT file_condition_met(r_ver.app_code, r_ver.ver_nbr, l_path, lr_ver.a_fil(l_path)) THEN
                     output_line(pr_cmd.cmd_id,'IN','prompt Skipping script "'||get_apps_dir(TRUE)||l_path||'" (condition not met)...');
                  ELSE
                     IF r_ver.app_code != 'dbm_utility' THEN
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||l_path||'", p_run_status=>"ONGOING")','"',''''));
                     END IF;
                     output_line(pr_cmd.cmd_id,'IN','prompt Executing script "'||get_apps_dir(TRUE)||l_path||'"...');
                     output_line(pr_cmd.cmd_id,'IN','@@'||get_apps_dir(TRUE)||l_path);
                     ...
                     END IF;
                     IF r_ver.app_code != 'dbm_utility' THEN
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||l_path||'", p_run_status=>"SUCCESS")','"',''''));
                     END IF;
                  END IF;
               END IF;
               EXIT WHEN lt_files.EXISTS(0);
            ...
            -- Validate migration
            IF l_validate THEN
               validate(pr_cmd.cmd_id, r_app.app_code, r_ver.ver_code, TRUE);
            END IF;
            -- Terminate
            ...
            IF r_ver.last_op_type = 'INSTALL' THEN
               output_line(pr_cmd.cmd_id,'IN',REPLACE('exec UPDATE dbm_files SET run_status = NULL, run_date = NULL WHERE app_code="'||r_ver.app_code||'" AND type!="INSTALL"','"',''''));
            ...
            ELSIF r_ver.last_op_type = 'UPGRADE' THEN
               output_line(pr_cmd.cmd_id,'IN',REPLACE('exec UPDATE dbm_files SET run_status = NULL, run_date = NULL WHERE app_code="'||r_ver.app_code||'" AND type NOT IN ("INSTALL","UPGRADE")','"',''''));
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         r_option := pr_cmd.t_options(i);
         IF r_option.name = 'force' THEN
            l_force := TRUE;
         ELSIF r_option.name = 'noforce' THEN
            l_force := FALSE;
         ELSE
            assert(FALSE, 'Invalid option "'||r_option.name||'" for command "'||pr_cmd.command||'"');
         END IF;
      ...
      FOR r_app IN c_app(p_app_code) LOOP
                  ...
                  FOR i IN NVL(lt_files.FIRST,1)..NVL(lt_files.LAST,0) LOOP
                     ...
                     ELSE
                        ...
                        IF r_app.app_code != 'dbm_utility' THEN
                           output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||l_path||'", p_run_status=>"ONGOING")','"',''''));
                        END IF;
                        output_line(pr_cmd.cmd_id,'IN','@@'||get_apps_dir(TRUE)||l_path||' '||r_ver.ver_code);
                        IF r_app.app_code != 'dbm_utility' THEN
                           output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||l_path||'", p_run_status=>"SUCCESS")','"',''''));
                        END IF;
                     END IF;
         ...
         IF r_ver.app_code != 'dbm_utility' THEN
            ...
            output_line(pr_cmd.cmd_id,'IN',REPLACE('exec UPDATE dbm_files SET run_status = NULL, run_date = NULL WHERE app_code="'||r_ver.app_code||'" AND type!="UNINSTALL"','"','''')); -- all versions
         END IF;
...
--               update_fil(p_path=>r_fil.path, p_run_condition=>r_fil.run_condition);
         ...
         ELSIF SUBSTR(LOWER(lt_path(lt_path.COUNT)),-4) = '.dbm' THEN
            l_pos := NVL(INSTR(r_str.text, ':'),0);
            IF l_pos > 0 THEN
               l_path := get_dir(l_path,TRUE)||LTRIM(RTRIM(SUBSTR(r_str.text, 1, l_pos-1)),'@');
               la_fil(l_path) := TRIM(SUBSTR(r_str.text, l_pos+1)); -- run condition
            ELSE
               l_path := get_dir(l_path,TRUE)||LTRIM(RTRIM(r_str.text),'@');
            END IF;
            ...
            ELSIF LOWER(lt_path(lt_path.COUNT)) = 'expose.dbm' THEN
               lt_exp(lt_exp.COUNT+1) := l_path;
            ELSIF LOWER(lt_path(lt_path.COUNT)) = 'conceal.dbm' THEN
               lt_con(lt_con.COUNT+1) := l_path;
...
--#end public
   ...
   BEGIN
      <<options_loop>>
      IF dbm_utility_var.g_trace THEN
         dbms_output.put_line('->process_dbm_options(): '||pr_cmd.t_options.COUNT||' options');
      END IF;
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         r_option := pr_cmd.t_options(i);
      ...
      IF p_save_cache AND pr_cmd.t_options.COUNT > 0 THEN
         save_cache;
      END IF;
      IF dbm_utility_var.g_trace THEN
...
--    output_line(r_cmd.cmd_id, 'IN', 'host del find-files.sql'); -- TBD, OS dependent
   ...
   BEGIN
      IF pr_cmd.t_params.COUNT > 0 THEN
         assert(LENGTH(pr_cmd.t_params(1))<=30, 'Application code must be 30 characters max!');
         r_ver.app_code := pr_cmd.t_params(1);
      ELSE
         r_ver.app_code := NULL;
      END IF;
      ...
      ELSIF pr_cmd.command = 'validate' THEN
         validate(pr_cmd.cmd_id, r_ver.app_code, r_ver.ver_code);
      ELSIF pr_cmd.command = 'configure' THEN
         configure(pr_cmd.cmd_id, r_ver);
      ELSIF pr_cmd.command IN ('setup') THEN
         setup(pr_cmd.cmd_id, r_ver.app_code, NULL);
      ELSIF pr_cmd.command IN ('expose','conceal') THEN
         expose_conceal(pr_cmd.cmd_id, r_ver.app_code, NULL, pr_cmd.command, l_schema);
      ELSIF pr_cmd.command = 'rollback' THEN
         rollback_migration(pr_cmd.cmd_id, r_ver.app_code, r_ver.ver_code);
      ELSIF pr_cmd.command = 'check-files' THEN
         check_files(pr_cmd.cmd_id, r_ver.app_code, r_ver.ver_code);
      ...
      ELSIF pr_cmd.command = 'best-guess-current' THEN
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.guess_set_current('||pr_cmd.cmd_id||', "'||r_ver.app_code||'", TRUE,TRUE);','"',''''));
      ELSIF pr_cmd.command = 'guess-current' THEN
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.guess_set_current('||pr_cmd.cmd_id||', "'||r_ver.app_code||'", TRUE,FALSE);','"',''''));
      ELSIF pr_cmd.command = 'set-current' THEN
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.set_current('||pr_cmd.cmd_id||', "'||r_ver.app_code||'", "'||r_ver.ver_code||'");','"',''''));
      ELSIF pr_cmd.command = 'make-inventory' THEN
         make_inventory(pr_cmd.cmd_id, r_ver.app_code, r_ver.ver_code);
...
--#end public
   IS
      ...
      RETURN dbm_utility_var.gr_command_type
      IS
      ...
      BEGIN
         ...
         WHILE p_col_no <= p_len AND SUBSTR(p_command_line, p_col_no, 1) = '-' LOOP
            ...
            consume_white_spaces(p_command_line, p_col_no, p_len);
         END LOOP options_loop;
         IF r_command.command != 'execute' THEN
            <<param_loop>>
            WHILE p_col_no <= p_len AND SUBSTR(p_command_line, p_col_no, 1) != '/' LOOP
...
--         IF l_col_no <= l_len THEN
--            output_line(p_cmd_id=>r_cmd.cmd_id, p_type=>'IN', p_text=>'host bin\plus @dbm-cli '||SUBSTR(p_command_line,l_col_no));
--         END IF;
      END;
   ...
   BEGIN
      ...
      <<commands_loop>>
      LOOP
      r_command := parse_command_line(p_command_line, l_col_no, l_len);
      l_command := r_command.command;
      IF l_command IS NULL THEN
         assert(l_cmd_cnt>0,'Missing command!');
         EXIT commands_loop;
      END IF;
      r_command.command := get_full_cmd(r_command.command);
      assert(r_command.command IS NOT NULL, 'Invalid command "'||l_command||'"!');
      l_command := r_command.command;
      l_cmd_cnt := l_cmd_cnt + 1;
--dbms_output.put_line('Command: '||l_command);
--dbms_output.put_line('Options: #'||r_command.t_options.COUNT);
--dbms_output.put_line('Params: #'||r_command.t_params.COUNT);
--dbms_output.put_line('Remain: '||SUBSTR(p_command_line, l_col_no));
--assert(l_cmd_cnt<=10,'Infinite loop detected!');
      IF l_command IS NULL THEN
      ...
      ELSIF l_command = 'execute' THEN
         insert_command(FALSE);
         IF r_command.t_options.COUNT > 0 THEN
            process_dbm_options (
               pr_cmd=>r_command
             , p_process_options=>TRUE
             , p_process_params=>TRUE
...
--          output_line(r_cmd.cmd_id, 'IN', 'host del read-files.sql'); -- TBD, OS dependent
      ...
      ELSIF l_command = 'create-restore-point' THEN
         ...
         output_whenever_sqlerror_exit(r_cmd.cmd_id, 'IN');
      ...
      END IF;
      IF l_batch AND l_command != 'execute' THEN
         l_exit := dbm_utility_var.g_exit;
      END IF;
...
--#end public
   ...
   BEGIN
      l_cmd_id := begin_command(p_command_line, p_cmd_id, p_log);
   END;
--#begin public
   ---
   -- Terminate a command
