...
--         output_line(p_cmd_id,'IN','prompt Migrating application "'||r_app.app_code||'"...');
   ...
   BEGIN
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         assert(lt_ver.EXISTS(r_ver.ver_nbr),'Version "'||r_ver.ver_code||'" of application "'||r_ver.app_code||'" not found on file system!');
         output_whenever_sqlerror_exit(p_cmd_id, 'IN');
         output_line(p_cmd_id,'IN','prompt Uninstalling application "'||r_ver.app_code||CASE WHEN NVL(r_ver.ver_code,'all')!='all' THEN '" version "'||r_ver.ver_code||'"' END||'...');
...
--            l_cmd := SUBSTR(p_command_line, l_col_no);
            ...
            IF p_execute_command THEN
               save_cache$;
               output_exit(FALSE);
               begin_command(l_cmd, p_cmd_id, l_log);
               l_log := FALSE;
            END IF;
         END IF;
         consume_white_spaces(p_command_line, l_col_no, p_len);
   ...
   BEGIN
      ...
      IF r_ver.app_code IS NOT NULL THEN
         ...
         ELSIF p_command = 'uninstall' AND LOWER(SUBSTR(p_command_line, l_col_no, 5)) = 'force' THEN
            ...
            l_force := TRUE;
         ELSIF p_command IN ('expose','conceal') THEN
            l_buf := LOWER(consume_identifier(p_command_line, l_col_no, p_len));
         END IF;
      END IF;
      ...
      ELSIF p_command IN ('setup') THEN
         setup(p_cmd_id, r_ver.app_code, NULL);
      ELSIF p_command IN ('expose','conceal') THEN
         expose_conceal(p_cmd_id, r_ver.app_code, NULL, p_command, l_buf);
...
--#end public
   IS
      ...
      BEGIN
         ...
         END IF;
         IF l_log THEN
            r_cmd.log_file_name := 'logs'||'\'||TO_CHAR(r_cmd.start_date_time,'YYYYMMDDHH24MISS')||'-'||r_cmd.cmd_id||'-'||TRANSLATE(TRIM(p_command_line),' /','--')||'.log';
            UPDATE dbm_commands
      ...
      BEGIN
         ...
         UPDATE dbm_streams
          ...
          WHERE cmd_id = r_cmd.cmd_id
            AND type = 'IN';
         COMMIT;
         output_line(p_cmd_id=>r_cmd.cmd_id, p_type=>'IN', p_text=>'host bin\'||p_sqlplus_script||' @tmp\step2', p_line=>1);
      END;
