...
---
   ...
   g_file_extensions VARCHAR2(500) := 'sql, pks, pkb, pls, plb, java';
      ...
      SELECT new.name
        ...
        FROM (
         ...
         SELECT obj.object_type||' '||obj.object_name name, MOD(SUM(sys.dbms_utility.get_hash_value(REGEXP_REPLACE(src.text,'\s*$'),1000000000,POWER(2,30))),POWER(2,30)) checksum, obj.status
          ...
          WHERE REGEXP_LIKE(obj.object_type||' '||obj.object_name, p_object_name_pattern)
         ...
         SELECT ind.uniqueness, ind.table_name, ind.index_name, ind.status
          ...
          WHERE REGEXP_LIKE('TABLE '||ind.table_name, p_object_name_pattern)
         ...
         SELECT 'SEQUENCE '||sequence_name, MOD(sys.dbms_utility.get_hash_value(' INCREMENT BY ' || increment_by
           ...
           FROM user_sequences
          WHERE REGEXP_LIKE('SEQUENCE '||sequence_name, p_object_name_pattern)
         ...
         SELECT obj.object_type||' '||obj.object_name, NULL, obj.status
           FROM user_objects obj
          WHERE obj.object_type NOT IN ('PACKAGE','PACKAGE BODY','PROCEDURE','FUNCTION','TYPE','TYPE BODY','TRIGGER'
