...
--
   ...
   BEGIN
      ...
      OR NOT p_color IN ('black','red','green','yellow','blue','magenta','cyan','white')
      THEN
         RETURN p_text;
      END IF;
      RETURN 
...
--    , p_app_code IN dbm_applications.app_code%TYPE
   ...
   BEGIN
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         assert(l_found, 'Application "'||r_app.app_code||'" is not installed!');
         assert(NVL(r_ver.deleted_flag,'N') = 'N','Version "'||r_ver.ver_code||'" of application "'||r_ver.app_code||'" no more exists on file system!');
         l_label := CASE pr_cmd.command WHEN 'expose' THEN 'Exposing' WHEN 'conceal' THEN 'Concealing' END;
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         r_option := pr_cmd.t_options(i);
         IF is_prefix(r_option.name, 'verbose', 4) THEN
            l_verbose := TRUE;
         ELSIF is_prefix(r_option.name, 'path', 4) THEN
            l_path := r_option.value;
            assert(l_path IS NOT NULL,'Missing value for option "'||r_option.name||'"');
         ELSIF is_prefix(r_option.name, 'date', 4) OR is_prefix(r_option.name, 'time', 4) THEN
   ...
   BEGIN
      ...
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         ELSIF is_prefix(r_option.name, 'nodependencies', 5) THEN
            l_nodep := TRUE;
         ELSIF is_prefix(r_option.name, 'reexecute', 4) THEN
            l_reexecute := TRUE;
         ELSIF p_precheck_only AND is_prefix(r_option.name, 'nostop-on-error', 6) THEN
            l_nostop := TRUE;
         ELSIF is_prefix(r_option.name, 'jump-to-statement', 4) THEN
         ...
         ELSIF is_prefix(r_option.name, 'skip-file', 4) THEN
            assert(r_option.value is not null, 'Missing file name for "skip-file" option!');
            l_skip_file := r_option.value;
         ELSIF is_prefix(r_option.name, 'nocond-stmt-exec', 6) THEN
   ...
   BEGIN
      ...
      FOR j IN 1..13 LOOP
         ...
         FOR i IN NVL(t_files.FIRST,1)..NVL(t_files.last,0) LOOP
            ...
            upsert_fil(p_app_code=>r_fil.app_code, p_ver_code=>r_fil.ver_code, p_path=>r_fil.path, p_type=>r_fil.type
...
--                  la_par.DELETE;
         ...
         ELSIF SUBSTR(LOWER(lt_path(lt_path.COUNT)),-4) = '.dbm' THEN
            ...
            IF get_file_extension(l_tmp) IS NOT NULL THEN
               l_tmp := get_dir(l_path,TRUE)||l_tmp;
               IF l_pos > 0 THEN
