...
--
   ---
   -- Return colored text (using ASCII escape characters)
   ---
   FUNCTION color (
      p_text IN VARCHAR2
    , p_color IN VARCHAR2
   ...
   BEGIN
      IF p_text IS NULL
      OR NOT dbm_utility_var.g_color
      OR NOT NVL(p_color,'none') IN ('black','red','green','yellow','blue','magenta','cyan','white')
      THEN
         RETURN p_text;
      END IF;
      RETURN 
         CHR(27)
      || CASE p_color WHEN 'black' THEN '[30m'
                      WHEN 'red' THEN '[31m'
                      WHEN 'green' THEN '[32m'
                      WHEN 'yellow' THEN '[33m'
                      WHEN 'blue' THEN '[34m'
                      WHEN 'magenta' THEN '[35m'
                      WHEN 'cyan' THEN '[36m'
                      WHEN 'white' THEN '[37m'
         END
      || p_text
   ...
   ---
   -- Return colored text (using ASCII escape characters)
   -- Colors are specified like this: {text;color}
   ---
   FUNCTION color (
      p_text IN VARCHAR2
   )
   ...
   BEGIN
      LOOP
         l_beg := NVL(INSTR(l_text,'{'),0);
         EXIT WHEN NOT l_beg > 0;
         l_end := NVL(INSTR(p_text,'}',l_beg+1),0);
         assert(l_end > 0, 'Internal error in color(): "{" without "}" in: '||p_text);
         l_mid := NVL(INSTR(p_text,';',l_beg+1),0);
         assert(l_mid BETWEEN l_beg+1 AND l_end-1, 'Internal error in color(): ";" missing between "{" and "}"'||p_text);
         l_text := SUBSTR(l_text,1,l_beg-1)
                || color(SUBSTR(l_text,l_beg+1,l_mid-l_beg-1),SUBSTR(l_text,l_mid+1,l_end-l_mid-1))
                || SUBSTR(l_text,l_end+1);
      END LOOP;
      RETURN l_text;
   END;
   ...
   END;
   -- Check whether a string is a valid prefix of another string.
   -- The mandatory prefix is identified with an open parenthesis.
   -- e.g. exec)ute matches exec, execu, execut,and execute.
   FUNCTION is_matching_prefix (
      p_str1 IN VARCHAR2
    , p_str2 IN VARCHAR2
   )
   RETURN BOOLEAN
   IS
   BEGIN
      RETURN is_prefix(p_str1, REPLACE(p_str2,')'), NVL(INSTR(p_str2,')')-1,0));
   END;
   -- Check whether a string starts with a sqlplus command or abbreviation
   -- Return length of command or abbreviation found and 0 if not found
   FUNCTION in_sqlplus_cmd (
      p_str IN VARCHAR2
    , p_cmd IN VARCHAR2
    , p_min_len IN PLS_INTEGER := NULL
   )
   RETURN PLS_INTEGER
   IS
      l_str_len PLS_INTEGER := NVL(LENGTH(p_str),0);
      l_cmd_len PLS_INTEGER := NVL(LENGTH(p_cmd),0);
   BEGIN
      FOR l_len IN p_min_len..l_cmd_len LOOP
        IF UPPER(SUBSTR(p_str||' ',1,l_len+1)) = UPPER(SUBSTR(p_cmd,1,l_len)||' ') THEN
           RETURN l_len;
        END IF;
      END LOOP;
      RETURN 0;
   END;
   ---
   -- Get slash or backslash depending on OS
   ---
   FUNCTION get_slash
   RETURN VARCHAR2
   IS
   BEGIN
      RETURN CASE dbm_utility_var.g_os_name WHEN 'Linux' THEN '/' ELSE '\' END; 
   END;
   ---
   -- rtrim slashes
   ---
   ...
   IS
   BEGIN
      RETURN RTRIM(RTRIM(p_str,'/'),'\');
   END;
   ---
   -- Adapt slashes to the OS
   ---
   FUNCTION adapt_slashes (
      p_str IN VARCHAR2
   )
   RETURN VARCHAR2
   IS
...
--#end public
   IS
      ...
      l_lf_terminated BOOLEAN := FALSE;
      k_chunk_size CONSTANT PLS_INTEGER := 20480; -- 20k
   BEGIN
      IF dbm_utility_var.g_os_name = 'Linux' THEN
         l_text := REPLACE(l_text, '\', '/');
      END IF;
      IF SUBSTR(p_text,1,5) = 'host '
      OR SUBSTR(p_text,1,1) = '@'
      THEN
         l_text := adapt_slashes(p_text);
      END IF;
      IF p_line IS NULL THEN
            ...
            r_str.type := p_type;
            l_update := p_base64 AND p_chunk>0;
--         END IF;
      ELSE
         r_str.cmd_id := p_cmd_id;
         r_str.type := p_type;
      ...
      IF p_base64 THEN
         ...
         l_lf_terminated := SUBSTR(l_text,LENGTH(l_text),1) = CHR(10);
      END IF;
      l_len := LENGTH(l_text);
      WHILE l_beg < l_len LOOP
         l_pos := NVL(INSTR(l_text,CHR(10),l_beg),0);
         EXIT WHEN l_pos <= 0;
--         IF l_pos - l_beg > 0 THEN -- store empty lines!
            r_str.text := REPLACE(SUBSTR(l_text,l_beg,l_pos-l_beg),CHR(13)); -- remove CRs
            IF l_update THEN
               UPDATE dbm_streams
                  SET text = text || r_str.text
                WHERE cmd_id = p_cmd_id
                  AND type = p_type
                  AND line = r_str.line - 1;
               l_update := FALSE;
            ELSE
               ...
               END IF;
            END IF;
--         END IF; -- store empty lines!
         l_beg := l_pos + 1;
      END LOOP;
      IF (l_len - l_beg + 1 > 0)
      OR (p_base64 AND l_len=k_chunk_size AND l_lf_terminated)
      THEN
         r_str.text := REPLACE(SUBSTR(l_text,l_beg,l_len-l_beg+1),CHR(13)); -- remove CRs
         IF p_line IS NULL THEN
            IF l_update THEN
               UPDATE dbm_streams
                  SET text = text || r_str.text
...
END;
   ...
   IS
   BEGIN
      output_line(p_cmd_id, p_type, 'PROMPT '||CASE WHEN SUBSTR(p_text,1,1)=' ' THEN '.'||SUBSTR(p_text,2) ELSE p_text END);
   END;
   ---
   -- Spool to file and remember last spool file
   ---
   PROCEDURE spool (
      p_cmd_id IN dbm_streams.cmd_id%TYPE
     ,p_type IN dbm_streams.type%TYPE -- IN[23], OUT, ERR
   ...
   IS
   BEGIN
      output_line(p_cmd_id, p_type, 'define spool="'||p_text||'"');
      output_line(p_cmd_id, p_type, 'spool '||p_text);
      IF LOWER(TRIM(p_text)) = 'off' THEN
         IF dbm_utility_var.gt_spool.COUNT>0 THEN
            -- pop from stack
            dbm_utility_var.gt_spool.DELETE(dbm_utility_var.gt_spool.COUNT);
            IF dbm_utility_var.gt_spool.COUNT>0 THEN
               -- restore last spool
               output_line(p_cmd_id, p_type, 'define spool="'||dbm_utility_var.gt_spool(dbm_utility_var.gt_spool.COUNT)||'"');
               output_line(p_cmd_id, p_type, 'spool '||dbm_utility_var.gt_spool(dbm_utility_var.gt_spool.COUNT)||' append');
            END IF;
         END IF;
      ELSIF p_text IS NOT NULL THEN
         -- push pool to stack
         dbm_utility_var.gt_spool(dbm_utility_var.gt_spool.COUNT+1) := p_text;
      END IF;
   END;
   ---
   -- Output file prompts
   ---
   PROCEDURE output_file_prompts (
      p_cmd_id IN dbm_streams.cmd_id%TYPE
    , p_type IN dbm_streams.type%TYPE -- IN[23], OUT, ERR
    , p_prompts IN dbm_files.prompts%TYPE
   )
   IS
      l_prompts dbm_files.prompts%TYPE := p_prompts;
      l_pos PLS_INTEGER;
   BEGIN
      WHILE TRUE LOOP
         l_pos := NVL(INSTR(l_prompts, CHR(10)),0);
         EXIT WHEN l_pos = 0;
         output_line(p_cmd_id, p_type, 'prompt '||SUBSTR(l_prompts, 1, l_pos-1));
         l_prompts := SUBSTR(l_prompts, l_pos+1);
      END LOOP;
      IF l_prompts IS NOT NULL THEN
         output_line(p_cmd_id, p_type, 'prompt '||l_prompts);
      END IF;
   END;
   ---
   ...
   BEGIN
      UPDATE dbm_versions
         SET ver_status = CASE WHEN p_ver_status = '~' THEN ver_status ELSE p_ver_status END
           ...
           , exportable = CASE WHEN p_exportable = '~' THEN exportable ELSE p_exportable END
           , importable = CASE WHEN p_importable = '~' THEN importable ELSE p_importable END
           , deleted_flag = CASE WHEN p_deleted_flag = '~' THEN deleted_flag ELSE p_deleted_flag END
       WHERE app_code = p_app_code
      ...
      IF SQL%NOTFOUND THEN
         ...
         INSERT INTO dbm_versions (
          ...
          , concealable, exportable, importable
          , deleted_flag
         ) VALUES (
          ...
          , CASE WHEN p_exportable = '~' THEN NULL ELSE p_exportable END
          , CASE WHEN p_importable = '~' THEN NULL ELSE p_importable END
          , CASE WHEN p_deleted_flag = '~' THEN NULL ELSE p_deleted_flag END
         );
      END IF;
      IF p_commit = 'Y' THEN
   ...
   BEGIN
      UPDATE dbm_files
         SET type = pr_fil.type
           , seq = pr_fil.seq
           , trusted_hash = CASE WHEN pr_fil.trusted_hash = 'NULL' THEN NULL ELSE NVL(pr_fil.trusted_hash, trusted_hash) END
           , trusted_status = NVL(pr_fil.trusted_status,trusted_status)
           , runtime_hash = CASE WHEN pr_fil.runtime_hash = 'NULL' THEN NULL ELSE NVL(pr_fil.runtime_hash, runtime_hash) END
           , runtime_status = NVL(pr_fil.runtime_status,runtime_status)
           , current_hash = CASE WHEN pr_fil.current_hash = 'NULL' THEN NULL ELSE NVL(pr_fil.current_hash, current_hash) END
       ...
       WHERE path = REPLACE(pr_fil.path,'\','/')
      ;
      IF SQL%NOTFOUND THEN
   ...
   BEGIN
      UPDATE dbm_files
         SET run_condition = CASE WHEN p_run_condition = '~' THEN run_condition ELSE p_run_condition END
           ...
           , trusted_hash = CASE WHEN p_trusted_hash = '~' THEN trusted_hash ELSE p_trusted_hash END
           , trusted_status = CASE WHEN p_trusted_status = '~' THEN trusted_status ELSE p_trusted_status END
           , runtime_hash = CASE WHEN p_runtime_hash = '~' THEN runtime_hash ELSE p_runtime_hash END
           , runtime_status = CASE WHEN p_runtime_status = '~' THEN runtime_status ELSE p_runtime_status END
           , current_hash = CASE WHEN p_current_hash = '~' THEN current_hash ELSE p_current_hash END
           ...
           , stmt_id = CASE WHEN p_stmt_id < 0 THEN stmt_id ELSE p_stmt_id END
           , prompts = CASE WHEN p_prompts = '~' THEN prompts ELSE p_prompts END
           , deleted_flag = CASE WHEN p_deleted_flag = '~' THEN deleted_flag ELSE p_deleted_flag END
       WHERE path = REPLACE(p_path,'\','/')
      ;
      IF SQL%NOTFOUND THEN
         INSERT INTO dbm_files (
            app_code, ver_code, path
          , type, run_condition, seq
          , trusted_hash, trusted_status
          , runtime_hash, runtime_status
          , current_hash, run_status
          , run_date, stmt_id, prompts
          , deleted_flag
         ) VALUES (
            p_app_code, p_ver_code, REPLACE(p_path,'\','/'), p_type
          , CASE WHEN p_run_condition = '~' THEN NULL ELSE p_run_condition END
          , CASE WHEN p_seq = 999999999 THEN NULL ELSE p_seq END
          , CASE WHEN p_trusted_hash = '~' THEN NULL ELSE p_trusted_hash END
          , CASE WHEN p_trusted_status = '~' THEN NULL ELSE p_trusted_status END
          , CASE WHEN p_runtime_hash = '~' THEN NULL ELSE p_runtime_hash END
          , CASE WHEN p_runtime_status = '~' THEN NULL ELSE p_runtime_status END
          , CASE WHEN p_current_hash = '~' THEN NULL ELSE p_current_hash END
          ...
          , CASE WHEN p_stmt_id < 0 THEN NULL ELSE p_seq END
          , CASE WHEN p_prompts = '~' THEN NULL ELSE p_prompts END
          , CASE WHEN p_deleted_flag = '~' THEN NULL ELSE p_deleted_flag END
         );
      END IF;
      IF p_commit = 'Y' THEN
...
--#end public
   ...
   PROCEDURE upsert_prv (
    ...
    , p_usage dbm_privileges.name%TYPE
...
--#end public
   ...
   BEGIN
      UPDATE dbm_files
         SET trusted_hash = DECODE(p_trusted_hash,'~',trusted_hash,p_trusted_hash)
           , trusted_status = DECODE(p_trusted_status,'~',trusted_status,p_trusted_status)
           , runtime_hash = DECODE(p_runtime_hash,'~',runtime_hash,p_runtime_hash)
           , runtime_status = DECODE(p_runtime_status,'~',runtime_status,p_runtime_status)
           , current_hash = DECODE(p_current_hash,'~',current_hash,p_current_hash)
...
--#end public
   IS
      ...
      IS
         SELECT fil.*
              , CASE WHEN p_type = 'RUNTIME' THEN fil.runtime_status
                     WHEN p_type = 'TRUSTED' THEN fil.trusted_status
                 END selected_status
           FROM dbm_files fil
          ...
          WHERE (NVL(p_app_code,'all')='all' OR fil.app_code = p_app_code)
            ...
            AND NOT path LIKE '%files.dbm'
            AND (NOT p_type = 'RUNTIME' OR (fil.type IN ('INSTALL','UPGRADE') AND fil.run_status = 'SUCCESS' AND fil.runtime_hash IS NOT NULL))
            AND (NOT p_type = 'TRUSTED' OR fil.trusted_hash IS NOT NULL)
            AND (p_all_status = 'Y'
             OR (NOT p_type = 'RUNTIME' AND fil.runtime_status != 'VALID')
             OR (NOT p_type = 'TRUSTED' AND fil.trusted_status != 'VALID')
                )
         ORDER BY app.seq, ver.ver_nbr, fil.seq
      ;
      l_file_count PLS_INTEGER := 0;
      l_hash_descr VARCHAR2(30);
   BEGIN
      l_hash_descr := CASE WHEN p_type = 'RUNTIME' THEN 'hashes at execution time' ELSE 'trusted hashes' END;
      dbms_output.put_line('Comparing '||l_hash_descr||' with current hashes...');
      FOR r_fil IN c_fil(p_verbose, p_type) LOOP
         IF r_fil.selected_status != 'VALID' THEN
            l_file_count := l_file_count + 1;
         END IF;
         IF r_fil.selected_status != 'VALID' OR p_verbose = 'Y'THEN
            dbms_output.put_line('File "'||r_fil.path||'"'
               ||CASE WHEN p_type = 'RUNTIME' THEN ' executed on '||TO_CHAR(r_fil.run_date,'DD/MM/YYYY')||' at '||TO_CHAR(r_fil.run_date,'HH24:MI:SS') END
               ||' is '
               ||color(r_fil.selected_status, CASE r_fil.selected_status WHEN 'VALID' THEN 'green' WHEN 'MISSING' THEN 'yellow' WHEN 'TAMPERED' THEN 'red' ELSE NULL END)
            );
         END IF;
      END LOOP;
      l_hash_descr := CASE WHEN p_type = 'RUNTIME' THEN ' executed' ELSE ' distributed' END;
      IF p_type = 'RUNTIME' THEN
         IF l_file_count = 1 THEN
            dbms_output.put_line('1 file has been changed or deleted after its execution!');
         ELSE
            dbms_output.put_line(CASE WHEN l_file_count=0 THEN 'No' ELSE TO_CHAR(l_file_count) END||' files have been changed or deleted after their execution!');
         END IF;
      ELSE
         IF l_file_count = 1 THEN
            dbms_output.put_line('1 distributed file is tampered or missing!');
         ELSE
            dbms_output.put_line(CASE WHEN l_file_count=0 THEN 'No' ELSE TO_CHAR(l_file_count) END||' distributed files are tampered or missing!');
         END IF;
      END IF;
   END;
--#begin public
...
--#end public
   ...
   IS
   BEGIN
      RETURN rtrim_slashes(get_par_value('dbm_utility', 'all', 'apps_dir','apps')) || CASE WHEN p_slash THEN get_slash END;
   END;
   ---
   ...
   IS
   BEGIN
      RETURN rtrim_slashes(get_par_value('dbm_utility', 'all', 'logs_dir','logs')) || CASE WHEN p_slash THEN get_slash END;
   END;
   ---
   ...
   IS
   BEGIN
      RETURN rtrim_slashes(get_par_value('dbm_utility', 'all', 'tmp_dir','tmp')) || CASE WHEN p_slash THEN get_slash END;
   END;
   ---
   ...
   IS
   BEGIN
      RETURN rtrim_slashes(get_par_value('dbm_utility', 'all', 'conf_path','conf\dbm_utility.conf')) || CASE WHEN p_slash THEN get_slash END;
   END;
   ---
   ...
   IS
      l_pos PLS_INTEGER;
   BEGIN
      IF dbm_utility_var.ga_cmd.EXISTS(p_command) THEN
         l_pos := dbm_utility_var.ga_cmd(p_command);
         RETURN SUBSTR(p_command,1,l_pos)
             || '['
             || SUBSTR(p_command,l_pos+1)
             || ']'
         ;
      END IF;
      RETURN p_command;
      ...
      IS
         SELECT DISTINCT SUBSTR(path,1,INSTR(REPLACE(path,'/','\'),'\',-1)-1) dir
          ...
          WHERE (NVL(p_app_code,'all')='all' OR app_code = p_app_code)
            AND (p_ver_code IS NULL OR ver_code = p_ver_code)
            AND NOT path LIKE '%files.dbm'
            AND ((NVL(p_hash_type,'RUNTIME') = 'RUNTIME' AND type IN ('INSTALL','UPGRADE') AND run_status = 'SUCCESS' AND runtime_hash IS NOT NULL)
              OR (NVL(p_hash_type,'TRUSTED') = 'TRUSTED' AND trusted_hash IS NOT NULL))
          ORDER BY 1
      ;
      l_apps_dir dbm_utility_var.g_par_value_type := get_apps_dir(TRUE);
      r_option dbm_utility_var.gr_option_type;
      l_verbose BOOLEAN := FALSE;
      l_distributed BOOLEAN := TRUE;
      l_executed BOOLEAN := TRUE;
      l_hash_type VARCHAR2(10 CHAR);
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         r_option := pr_cmd.t_options(i);
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all] [<version>]');
            prompt(pr_cmd.cmd_id,'IN','Check integrity of distributed files and/or executed files using hashes.');
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<version> is an application version (if a single app)');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -dist[ributed]: check distributed files only');
            prompt(pr_cmd.cmd_id,'IN','   -exec[uted]: check executed files only');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            prompt(pr_cmd.cmd_id,'IN','   -v[erbose]: display status of all files');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'verbose') THEN
            l_verbose := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'exec)cuted') THEN
            l_distributed := FALSE;
         ELSIF is_matching_prefix(r_option.name, 'dist)ributed') THEN
            l_executed := FALSE;
         ELSE
            assert(FALSE, 'Invalid option "-'||r_option.name||'" for "'||pr_cmd.command||'" command!'||CHR(10)||'Hint: use "-h[elp]" option to get list of possible options');
         END IF;
      END LOOP;
      IF NOT (l_distributed OR l_executed) THEN
         l_distributed := TRUE;
         l_executed := TRUE;
      END IF;
      IF l_executed THEN
         UPDATE dbm_files
            SET runtime_status = 'MISSING'
          WHERE (NVL(p_app_code,'all')='all' OR app_code = p_app_code)
            AND (p_ver_code IS NULL OR ver_code = p_ver_code)
            AND type IN ('INSTALL','UPGRADE') AND run_status = 'SUCCESS' AND runtime_hash IS NOT NULL
         ;
      END IF;
      IF l_distributed THEN
         UPDATE dbm_files
            SET trusted_status = 'MISSING'
          WHERE (NVL(p_app_code,'all')='all' OR app_code = p_app_code)
            AND (p_ver_code IS NULL OR ver_code = p_ver_code)
            AND trusted_hash IS NOT NULL
         ;
      END IF;
      l_hash_type := CASE WHEN l_distributed AND l_executed THEN NULL WHEN l_distributed THEN 'TRUSTED' WHEN l_executed THEN 'RUNTIME' END;
      output_line(pr_cmd.cmd_id, 'IN', 'prompt Computing file haches...');
      FOR r_fil IN c_fil(l_hash_type) LOOP
         IF l_verbose THEN
            output_line(pr_cmd.cmd_id, 'IN', 'prompt Computing hashes of files in '||adapt_slashes(l_apps_dir||r_fil.dir)||'...');
         END IF;
         output_line(pr_cmd.cmd_id, 'IN', 'host bin\get-hashes '||l_apps_dir||r_fil.dir||' ' ||pr_cmd.cmd_id||' chk-hashes '||rtrim_slashes(l_apps_dir)||' >'||get_tmp_dir(TRUE)||'~get-hashes.sql');
         output_line(pr_cmd.cmd_id, 'IN', '@@'||get_tmp_dir(TRUE)||'~get-hashes.sql');
      END LOOP;
      IF l_distributed THEN
         output_line(pr_cmd.cmd_id, 'IN', 'prompt Checking integrity of distributed files...');
         output_line(pr_cmd.cmd_id, 'IN', REPLACE('exec dbm_utility_krn.report_on_files_integrity(p_cmd_id=>'||pr_cmd.cmd_id||', p_app_code=>"'||p_app_code||'", p_ver_code=>"'||p_ver_code||'",p_type=>"TRUSTED" , p_verbose=>"'||CASE WHEN l_verbose THEN 'Y' ELSE 'N' END||'")','"',''''));
      END IF;
      IF l_executed THEN
         output_line(pr_cmd.cmd_id, 'IN', 'prompt Checking integrity of already executed files...');
         output_line(pr_cmd.cmd_id, 'IN', REPLACE('exec dbm_utility_krn.report_on_files_integrity(p_cmd_id=>'||pr_cmd.cmd_id||', p_app_code=>"'||p_app_code||'", p_ver_code=>"'||p_ver_code||'",p_type=>"RUNTIME" , p_verbose=>"'||CASE WHEN l_verbose THEN 'Y' ELSE 'N' END||'")','"',''''));
      END IF;
      COMMIT;
   END;
   ---
...
--    , p_app_code IN dbm_applications.app_code%TYPE
   ...
   --      output_line(p_cmd_id, 'IN', 'exec dbm_utility_krn.parse_configuration('||p_cmd_id||')');
         dbm_utility_var.g_color := LOWER(get_par_value('dbm_utility', 0, 'color', 'no')) IN ('true','yes','y');
      END IF;
   ...
   END;
   ---
   -- Check whether help option has been specified.
   -- Exception raised if any other option specified.
   --- 
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>]'||p_usage);
            prompt(pr_cmd.cmd_id,'IN',p_descr);
            IF INSTR(p_usage,'<apps-list>')>0 THEN
               prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            END IF;
            IF INSTR(p_usage,'<version>')>0 THEN
               prompt(pr_cmd.cmd_id,'IN','<version> is an application version (if a single app)');
            END IF;
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
   ...
   IS
      ...
      IS
         SELECT DISTINCT SUBSTR(path,1,GREATEST(INSTR(path, '/', -1), INSTR(path, '\', -1))-1) path
           FROM dbm_files
          WHERE app_code = p_app_code
            AND ver_code = p_ver_code
          ORDER BY 1
      ...
      l_found BOOLEAN;
      l_apps_dir dbm_utility_var.g_par_value_type := get_apps_dir(FALSE);
      r_option dbm_utility_var.gr_option_type;
      l_files BOOLEAN := TRUE;
      l_db_objects BOOLEAN := TRUE;
      l_count PLS_INTEGER;
      l_last_app_code dbm_applications.app_code%TYPE;
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         r_option := pr_cmd.t_options(i);
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] <app> [<ver>]');
            prompt(pr_cmd.cmd_id,'IN','Make inventory of database objects and files with checksums/hashes.');
            prompt(pr_cmd.cmd_id,'IN','<app> is the application code or abbreviation or "all"');
            prompt(pr_cmd.cmd_id,'IN','<ver> is the application version (default: latest)');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -db[-objects]: make db objects inventory only');
            prompt(pr_cmd.cmd_id,'IN','   -file[s]: make files inventory only');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'file)s') THEN
            l_db_objects := FALSE;
         ELSIF is_matching_prefix(r_option.name, 'db)-objects') THEN
            l_files := FALSE;
         ELSE
            assert(FALSE, 'Invalid option "-'||r_option.name||'" for "'||pr_cmd.command||'" command!'||CHR(10)||'Hint: use "-h[elp]" option to get list of possible options');
         END IF;
      END LOOP;
      IF NOT l_db_objects AND NOT l_files THEN
        l_db_objects := TRUE;
        l_files := TRUE;
      END IF;
      <<app_loop>>
      FOR r_app IN c_app LOOP
         ...
         l_count := l_count + 1;
         l_last_app_code := r_app.app_code;
         -- Make db objects inventory
         IF l_db_objects THEN
            ...
            output_line(pr_cmd.cmd_id, 'IN', 'prompt Making DB objects inventory for application "'||r_app.app_code||'" version "'||r_ver.ver_code||'"...');
            output_line(pr_cmd.cmd_id, 'IN', 'set termout off');
            spool(pr_cmd.cmd_id, 'IN', l_fil_path);
         ...
         END IF;
         IF l_files THEN
            -- Compute trusted hash of all files in all directories
            output_line(pr_cmd.cmd_id, 'IN', 'prompt Making files inventory for application "'||r_app.app_code||'" version "'||r_ver.ver_code||'"...');
            FOR r_fil IN c_fil(r_ver.app_code, r_ver.ver_code) LOOP
               -- Compute and set trusted hash of migration files
               output_line(pr_cmd.cmd_id, 'IN', 'host bin\get-hashes '||l_apps_dir||get_slash||r_fil.path||' ' ||pr_cmd.cmd_id||' set-trusted-hashes '||rtrim_slashes(l_apps_dir)||' >'||get_tmp_dir(TRUE)||'~get-hashes.sql');
               output_line(pr_cmd.cmd_id, 'IN', '@@'||get_tmp_dir(TRUE)||'~get-hashes.sql');
               output_line(pr_cmd.cmd_id, 'IN', 'set termout off');
               spool(pr_cmd.cmd_id, 'IN', l_apps_dir||get_slash||r_fil.path||get_slash||'files.dbm');
               l_sql := REPLACE(
'SELECT SUBSTR(path,GREATEST(INSTR(path, "/", -1), INSTR(path, "\", -1))+1) || ":"
    || CASE WHEN trusted_hash IS NOT NULL THEN " "||trusted_hash END
    || CASE WHEN run_condition IS NOT NULL THEN " "||run_condition END
  FROM dbm_files
 WHERE app_code = "'||r_ver.app_code||'"
   AND ver_code = "'||r_ver.ver_code||'"
   AND (trusted_hash IS NOT NULL OR run_condition IS NOT NULL)
   AND path LIKE "'||r_fil.path||'%"
   AND path NOT LIKE "%files.dbm"
 ORDER BY type, seq, path;','"','''');
               output_line(pr_cmd.cmd_id, 'IN', l_sql);
               spool(pr_cmd.cmd_id, 'IN', 'off');
               output_line(pr_cmd.cmd_id, 'IN', 'set termout on');
            END LOOP;
         END IF;
      END LOOP app_loop;
      -- Reload config into cache and save it
      read_config(p_cmd_id=>pr_cmd.cmd_id, p_apps_config=>FALSE, p_inventories=>TRUE);
      output_line(pr_cmd.cmd_id, 'IN', REPLACE('exec dbm_utility_krn.save_cache("'||CASE WHEN l_count > 1 THEN 'all' ELSE l_last_app_code END||'")','"',''''));
   END;
   ---
   ...
   BEGIN
      IF NOT p_guess THEN
         IF help_option(pr_cmd, ' <app> <ver>', 'Set the current version of an application.') THEN
            RETURN;
         END IF;
      END IF;
      ...
      END IF;
      l_ver_nbr := CASE WHEN p_ver_code = '0' THEN 0 ELSE get_version_nbr(p_ver_code) END;
      UPDATE dbm_files
         SET run_status = NULL
           , run_date = NULL
           , stmt_id = NULL
   ...
   IS
      ...
      r_option dbm_utility_var.gr_option_type;
   BEGIN
      IF NOT p_from_other_op AND help_option(pr_cmd, ' [<apps-list> | all]', 'Check the validity of given applications using checksums.') THEN
         RETURN;
      END IF;
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         END IF;
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.upsert_app(p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'",p_ver_status=>"INVALID",p_commit=>"Y")','"',''''));
         ...
         FOR k IN 1..3 LOOP -- 1:app/ver, 2:app/0, 3:all/0
            ...
            FOR r_fil IN c_fil(l_app_code, l_ver_code, r_ver.last_op_type) LOOP
               ...
               ELSE
                  ...
                  output_file_prompts(pr_cmd.cmd_id, 'IN', r_fil.prompts);
            ...
            NULL;
         END LOOP;
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.upsert_app(p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'",p_ver_status=>"VALID",p_commit=>"Y")','"',''''));
         IF NOT p_from_other_op THEN
   ...
   BEGIN
      IF help_option(pr_cmd, ' [<apps-list> | all] [<grantee>]'
                   ,CASE pr_cmd.command WHEN 'expose' THEN 'Expose an application using private or public grants and synonyms.'
                                        WHEN 'conceal' THEN 'Conceal an application by revoking grants and dropping synonyms.'
                     END
                   ||CHR(10)||'PROMPT <grantee> can be a private schema or "public" (default)'
                    )
      THEN
         RETURN;
      END IF;
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         assert(l_found, 'Application "'||r_app.app_code||'" is not installed!');
         assert(NVL(r_ver.deleted_flag,'N') = 'N','Version "'||r_ver.ver_code||'" of application "'||r_ver.app_code||'" no more exists on file system!');
         l_label := INITCAP(RTRIM(pr_cmd.command,'e')||'ing');
   ...
   END;
   ---
   -- Export or import application configuration data
   ---
   ...
   IS
      ...
      r_option dbm_utility_var.gr_option_type;
      l_timestamp VARCHAR2(12);
      k_ts_format CONSTANT VARCHAR2(14) := 'YYMMDDHH24MISS';
      k_dt_format CONSTANT VARCHAR2(6) := 'YYMMDD';
      l_format VARCHAR2(14);
      l_verbose BOOLEAN := FALSE;
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all]');
            prompt(pr_cmd.cmd_id,'IN',INITCAP(pr_cmd.command)||' configuration of given applications.');
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            prompt(pr_cmd.cmd_id,'IN','   -date[=<YYMMDD>]: add date to file name (default: today)');
            prompt(pr_cmd.cmd_id,'IN','   -path=<path>: path of default directory');
            prompt(pr_cmd.cmd_id,'IN','   -time[=<YYMMDDHHMMSS>]: add date/time to file name (default: now)');
            prompt(pr_cmd.cmd_id,'IN','   -verb[ose]: show information messages');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'verb)ose') THEN
            l_verbose := TRUE;
         ELSIF r_option.name = 'path' THEN
            l_path := r_option.value;
            assert(l_path IS NOT NULL,'Missing value for option "'||r_option.name||'"');
         ELSIF r_option.name IN ('date','time') THEN
            l_format := CASE WHEN is_prefix(r_option.name, 'date', 4) THEN k_dt_format ELSE k_ts_format END;
            IF r_option.value IS NULL THEN
               l_timestamp := TO_CHAR(SYSDATE,l_format);
            ELSE
               BEGIN
                  l_timestamp := TO_CHAR(TO_DATE(r_option.value,l_format),l_format);
               EXCEPTION
                  WHEN OTHERS THEN
                     assert(FALSE, 'Invalid value "'||r_option.value||'" for option "'||r_option.name||'" (expected format is "'||REPLACE(l_format,'HH24MI','HHMM')||'")');
               END;
            END IF;
         ELSE
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         r_ver.last_op_type := UPPER(pr_cmd.command);
         define_variables(pr_cmd.cmd_id, r_ver, 'IN');
         output_line(pr_cmd.cmd_id,'IN','define file_path="'||rtrim_slashes(NVL(l_path,get_tmp_dir(FALSE)))||get_slash||r_app.app_code||CASE WHEN l_timestamp IS NOT NULL THEN '_'||l_timestamp END||'"');
         output_line(pr_cmd.cmd_id,'IN','define message_filter=EW"'||CASE WHEN l_verbose THEN 'I' END||'"');
         output_exit_on_error(pr_cmd.cmd_id, 'IN');
         output_line(pr_cmd.cmd_id,'IN','prompt '||INITCAP(pr_cmd.command)||'ing data of application "'||r_ver.app_code||'" version "'||r_ver.ver_code||'"...');
         output_exit_on_error(pr_cmd.cmd_id, 'IN');
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_ver(p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'",p_last_op_type=>"'||UPPER(pr_cmd.command)||'", p_last_op_status=>"ONGOING")','"',''''));
         output_line(pr_cmd.cmd_id,'IN','define home_dir="'||get_apps_dir(TRUE)||r_app.home_dir||'"');
         output_line(pr_cmd.cmd_id,'IN','define ver_code="'||r_ver.ver_code||'"');
         ...
         FOR k IN 1..3 LOOP -- 1:app/ver, 2:app/0, 3:all/0
            ...
            NULL;
         END LOOP;
         l_tot_count := l_tot_count + l_files_count;
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_ver(p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'",p_last_op_status=>"SUCCESS")','"',''''));
         output_line(pr_cmd.cmd_id,'IN','set termout on');
         IF l_files_count = 0 THEN
            output_line(pr_cmd.cmd_id, 'IN', 'prompt No script was found to '||pr_cmd.command||' data for application "'||r_ver.app_code||'" version "'||r_ver.ver_code||'"');
         ELSE
            output_line(pr_cmd.cmd_id,'IN','prompt Data '||pr_cmd.command||' for application "'||r_ver.app_code||'" version "'||r_ver.ver_code||'" was successful');
         END IF;
         <<next_app>>
         NULL;
      END LOOP app_loop;
      IF l_tot_count = 0 AND NVL(p_app_code,'all')='all' THEN
         output_line(pr_cmd.cmd_id,'IN','prompt No data '||pr_cmd.command||' was performed.');
      END IF;
   END;
   ---
   ...
   IS
      ...
      l_found BOOLEAN;
   BEGIN
      IF help_option(pr_cmd, ' [<apps-list> | all]', 'Perform the setup or configuration of given applications.') THEN
         RETURN;
      END IF;
      <<app_loop>>
      FOR r_app IN c_app(p_app_code) LOOP
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all]');
            prompt(pr_cmd.cmd_id,'IN','Rollback failed migration of given applications.');
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            prompt(pr_cmd.cmd_id,'IN','   -noshow[-errors]: do not show compilation errors');
            prompt(pr_cmd.cmd_id,'IN','   -nostop[-on-warning]: do not stop on warning');
            prompt(pr_cmd.cmd_id,'IN','   -show[-errors]: show compilation errors');
            prompt(pr_cmd.cmd_id,'IN','   -stop[-on-warning]: stop on warning');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'show)-errors')
         OR is_matching_prefix(r_option.name, 'noshow)-errors')
         OR is_matching_prefix(r_option.name, 'stop)-on-warning')
         OR is_matching_prefix(r_option.name, 'nostop)-on-warning')
         THEN
            NULL; -- processed later
         ELSE
            assert(FALSE, 'Invalid option "-'||r_option.name||'" for "'||pr_cmd.command||'" command!'||CHR(10)||'Hint: use "-h[elp]" option to get list of possible options');
         END IF;
      END LOOP;
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         FOR r_fil IN c_fil(r_app.app_code) LOOP
            ...
            IF l_file_count = 1 THEN
               ...
               UPDATE dbm_files SET runtime_status = 'MISSING' WHERE app_code = r_fil.app_code AND ver_code = r_fil.ver_code AND type = l_fil_type;
            END IF;
...
--#end public
   ...
   PROCEDURE check_privileges (
    ...
    , p_app_code dbm_privileges.app_code%TYPE
    , p_ver_code dbm_privileges.ver_code%TYPE
    , p_usage dbm_privileges.usage%TYPE
   ...
   IS
      ...
      CURSOR c_prv (
       ...
       , p_ver_code dbm_privileges.ver_code%TYPE
       , p_usage dbm_privileges.usage%TYPE
       , p_type dbm_privileges.type%TYPE
      )
      IS
         SELECT *
           FROM dbm_privileges
      ...
      IS
        ...
        CONNECT BY REGEXP_SUBSTR(p_privs,'[^,]+', 1, level) IS NOT NULL
         ...
         SELECT rsp.privilege || ' WITH ADMIN OPTION' AS privilege
          ...
          INNER JOIN dba_role_privs urp
             ON grantee IN (USER, 'PUBLIC')
      ...
      IS
      BEGIN
         IF pr_cmd.cmd_id IS NOT NULL THEN
            IF NOT l_silent THEN
               output_line(pr_cmd.cmd_id,'IN','Prompt '||p_text);
            END IF;
         ELSE
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_prefix(r_option.name, 'verbose', 3) THEN
            l_verbose := TRUE;
         ELSIF is_prefix(r_option.name, 'silent', 3) THEN
            l_silent := TRUE;
         END IF;
      END LOOP;
      FOR k IN 1..3 LOOP
         ...
         WHILE l_prv_idx IS NOT NULL LOOP
            ...
            THEN
               IF l_verbose THEN
                  print_line('Info: '||l_prv_desc||' privilege "'||r_prv.text||'" is well granted through "'||la_nan_priv(l_text)||'"');
               END IF;
            ELSE
         ...
         WHILE l_text IS NOT NULL LOOP
            IF la_priv_res(l_text) THEN
               print_line('ERROR: '||l_prv_desc||' privilege "'||l_text||'" is missing!', l_text);
            END IF;
         ...
         IF l_verbose THEN
            ...
            WHILE l_text IS NOT NULL LOOP
               IF NOT la_priv_res(l_text) THEN
                  print_line('Info: '||l_prv_desc||' privilege "'||l_text||'" is well granted');
               END IF;
               l_text := la_priv_res.NEXT(l_text);
            END LOOP;
         END IF;
...
--#end public
   IS
      ...
      r_ver c_ver%ROWTYPE;
      l_found BOOLEAN;
      r_cmd dbm_utility_var.gr_command_type;
      r_option dbm_utility_var.gr_option_type;
   BEGIN
      -- Set options
      IF p_options IN ('silent','verbose') THEN
         r_option.name := p_options;
         r_cmd.t_options(r_cmd.t_options.COUNT+1) := r_option;
      END IF;
      -- Check runtime privileges applicable to all applications
      check_privileges(r_cmd, 'all', 'all', 'OPERATE');
      <<app_loop>>
      FOR r_app IN c_app(p_app_code) LOOP
         -- Check runtime privileges applicable to all versions of application
         check_privileges(r_cmd, r_app.app_code, 'all', 'OPERATE');
         -- Get current version
         OPEN c_ver(r_app.app_code);
         FETCH c_ver INTO r_ver;
         l_found := c_ver%FOUND;
         CLOSE c_ver;
         -- Check runtime privileges applicable to current version of application
         IF l_found THEN
            check_privileges(r_cmd, r_app.app_code, r_ver.ver_code, 'OPERATE');
         END IF;
      END LOOP;
   END;
   ---
   ...
   IS
      ...
      l_file_path dbm_files.path%TYPE;
      l_file_extension VARCHAR2(4);
      l_jump_stmt_id dbm_files.stmt_id%TYPE;
      l_skip_file dbm_files.path%TYPE;
      l_cond_stmt_exec BOOLEAN;
      l_reexecute BOOLEAN := FALSE;
      l_hash BOOLEAN := TRUE;
   BEGIN
      l_cond_stmt_exec := LOWER(get_par_value('dbm_utility', 0, 'conditional_statement_exec', 'yes')) IN ('true','yes','y');
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         r_option := pr_cmd.t_options(i);
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all]');
            IF pr_cmd.command = 'precheck' THEN
               prompt(pr_cmd.cmd_id,'IN','Check installation pre-requisites (without performing any migration).');
            ELSIF pr_cmd.command = 'install' THEN
               prompt(pr_cmd.cmd_id,'IN','Install applications that are not installed yet (do not upgrade already installed ones).');
            ELSIF pr_cmd.command = 'upgrade' THEN
               prompt(pr_cmd.cmd_id,'IN','Upgrade applications that are already installed (do not install non-installed ones).');
            ELSE 
               prompt(pr_cmd.cmd_id,'IN','Install non-installed applications and upgrade installed ones.');
            END IF;
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            IF pr_cmd.command != 'precheck' THEN
               prompt(pr_cmd.cmd_id,'IN','   -jump[-to-statement] <id>: jump to statement id');
               prompt(pr_cmd.cmd_id,'IN','   -nocond[-stmt-exec]: disable conditional statement execution');
               prompt(pr_cmd.cmd_id,'IN','   -nodep[endencies]: do no check application dependencies');
               prompt(pr_cmd.cmd_id,'IN','   -nohash[-check]: do no check file hashes (i.e., ignore tampered files)');
               prompt(pr_cmd.cmd_id,'IN','   -noshow[-errors]: do not show compilation errors');
               prompt(pr_cmd.cmd_id,'IN','   -nopriv[ileges]: do no check required privileges');
               prompt(pr_cmd.cmd_id,'IN','   -nostop[-on-error]: do not stop on error');
               prompt(pr_cmd.cmd_id,'IN','   -nostop[-on-warning]: do not stop on warning');
               IF pr_cmd.command = 'install' THEN
                  prompt(pr_cmd.cmd_id,'IN','   -noup[grade]: do not upgrade after install');
               END IF;
               prompt(pr_cmd.cmd_id,'IN','   -noval[idate]: do not validate after '||pr_cmd.command);
               prompt(pr_cmd.cmd_id,'IN','   -reexec[ute]: re-execute scripts already successfuly executed');
               prompt(pr_cmd.cmd_id,'IN','   -show[-errors]: show compilation errors');
               prompt(pr_cmd.cmd_id,'IN','   -sim[ulate]: simulate i.e. do not execute '||pr_cmd.command);
               prompt(pr_cmd.cmd_id,'IN','   -skip[-file] <file>: skip specified file');
               prompt(pr_cmd.cmd_id,'IN','   -stop[-on-warning]: stop on warning');
            END IF;
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'nohash)-check') THEN
            l_hash := FALSE;
         ELSIF is_matching_prefix(r_option.name, 'sim)ulate') THEN
            l_simu := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'noval)idate') THEN
            l_validate := FALSE;
         ELSIF is_matching_prefix(r_option.name, 'noup)grade') THEN
            l_upgrade := FALSE;
         ELSIF is_matching_prefix(r_option.name, 'nopriv)ileges') THEN
            l_nopriv := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'nopre)checks') THEN
            l_noprecheck := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'nodep)endencies') THEN
            l_nodep := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'reexec)ute') THEN
            l_reexecute := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'nostop)-on-error') THEN
            l_nostop := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'jump)-to-statement') THEN
            assert(r_option.value is not null, 'Missing statement id for "jumt-to-statement" option!');
            assert(REGEXP_LIKE(r_option.value,'^[0-9]{1,5}$'), 'Invalid statement id "'||r_option.value||'" for "jump-to-statement" option!');
            l_jump_stmt_id := TO_NUMBER(r_option.value);
         ELSIF is_matching_prefix(r_option.name, 'skip)-file') THEN
            assert(r_option.value is not null, 'Missing file name for "skip-file" option!');
            l_skip_file := r_option.value;
         ELSIF is_matching_prefix(r_option.name, 'nocond)-stmt-exec') THEN
            l_cond_stmt_exec := FALSE;
         ELSIF is_matching_prefix(r_option.name, 'show)-errors')
            OR is_matching_prefix(r_option.name, 'noshow)-errors')
            OR is_matching_prefix(r_option.name, 'stop)-on-warning')
            OR is_matching_prefix(r_option.name, 'nostop)-on-warning')
      ...
      IF NOT l_nopriv AND p_precheck_only THEN
         check_privileges(pr_cmd, 'all', 'all', 'MIGRATE', l_nostop);
...
--         output_line(pr_cmd.cmd_id,'IN','prompt Migrating application "'||r_app.app_code||'"...');
         ...
         IF NOT l_nopriv AND p_precheck_only THEN
            check_privileges(pr_cmd, r_app.app_code, 'all', 'MIGRATE', l_nostop);
         ...
         FOR r_ver IN c_ver(r_app.app_code, pr_ver.ver_status) LOOP
            ...
            IF NOT l_nopriv THEN
               ...
               IF l_check_app_privs THEN
                  ...
                  l_check_app_privs := FALSE;
               END IF;
               check_privileges(pr_cmd, r_app.app_code, r_ver.ver_code, 'MIGRATE', l_nostop);
            END IF;
               ...
               FOR k IN 1..3 LOOP -- 1: app/ver, 2:app/all, 3: all/all
                  ...
                  FOR r_fil IN c_fil(l_app_code, l_ver_code, 'PRECHECK') LOOP
                     IF r_fil.run_condition IS NOT NULL AND NOT file_condition_met(l_app_code, r_ver.ver_nbr, r_fil.path, r_fil.run_condition) THEN
                        ...
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"SKIPPED")','"',''''));
                     ELSIF l_skip_file IN (get_filename(r_fil.path),r_fil.path) THEN
                        output_line(pr_cmd.cmd_id,'IN','prompt Skipping pre-check script "'||get_apps_dir(TRUE)||r_fil.path||'" on request...');
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"SKIPPED")','"',''''));
                     ELSE
                        output_file_prompts(pr_cmd.cmd_id, 'IN', r_fil.prompts);
            ...
            FOR r_fil IN c_fil(r_ver.app_code, r_ver.ver_code, r_ver.next_op_type) LOOP
               ...
               IF l_fil_count = 1 THEN
                  -- Compute and set current hash of migration files
                  output_line(pr_cmd.cmd_id, 'IN', 'host bin\get-hashes '||l_apps_dir||get_dir(r_fil.path)||' ' ||pr_cmd.cmd_id
                     ||' set-runtime-hashes'||CASE WHEN NOT l_hash THEN '-ignore-tampered' END||' '||rtrim_slashes(l_apps_dir)||' >'||get_tmp_dir(TRUE)||'~get-hashes.sql');
                  output_line(pr_cmd.cmd_id, 'IN', '@@'||get_tmp_dir(TRUE)||'~get-hashes.sql');
               END IF;
               IF r_fil.run_status = 'SUCCESS' AND NOT l_reexecute THEN
               ...
               ELSE
                  IF r_fil.run_condition IS NOT NULL AND NOT file_condition_met(r_ver.app_code, r_ver.ver_nbr, r_fil.path, r_fil.run_condition) THEN
                     ...
                     output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"SKIPPED")','"',''''));
                  ELSIF l_skip_file IN (get_filename(r_fil.path),r_fil.path) THEN
                     output_line(pr_cmd.cmd_id,'IN','prompt Skipping script "'||get_apps_dir(TRUE)||r_fil.path||'" on request...');
                     output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"SKIPPED")','"',''''));
                  ELSE
                     l_file_extension := LOWER(SUBSTR(get_file_extension(r_fil.path),1,4));
                     output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"ONGOING")','"',''''));
                     output_file_prompts(pr_cmd.cmd_id, 'IN', r_fil.prompts);
                     IF l_cond_stmt_exec AND l_file_extension = 'sql' THEN
                        -- Create a temporary file (prefixed with ~) as a copy of the sql script to execute + parse it and remove statements already executed successfully
                        l_file_path := get_dir(r_fil.path,TRUE)||'~'||get_filename(r_fil.path); -- temporary file
                        output_line(pr_cmd.cmd_id,'IN','prompt Copying script "'||get_apps_dir(TRUE)||r_fil.path||'"...');
                        output_line(pr_cmd.cmd_id,'IN','host bin\read-file '||get_apps_dir(TRUE)||r_fil.path||' '||pr_cmd.cmd_id||' '||get_tmp_dir||' >'||get_tmp_dir(TRUE)||'~read-file.sql');
                        output_line(pr_cmd.cmd_id,'IN','@@'||get_tmp_dir(TRUE)||'~read-file.sql');
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.parse_script(p_cmd_id=>"'||pr_cmd.cmd_id||'", p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'", p_path=>"'||r_fil.path||'", p_stmt_id=>"'||NVL(l_jump_stmt_id,r_fil.stmt_id)||'")','"',''''));
                        output_line(pr_cmd.cmd_id,'IN','set termout off');
                        spool(pr_cmd.cmd_id,'IN',get_apps_dir(TRUE)||l_file_path);
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('SELECT RTRIM(RTRIM(text,CHR(13))) text FROM dbm_streams WHERE cmd_id='||pr_cmd.cmd_id||' AND type="OUT" AND line >= 2 ORDER BY line;','"',''''));
                        spool(pr_cmd.cmd_id,'IN','off');
                        output_line(pr_cmd.cmd_id,'IN','set termout on');
                        output_line(pr_cmd.cmd_id,'IN','commit;');
                        output_line(pr_cmd.cmd_id,'IN',REPLACE('DELETE dbm_streams WHERE cmd_id = '||pr_cmd.cmd_id||' AND type = "OUT";','"',''''));
                        output_line(pr_cmd.cmd_id,'IN','COMMIT;');
                     ELSE
                        l_file_path := r_fil.path;
                     END IF;
                     output_line(pr_cmd.cmd_id,'IN','prompt Executing script "'||get_apps_dir(TRUE)||l_file_path||'"...');
                     output_line(pr_cmd.cmd_id,'IN','@@'||get_apps_dir(TRUE)||l_file_path);
                     IF l_cond_stmt_exec AND l_file_extension = 'sql' THEN
                        -- Delete temporary file
                        output_line(pr_cmd.cmd_id,'IN','host bin\delete-file '||get_apps_dir(TRUE)||l_file_path);
                        l_jump_stmt_id := NULL; -- no more jump to statement in other sql scripts
                     END IF;
                     IF l_file_extension IN ('pks','pkb','pls','plb','tps','tpb','prc','fnc') THEN
                        show_errors(pr_cmd);
                        stop_on_warning(pr_cmd, r_app.app_code);
                     END IF;
                     output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"SUCCESS", p_stmt_id=>NULL)','"',''''));
                  END IF;
               END IF;
               EXIT WHEN r_fil.seq = 0;
            END LOOP;
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all]');
            prompt(pr_cmd.cmd_id,'IN','Uninstall given applications.');
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -f[orce]: force uninstall');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'f)orce')  THEN
            l_force := TRUE;
         ELSE
            assert(FALSE, 'Invalid option "-'||r_option.name||'" for "'||pr_cmd.command||'" command!'||CHR(10)||'Hint: use "-h[elp]" option to get list of possible options');
         END IF;
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         r_ver.last_op_status := 'ONGOING';
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_ver(p_app_code=>"'||r_ver.app_code||'",p_ver_code=>"'||r_ver.ver_code||'",p_last_op_type=>"'||r_ver.last_op_type||'", p_last_op_status=>"'||r_ver.last_op_status||'")','"',''''));
         output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.upsert_app(p_app_code=>"'||r_ver.app_code||'",p_ver_status=>"UNINSTALLING",p_exposed_flag=>"N")','"',''''));
         define_variables(pr_cmd.cmd_id, r_ver, 'IN');
         l_files_count := 0;
         FOR k IN REVERSE 1..3 LOOP -- 1:app/ver, 2:app/0, 3:all/0
            ...
            FOR r_fil IN c_fil(l_app_code, l_ver_code, r_ver.last_op_type) LOOP
               ...
               IF l_fil_count = 1 THEN
                  -- Compute and set current hash of migration files
                  output_line(pr_cmd.cmd_id, 'IN', 'host bin\get-hashes '||l_apps_dir||get_dir(r_fil.path)||' ' ||pr_cmd.cmd_id||' set-runtime-hashes '||rtrim_slashes(l_apps_dir)||' >'||get_tmp_dir(TRUE)||'~get-hashes.sql');
                  output_line(pr_cmd.cmd_id, 'IN', '@@'||get_tmp_dir(TRUE)||'~get-hashes.sql');
               END IF;
               IF r_fil.run_condition IS NOT NULL AND NOT file_condition_met(r_ver.app_code, r_ver.ver_nbr, r_fil.path, r_fil.run_condition) THEN
                  output_line(pr_cmd.cmd_id,'IN','prompt Skipping script "'||get_apps_dir(TRUE)||r_fil.path||'" (condition not met)...');
         ...
         IF r_ver.app_code != 'dbm_utility' THEN
            ...
            output_line(pr_cmd.cmd_id,'IN',REPLACE('exec UPDATE dbm_files SET runtime_hash = NULL, run_status = NULL, run_date = NULL WHERE app_code="'||r_ver.app_code||'" AND type!="UNINSTALL"','"','''')); -- all versions
         END IF;
         output_line(pr_cmd.cmd_id,'IN','COMMIT;'); -- all versions
   ...
   IS
      ...
      IS
         SELECT app.*
           FROM dbm_all_applications app
          WHERE (NVL(p_app_code,'all')='all' OR app.app_code = p_app_code)
            AND (p_owner IS NULL OR app.owner = p_owner)
          ORDER BY app.owner, app.seq, app.app_code
      ;
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all]');
            prompt(pr_cmd.cmd_id,'IN','Show currently installed version of given applications.');
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -all[-schemas]: show applications of all schemas');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'all)-schemas') THEN
            l_all := TRUE;
         ELSE
            assert(FALSE, 'Invalid option "-'||r_option.name||'" for "'||pr_cmd.command||'" command!'||CHR(10)||'Hint: use "-h[elp]" option to get list of possible options');
         END IF;
      ...
      FOR r_app IN c_app(p_owner=>CASE WHEN l_all THEN NULL ELSE USER END) LOOP
         ...
         l_version := 'has'||CASE WHEN r_app.app_code != 'all' THEN CASE WHEN r_app.next_ver_code IS NULL THEN color(' latest','green') ELSE color(' old','yellow') END END||' version "'||r_app.ver_code||'" installed';
         IF r_app.app_code != 'all' THEN
            l_accessible := ', '|| CASE WHEN NVL(r_app.exposed_flag,'N') = 'N' THEN 'NOT ' END||'exposed to PUBLIC';
            l_status := ', status '||color(NVL(r_app.ver_status,'UNKNOW'), CASE r_app.ver_status WHEN 'VALID' THEN 'green' WHEN 'INVALID' THEN 'red' ELSE 'yellow' END);
            l_next := CASE WHEN r_app.next_ver_code IS NOT NULL THEN ', version "'||r_app.next_ver_code||'" is available for '||LOWER(r_app.next_ver_op_type) END;
         ELSE
            l_accessible := NULL;
            l_status := NULL;
            l_next := NULL;
         ...
         IF r_app.ver_code IS NOT NULL THEN
            dbms_output.put_line('Application "'||r_app.app_code||'" '||l_owned_by||l_version||l_status||l_accessible||l_next);
         ELSE
            l_obj_name_pattern := get_par_value(r_app.app_code, 0, 'object_name_pattern');
            l_obj_name_anti_pattern := get_par_value(r_app.app_code, 0, 'object_name_anti_pattern', '^$');
            ...
            IF l_obj_count = 0 AND r_app.owner = USER THEN
               dbms_output.put_line('Application "'||r_app.app_code||'" is '||color('not installed','cyan')||CASE WHEN l_all THEN ' locally' END||l_next);
            ELSE
            dbms_output.put_line('Application "'||r_app.app_code||'" '||l_owned_by||l_version||l_status||l_accessible||l_next);
            END IF;
         END IF;
      END LOOP;
      IF NOT l_all THEN
         dbms_output.put_line('Hint: use "-all" option to see all schemas');
      END IF;
   END;
   ...
   BEGIN
      ...
      RETURN r_obj;
   END;
   -- Parse privilege line (format: [<usage>] <type> <priv> [WITH GRANT|ADMIN OPTION] [: <condition>])
   ...
   BEGIN
      r_prv.usage := 'MIGRATE';
      IF UPPER(SUBSTR(l_line,1,8)) IN ('MIGRATE ','OPERATE ') THEN
         r_prv.usage := UPPER(SUBSTR(l_line,1,7));
         l_pos := LENGTH(r_prv.usage)+1;
         WHILE SUBSTR(l_line,l_pos,1) = ' ' LOOP
            l_pos := l_pos + 1;
         END LOOP;
         l_line := SUBSTR(l_line, l_pos);
      END IF;
      IF UPPER(SUBSTR(l_line,1,8)) = 'SYSPRIV ' THEN
         r_prv.type := 'SYSPRIV';
      ELSIF UPPER(SUBSTR(l_line,1,8)) IN ('TABPRIV ','OBJPRIV ') THEN
         r_prv.type := 'TABPRIV';
      ELSIF UPPER(SUBSTR(l_line,1,9)) = 'ROLEPRIV ' THEN
         r_prv.type := 'ROLEPRIV';
      ELSE
      ...
      IF l_pos > 0 THEN
         r_prv.condition := TRIM(SUBSTR(l_line, l_pos+1));
         l_line := TRIM(SUBSTR(l_line, 1, l_pos-1));
      END IF;
      l_line := TRIM(SUBSTR(l_line, l_beg));
      r_prv.text := l_line;
      IF UPPER(SUBSTR(l_line,-17)) IN ('WITH GRANT OPTION','WITH ADMIN OPTION') THEN
         r_prv.delegable := 'Y';
         l_line := TRIM(SUBSTR(l_line, 1, LENGTH(l_line)-17));
      END IF;
      IF UPPER(SUBSTR(l_line,1,7)) = 'DIRECT ' THEN
         r_prv.direct_flag := 'Y';
   ...
   BEGIN
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
            prompt(pr_cmd.cmd_id,'IN','========================================');
            prompt(pr_cmd.cmd_id,'IN','Usage: '||get_command_usage(pr_cmd.command)||' [<options-list>] [<apps-list> | all]');
            prompt(pr_cmd.cmd_id,'IN','Guess which version of given applications are installed using checksums.');
            prompt(pr_cmd.cmd_id,'IN','A best guess can be performed in case no exact match is found.');
            prompt(pr_cmd.cmd_id,'IN','Application version can be changed so as to reflect matching one.');
            prompt(pr_cmd.cmd_id,'IN','<apps-list> is a comma-separated list of applications.');
            prompt(pr_cmd.cmd_id,'IN','<options-list> is a space-separated list of options:');
            prompt(pr_cmd.cmd_id,'IN','   -best[-guess]: make a best guess (vs exact match)');
            prompt(pr_cmd.cmd_id,'IN','   -d[ebug]: enable debugging');
            prompt(pr_cmd.cmd_id,'IN','   -h[elp] or -?: this help');
            prompt(pr_cmd.cmd_id,'IN','   -set[-current]: set guessed version as current');
            RETURN;
         ELSIF is_matching_prefix(r_option.name, 'd)ebug') THEN
            l_debug := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'best)-guess') THEN
            l_best := TRUE;
         ELSIF is_matching_prefix(r_option.name, 'set)-current') THEN
            l_set := TRUE;
         ELSE
            assert(FALSE, 'Invalid option "-'||r_option.name||'" for "'||pr_cmd.command||'" command!'||CHR(10)||'Hint: use "-h[elp]" option to get list of possible options');
         END IF;
         ...
         ELSE
            ...
            FOR r_ver IN c_ver(r_app.app_code) LOOP
               ...
               WHILE l_db_obj_name IS NOT NULL AND l_ver_obj_name IS NOT NULL LOOP
                  ...
                  IF (l_public = 'N' AND l_ver_obj_name LIKE 'PUBLIC%')
                  OR (l_ver_obj_name IS NOT NULL AND NOT object_condition_met(r_ver.app_code, r_ver.ver_nbr, l_ver_obj_name, la_ver_obj(l_ver_obj_name).condition))
                  THEN
                  ...
                  ELSIF l_db_obj_name IS NULL OR l_db_obj_name > l_ver_obj_name THEN
                     ...
                     l_ver_obj_name := la_ver_obj.NEXT(l_ver_obj_name);
                  ELSIF l_ver_obj_name IS NULL OR l_ver_obj_name > l_db_obj_name THEN
                     IF l_debug THEN
...
--#end public
   ...
   BEGIN
      ...
      FOR i IN 1..lt_obj.COUNT LOOP
         l_obj_name := UPPER(lt_obj(i).name);
      ...
      FOR r_obj IN dbm_utility_var.gc_obj(p_obj_name_pattern,NVL(p_obj_name_anti_pattern,'^$'),p_public) LOOP
         ...
         IF l_pos > 0 THEN
            l_obj_name := UPPER(SUBSTR(r_obj.object,1,l_pos-1));
            l_checksum := TRIM(SUBSTR(r_obj.object,l_pos+1));
         ELSE
            l_obj_name := UPPER(r_obj.object);
            l_checksum := NULL;
         END IF;
         IF la_obj.EXISTS(l_obj_name) THEN
            IF p_last_op_type = 'UNINSTALL' THEN
               dbms_output.put_line(color('ERROR: '||l_obj_name||' was {NOT DROPPED;red}!'));
               l_err_count := l_err_count + 1;
            ELSE
               IF r_obj.status = 'INVALID' THEN
                  dbms_output.put_line(color('ERROR: '||l_obj_name||' is {INVALID;red}!'));
                  l_err_count := l_err_count + 1;
               END IF;
               IF l_checksum != NVL(la_obj(l_obj_name).checksum,l_checksum) THEN
                  dbms_output.put_line(color('ERROR: '||l_obj_name||' is {TAMPERED;red}! (got checksum "'||l_checksum||'" while expecting "'||la_obj(l_obj_name).checksum||'")'));
                  l_err_count := l_err_count + 1;
               END IF;
            END IF;
            la_obj.DELETE(l_obj_name);
         ELSIF p_last_op_type = 'VALIDATE' AND p_obj_name_pattern IS NOT NULL THEN
            dbms_output.put_line(l_obj_name||color(' is {IN EXCESS;yellow}!'));
            l_err_count := l_err_count + 1;
         END IF;
      END LOOP;
      ...
      IF p_last_op_type = 'VALIDATE' THEN
         ...
         WHILE l_obj_name IS NOT NULL LOOP
            dbms_output.put_line('ERROR: '||l_obj_name||color(' is {MISSING;red}!'));
            l_err_count := l_err_count + 1;
   ...
   IS
      ...
      ;
   BEGIN
      IF help_option(pr_cmd, ' [<apps-list> | all]', 'Configure substitution variables for given applications.') THEN
         RETURN;
      END IF;
   ...
   IS
      ...
      t_prompts sys.dbms_sql.varchar2a;
      l_prompts dbm_files.prompts%TYPE;
   BEGIN
      ...
      FOR j IN 1..13 LOOP
         t_files := CASE j WHEN 1 THEN r_curr_ver.t_ins_files
                           ...
                           WHEN 12 THEN r_curr_ver.t_xpo_files
                           WHEN 13 THEN r_curr_ver.t_imp_files
                     END;
         t_prompts := CASE j WHEN 1 THEN r_curr_ver.t_ins_prompts
                             WHEN 2 THEN r_curr_ver.t_inr_prompts
                             WHEN 3 THEN r_curr_ver.t_upg_prompts
                             WHEN 4 THEN r_curr_ver.t_upr_prompts
                             WHEN 5 THEN r_curr_ver.t_uni_prompts
                             WHEN 6 THEN r_curr_ver.t_val_prompts
                             WHEN 7 THEN r_curr_ver.t_cfg_prompts
                             WHEN 8 THEN r_curr_ver.t_pre_prompts
                             WHEN 9 THEN r_curr_ver.t_set_prompts
                             WHEN 10 THEN r_curr_ver.t_exp_prompts
                             WHEN 11 THEN r_curr_ver.t_con_prompts
                             WHEN 12 THEN r_curr_ver.t_xpo_prompts
                             WHEN 13 THEN r_curr_ver.t_imp_prompts
                     END;
         ...
         FOR i IN NVL(t_files.FIRST,1)..NVL(t_files.last,0) LOOP
            ...
            r_fil.prompts := CASE WHEN t_prompts.EXISTS(i) THEN t_prompts(i) ELSE NULL END;
            r_fil.seq := i;
            r_fil.run_condition := CASE WHEN r_curr_ver.a_fil.EXISTS(r_fil.path) THEN r_curr_ver.a_fil(r_fil.path) ELSE NULL END;
            r_fil.trusted_hash := CASE WHEN r_curr_ver.a_crc.EXISTS(r_fil.path) THEN r_curr_ver.a_crc(r_fil.path) ELSE NULL END;
            upsert_fil(p_app_code=>r_fil.app_code, p_ver_code=>r_fil.ver_code, p_path=>r_fil.path, p_type=>r_fil.type, p_trusted_hash=>r_fil.trusted_hash
                     , p_seq=>r_fil.seq, p_run_condition=>r_fil.run_condition, p_prompts=>r_fil.prompts, p_deleted_flag=>NULL);
         END LOOP;
      END LOOP;
   END;
--#begin public
...
--#end public
   IS
      CURSOR c_str IS
         SELECT RTRIM(RTRIM(text,CHR(13))) text, line
           FROM dbm_streams
          WHERE cmd_id = p_cmd_id
            AND type = 'OUT'
      ...
      lt_xpo sys.dbms_sql.varchar2a;
      lt_imp sys.dbms_sql.varchar2a;
      lt_prompts sys.dbms_sql.varchar2a;
      l_prompts VARCHAR2(4000 CHAR);
      la_obj dbm_utility_var.ga_obj_type;
      la_fil dbm_utility_var.ga_fil_type;
      la_crc dbm_utility_var.ga_crc_type;
      ...
      FOR r_str IN c_str LOOP
         ...
         IF SUBSTR(r_str.text,1,1) = '#' THEN
            ...
            IF SUBSTR(r_str.text,1,2) = '#!' THEN
               file_break;
               l_path := REPLACE(TRIM(SUBSTR(r_str.text,3)),'\','/');
...
--                  lt_var.DELETE;
--                  la_par.DELETE;
                  la_fil := lr_ver.a_fil; -- keep existing file conditions
                  la_crc := lr_ver.a_crc; -- keep existing file hashes
               ...
               ELSIF lt_path.COUNT >= 1 THEN
                  IF l_file_ext = 'conf' THEN
                     ...
                     IF lt_ver.EXISTS(l_ver_nbr) THEN
                        ...
                        lt_xpo := lr_ver.t_xpo_files;
                        lt_imp := lr_ver.t_imp_files;
                        la_obj := lr_ver.a_obj;
                        la_fil := lr_ver.a_fil;
                        la_crc := lr_ver.a_crc;
                        lt_var := lr_ver.t_var;
         ...
         ELSIF l_file_ext = 'conf' THEN
            ...
            ELSIF la_cfg(1) = LOWER(l_app_code) AND la_cfg(2) = 'par' AND la_cfg.COUNT >= 3 THEN
               la_par(la_cfg(3)) := CASE WHEN la_cfg.COUNT >= 4 THEN SUBSTR(la_cfg(4),1,4000) ELSE NULL END;
            END IF;
         ...
         ELSIF LOWER(lt_path(lt_path.COUNT)) = 'files.dbm' THEN
            ...
            IF l_pos > 0 THEN
               ...
               l_pos := l_pos + 1;
               WHILE SUBSTR(r_str.text,l_pos,1) = ' ' LOOP
                  l_pos := l_pos + 1;
               END LOOP;
               IF REGEXP_LIKE(SUBSTR(r_str.text,l_pos),'[a-f0-9]{32}($| )') THEN
                  r_fil.trusted_hash := SUBSTR(r_str.text,l_pos,32);
                  l_pos := l_pos + 32;
               END IF;
               WHILE SUBSTR(r_str.text,l_pos,1) = ' ' LOOP
                  l_pos := l_pos + 1;
               END LOOP;
               IF NOT REGEXP_LIKE(SUBSTR(r_str.text,l_pos),'[a-f0-9]{32}') THEN
                  r_fil.run_condition := TRIM(SUBSTR(r_str.text,l_pos));
               END IF;
            ELSE
               r_fil.path := get_dir(l_path,TRUE)||r_str.text;
               r_fil.run_condition := NULL;
            END IF;
            r_fil.path := REPLACE(r_fil.path,'\','/');
            IF r_fil.run_condition IS NOT NULL THEN
               la_fil(r_fil.path) := r_fil.run_condition;
            END IF;
            IF r_fil.trusted_hash IS NOT NULL THEN
               la_crc(r_fil.path) := r_fil.trusted_hash;
            END IF;
         ELSIF SUBSTR(LOWER(lt_path(lt_path.COUNT)),-4) = '.dbm' THEN
            l_pos := NVL(INSTR(r_str.text, ':'),0);
            l_tmp := LTRIM(RTRIM(CASE WHEN l_pos > 0 THEN SUBSTR(r_str.text, 1, l_pos-1) ELSE r_str.text END),'@');
            IF get_file_extension(l_tmp) IS NOT NULL THEN
               l_tmp := REPLACE(get_dir(l_path,TRUE)||l_tmp,'\','/');
               IF l_pos > 0 THEN
                  la_fil(l_tmp) := TRIM(SUBSTR(r_str.text, l_pos+1)); -- run condition
               END IF;
               IF LOWER(lt_path(lt_path.COUNT)) = 'install.dbm' THEN
                  lt_ins(lt_ins.COUNT+1) := l_tmp;
                  lt_prompts(lt_ins.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'upgrade.dbm' THEN
                  lt_upg(lt_upg.COUNT+1) := l_tmp;
                  lt_prompts(lt_upg.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'uninstall.dbm' THEN
                  lt_uni(lt_uni.COUNT+1) := l_tmp;
                  lt_prompts(lt_uni.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'validate.dbm' THEN
                  lt_val(lt_val.COUNT+1) := l_tmp;
                  lt_prompts(lt_val.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'setup.dbm' THEN
                  lt_set(lt_set.COUNT+1) := l_tmp;
                  lt_prompts(lt_set.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'expose.dbm' THEN
                  lt_exp(lt_exp.COUNT+1) := l_tmp;
                  lt_prompts(lt_exp.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'conceal.dbm' THEN
                  lt_con(lt_con.COUNT+1) := l_tmp;
                  lt_prompts(lt_con.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'export.dbm' THEN
                  lt_xpo(lt_xpo.COUNT+1) := l_tmp;
                  lt_prompts(lt_xpo.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'import.dbm' THEN
                  lt_imp(lt_imp.COUNT+1) := l_tmp;
                  lt_prompts(lt_imp.COUNT) := l_prompts;
               ELSIF LOWER(lt_path(lt_path.COUNT)) = 'precheck.dbm' THEN
                  lt_pre(lt_pre.COUNT+1) := l_tmp;
                  lt_prompts(lt_pre.COUNT) := l_prompts;
               END IF;
               l_prompts := NULL;
            ELSE
               l_len := in_sqlplus_cmd(r_str.text, 'PROMPT', 3);
               IF l_len > 0 AND TRIM(SUBSTR(r_str.text,l_len)) IS NOT NULL THEN
                  l_prompts := CASE WHEN l_prompts IS NOT NULL THEN l_prompts || CHR(10) END || SUBSTR(r_str.text,8);
               END IF;
            END IF;
         END IF;
      END LOOP;
...
--#end public
   IS
      CURSOR c_str IS
         SELECT cmd_id, line-1 line, type, RTRIM(RTRIM(text,CHR(13))) text
           FROM dbm_streams
          WHERE cmd_id = p_cmd_id
            AND type = 'OUT'
      ...
      l_pos2 PLS_INTEGER;
      l_beg PLS_INTEGER;
      l_stmt_id dbm_files.stmt_id%TYPE;
      l_last_stmt_id dbm_files.stmt_id%TYPE;
      l_line_no PLS_INTEGER;
      l_update BOOLEAN;
      l_run_condition dbm_files.run_condition%TYPE;
      l_condition_met BOOLEAN := TRUE;
      l_ver_nbr dbm_versions.ver_nbr%TYPE;
   BEGIN
      IF dbm_utility_var.g_trace THEN
         dbms_output.put_line('->parse_script(p_cmd_id=>'||p_cmd_id||','||p_app_code||','||p_ver_code||','||p_path||','||p_stmt_id||')');
      END IF;
      l_ver_nbr := get_version_nbr(p_ver_code);
      FOR r_str IN c_str LOOP
         l_update := FALSE;
         ...
         IF SUBSTR(r_str.text,1,2) = '#!' THEN
            ...
            ;
            assert(SQL%ROWCOUNT=1,'Error while deleting file header line from stream!');
         ELSE
            l_beg := in_sqlplus_cmd(r_str.text, 'REMARK', 3);
            IF l_beg > 0 THEN
               l_beg := l_beg + 1;
               WHILE SUBSTR(r_str.text,l_beg,1) = ' ' LOOP
                  l_beg := l_beg + 1;
               END LOOP;
               IF UPPER(SUBSTR(r_str.text,l_beg,4)) = 'DBM-' THEN
                  l_pos := l_beg;
                  l_stmt_id := 0;
                  l_pos := l_pos + 4;
                  WHILE SUBSTR(r_str.text,l_pos,1) BETWEEN '0' AND '9' LOOP
                     l_stmt_id := l_stmt_id * 10 + (ASCII(SUBSTR(r_str.text,l_pos,1)) - ASCII('0'));
                     l_pos := l_pos + 1;
                  END LOOP;
                  assert(l_stmt_id > l_last_stmt_id,'Invalid statement id "'||SUBSTR(r_str.text,l_beg,l_pos-l_beg)||'" in "'||p_path||'" at line '||r_str.line||'!');
                  l_last_stmt_id := l_stmt_id;
                  IF l_last_stmt_id = p_stmt_id THEN
                     dbms_output.put_line('Restarting execution from statement "'||SUBSTR(r_str.text,l_beg,l_pos-l_beg)||'"...');
                  END IF;
                  l_pos2 := INSTR(r_str.text, ':', -1); -- search for last occurence of semi-column
                  l_condition_met := TRUE;
                  IF l_pos2 > 0 THEN
                     l_run_condition := TRIM(SUBSTR(r_str.text,l_pos2+1));
                     l_condition_met := file_condition_met(p_app_code, l_ver_nbr, p_path, l_run_condition);
                  END IF;
               END IF;
            END IF;
            IF l_last_stmt_id < NVL(p_stmt_id,0) OR NOT l_condition_met THEN
               IF NOT l_condition_met AND l_last_stmt_id >= NVL(p_stmt_id,l_last_stmt_id) AND l_pos > 0 THEN
                  r_str.text := 'prompt Skipping statement "'||SUBSTR(r_str.text,l_beg,l_pos-l_beg)||'" as condition "'||l_run_condition||'" is not met!';
               ELSE
                  r_str.text := NULL;
               END IF;
               l_update := TRUE;
            ELSIF l_last_stmt_id >= NVL(p_stmt_id,l_last_stmt_id) THEN
               IF l_pos > 0 THEN
                  r_str.text := REPLACE('exec dbm_utility_krn.upsert_fil(p_path=>"'||p_path||'", p_stmt_id=>'||l_stmt_id||')','"','''');
                  l_update := TRUE;
               END IF;
            END IF;
            IF l_update THEN
               UPDATE dbm_streams
                ...
                WHERE CURRENT OF c_str
               ;
               assert(SQL%ROWCOUNT=1,'Error while updating stream!');
            END IF;
         END IF; -- header
      END LOOP;
      COMMIT;
   ...
   IS
      ...
      FUNCTION get_file_attributes (
         pr_ver dbm_utility_var.r_ver_type
       , p_path dbm_utility_var.g_path_type
      )
      RETURN VARCHAR2
      IS
      BEGIN
         RETURN CASE WHEN pr_ver.a_fil.EXISTS(p_path) OR pr_ver.a_crc.EXISTS(p_path) THEN ':' END
             || CASE WHEN pr_ver.a_fil.EXISTS(p_path) THEN ' ' || pr_ver.a_fil(p_path) END
             || CASE WHEN pr_ver.a_crc.EXISTS(p_path) THEN ' ' || pr_ver.a_crc(p_path) END;
      END;
   BEGIN
      IF help_option(pr_cmd, ' [<apps-list> | all] [version]', 'Display information about given applications (stored in the internal cache).') THEN
         RETURN;
      END IF;
            ...
            WHILE l_ver_idx IS NOT NULL LOOP
               ...
               IF p_ver_code IS NULL OR lr_ver.r_ver.ver_code = p_ver_code THEN
                  ...
                  FOR i IN NVL(lr_ver.t_ins_files.FIRST,1)..NVL(lr_ver.t_ins_files.LAST,0) LOOP
                     ...
                     dbms_output.put_line('.   '||l_path||get_file_attributes(lr_ver,l_path));
                  END LOOP;
                  ...
                  FOR i IN NVL(lr_ver.t_con_files.FIRST,1)..NVL(lr_ver.t_con_files.LAST,0) LOOP
                     ...
                     dbms_output.put_line('.   '||l_path||get_file_attributes(lr_ver,l_path));
                  END LOOP;
                  dbms_output.put_line('- Exportable: '||NVL(lr_ver.r_ver.exportable,'N'));
                  FOR i IN NVL(lr_ver.t_xpo_files.FIRST,1)..NVL(lr_ver.t_xpo_files.LAST,0) LOOP
                     l_path := lr_ver.t_xpo_files(i);
                     dbms_output.put_line('.   '||l_path||get_file_attributes(lr_ver,l_path));
                  END LOOP;
                  dbms_output.put_line('- Importable: '||NVL(lr_ver.r_ver.importable,'N'));
                  FOR i IN NVL(lr_ver.t_imp_files.FIRST,1)..NVL(lr_ver.t_imp_files.LAST,0) LOOP
                     l_path := lr_ver.t_imp_files(i);
                     dbms_output.put_line('.   '||l_path||get_file_attributes(lr_ver,l_path));
                  END LOOP;
                  ...
                  FOR i IN 1..lr_ver.t_prv.COUNT LOOP
                     dbms_output.put_line('.   '||lr_ver.t_prv(i).usage||' '||lr_ver.t_prv(i).type||' '||lr_ver.t_prv(i).text||CASE WHEN lr_ver.t_prv(i).condition IS NOT NULL THEN ': '||lr_ver.t_prv(i).condition END);
                  END LOOP;
               END IF;
...
--#end public
   IS
      r_cmd dbm_utility_var.gr_command_type;
   BEGIN
      display_application (r_cmd, p_app_code, p_ver_code);
   END;
   ---
   ...
   BEGIN
      ...
      FOR i IN 1..pr_cmd.t_options.COUNT LOOP
         ...
         ELSIF p_process_options THEN
            ...
            ELSIF r_option.name = 'nocolor' THEN
               dbm_utility_var.g_color := FALSE;
            ELSIF r_option.name = 'color' THEN
               dbm_utility_var.g_color := TRUE;
            ...
            ELSE
               raise_application_error(-20000,'Invalid option "-'||r_option.name||'" for dbm-cli');
            END IF;
         END IF;
         ...
         FOR r_ver IN c_ver(r_app.app_code) LOOP
            ...
            FOR r_fil IN c_fil(r_app.app_code, r_ver.ver_code) LOOP
               IF    r_fil.type = 'INSTALL' THEN
                  ...
                  lr_ver.t_ins_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'ROLLBACK INSTALL' THEN
                  lr_ver.t_inr_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_inr_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'UPGRADE' THEN
                  lr_ver.t_upg_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_upg_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'ROLLBACK UPGRADE' THEN
                  lr_ver.t_upr_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_upr_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'UNINSTALL' THEN
                  lr_ver.t_uni_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_uni_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'VALIDATE' THEN
                  lr_ver.t_val_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_val_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'CONFIG' THEN
                  lr_ver.t_cfg_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_cfg_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'PRECHECK' THEN
                  lr_ver.t_pre_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_pre_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'SETUP' THEN
                  lr_ver.t_set_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_set_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'EXPOSE' THEN
                  lr_ver.t_exp_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_exp_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'CONCEAL' THEN
                  lr_ver.t_con_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_con_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'EXPORT' THEN
                  lr_ver.t_xpo_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_xpo_prompts(r_fil.seq) := r_fil.prompts;
               ELSIF r_fil.type = 'IMPORT' THEN
                  lr_ver.t_imp_files(r_fil.seq) := r_fil.path;
                  lr_ver.t_imp_prompts(r_fil.seq) := r_fil.prompts;
               END IF;
               IF r_fil.run_condition IS NOT NULL THEN
                  lr_ver.a_fil(r_fil.path) := r_fil.run_condition;
               END IF;
               IF r_fil.trusted_hash IS NOT NULL THEN
                  lr_ver.a_crc(r_fil.path) := r_fil.trusted_hash;
               END IF;
            END LOOP;
            ...
            FOR r_par IN c_par(r_app.app_code, r_ver.ver_code) LOOP
               ...
               ELSIF r_par.name = 'color' THEN
                  dbm_utility_var.g_color := UPPER(r_par.value) = 'TRUE';
               ELSE
...
--#end public
   ...
   BEGIN
      ...
      WHILE l_app_code IS NOT NULL LOOP
         ...
         WHILE l_ver_nbr IS NOT NULL LOOP
            ...
            upsert_ver (
             ...
             , p_exportable=>r_ver.exportable
             , p_importable=>r_ver.importable
             , p_deleted_flag=>NULL
            );
            -- Save files
            ...
            FOR i IN 1..lt_prv.COUNT LOOP
               lr_prv := lt_prv(i);
               upsert_prv(p_app_code=>lr_prv.app_code, p_ver_code=>lr_prv.ver_code, p_usage=>lr_prv.usage, p_text=>lr_prv.text
      ...
      upsert_par(p_app_code=>'dbm_utility', p_ver_code=>'all', p_name=>'color', p_value=>CASE WHEN dbm_utility_var.g_color THEN 'TRUE' ELSE 'FALSE' END, p_deleted_flag=>NULL);
...
--         dbms_output.put_line(r_str.line||': '||r_str.text);
         ...
         IF lt_path.COUNT >= 3 AND LOWER(lt_path(2)) = 'releases' THEN
            ...
            IF r_curr_ver.r_ver.ver_code IS NOT NULL AND lt_path.COUNT = 6 AND is_valid_file_extension(lt_path(6)) THEN
               IF lt_path(4) = 'install' AND lt_path(5) = 'rollback' THEN
                  r_curr_ver.t_inr_files(NVL(r_curr_ver.t_inr_files.LAST,0)+1) := r_str.text;
                  r_curr_ver.r_ver.install_rollbackable := 'Y';
               ELSIF lt_path(4) = 'upgrade' AND lt_path(5) = 'rollback' THEN
                  r_curr_ver.t_upr_files(NVL(r_curr_ver.t_upr_files.LAST,0)+1) := r_str.text;
                  r_curr_ver.r_ver.upgrade_rollbackable := 'Y';
               END IF;
            ELSIF r_curr_ver.r_ver.ver_code IS NOT NULL AND lt_path.COUNT = 5 THEN
               IF LOWER(get_file_extension(lt_path(5))) IN ('conf','dbm') THEN
               ...
               ELSIF lt_path(4) = 'expose' THEN
                  ...
                  END IF;
                  r_curr_ver.r_ver.exposable := CASE WHEN r_curr_ver.t_xpo_files.COUNT > 0 THEN 'Y' ELSE NULL END;
               ...
               ELSIF lt_path(4) = 'export' THEN
                  IF lt_path(5) = 'export.dbm' THEN
                     r_curr_ver.t_cfg_files(NVL(r_curr_ver.t_cfg_files.LAST,0)+1) := r_str.text;
                  ELSIF lt_path(5) = 'export.sql' THEN
                     r_curr_ver.t_xpo_files(0) := r_str.text;
                  ELSIF is_valid_file_extension(lt_path(5)) THEN
                     r_curr_ver.t_xpo_files(NVL(r_curr_ver.t_xpo_files.LAST,0)+1) := r_str.text;
                  END IF;
                  r_curr_ver.r_ver.exportable := CASE WHEN r_curr_ver.t_xpo_files.COUNT > 0 THEN 'Y' ELSE NULL END;
               ELSIF lt_path(4) = 'import' THEN
                  IF lt_path(5) = 'import.dbm' THEN
                     r_curr_ver.t_cfg_files(NVL(r_curr_ver.t_cfg_files.LAST,0)+1) := r_str.text;
                  ELSIF lt_path(5) = 'import.sql' THEN
                     r_curr_ver.t_imp_files(0) := r_str.text;
                  ELSIF is_valid_file_extension(lt_path(5)) THEN
                     r_curr_ver.t_imp_files(NVL(r_curr_ver.t_imp_files.LAST,0)+1) := r_str.text;
                  END IF;
                  r_curr_ver.r_ver.importable := CASE WHEN r_curr_ver.t_imp_files.COUNT > 0 THEN 'Y' ELSE NULL END;
               ...
               ELSIF (lt_path(4) = 'config' AND LOWER(get_file_extension(lt_path(5))) = 'conf') OR LOWER(lt_path(5)) = 'objects.dbm' THEN
                  r_curr_ver.t_cfg_files(NVL(r_curr_ver.t_cfg_files.LAST,0)+1) := r_str.text;
               END IF;
            END IF;
         END IF;
   ...
   IS
      CURSOR c_str IS
         SELECT *
          ...
          ORDER BY line
      ;
      l_hash dbm_files.runtime_hash%TYPE;
      l_path dbm_files.path%TYPE;
      r_fil dbm_files%ROWTYPE;
      l_apps_dir dbm_utility_var.g_par_value_type;
      l_apps_len PLS_INTEGER;
      l_status dbm_files.trusted_status%TYPE;
   BEGIN
      l_apps_dir := rtrim_slashes(p_apps_dir);
      l_apps_len  := LENGTH(l_apps_dir);
      FOR r_str IN c_str LOOP
         l_hash := SUBSTR(r_str.text,1,32);
         l_path := REPLACE(SUBSTR(r_str.text,34),'\','/');
         IF SUBSTR(l_path,1,l_apps_len) = l_apps_dir AND SUBSTR(l_path,l_apps_len+1,1) IN ('/','\') THEN
            l_path := SUBSTR(l_path,l_apps_len+2);
         END IF;
         r_fil := get_file(l_path);
         IF r_fil.path IS NULL THEN
            GOTO next_str;
         END IF;
         IF p_op = 'set-trusted-hashes' THEN -- make command
            update_fil(p_path=>l_path, p_current_hash=>l_hash, p_trusted_hash=>l_hash, p_trusted_status=>'VALID', p_raise_exception=>FALSE);
         ELSIF p_op LIKE 'set-runtime-hashes%' THEN -- migrate/uninstall commands
            update_fil(p_path=>l_path, p_current_hash=>l_hash, p_runtime_hash=>l_hash, p_runtime_status=>'VALID', p_raise_exception=>FALSE);
         END IF;
         IF p_op IN ('chk-hashes','chk-trusted-hashes') OR p_op LIKE 'set-runtime-hashes%' THEN -- check + migrate/uninstall commands
            l_status := CASE WHEN r_fil.path LIKE '%files.dbm' OR NVL(r_fil.trusted_hash,l_hash) = l_hash THEN 'VALID' ELSE 'TAMPERED' END;
            IF r_fil.trusted_status IS NULL OR l_status != r_fil.trusted_status THEN
               update_fil(p_path=>l_path, p_trusted_status=>l_status, p_raise_exception=>FALSE);
            END IF;
            -- Don't execute a file which is tampered (unless end-user explicitly asked to ignore hashes)!
            assert(p_op = 'set-runtime-hashes-ignore-tampered' OR p_op LIKE 'chk%' OR l_status = 'VALID','File "'||r_fil.path||'" is '||color(l_status,'red')||'!');
         END IF;
         IF p_op IN ('chk-hashes','chk-runtime-hashes') THEN -- check command
            IF r_fil.run_status = 'SUCCESS' THEN -- only check files executed successfully
               l_status := CASE WHEN NVL(r_fil.runtime_hash,l_hash) = l_hash THEN 'VALID' ELSE 'TAMPERED' END;
               IF r_fil.runtime_status IS NULL OR l_status != r_fil.runtime_status THEN
                  update_fil(p_path=>l_path, p_runtime_status=>l_status, p_raise_exception=>FALSE);
               END IF;
            END IF;
         END IF;
         <<next_str>>
...
--    output_line(r_cmd.cmd_id, 'IN', 'host del find-files.sql'); -- TBD, OS dependent
   ...
   BEGIN
      ...
      LOOP
         ...
         IF l_app <= t_app.COUNT THEN
            ...
            assert(r_app.app_code IS NOT NULL, 'Application "'||t_app(l_app)||'" does not exist!'||CHR(10)||'Hint: use "show" command to list existing applications');
         ...
         FOR i IN 1..pr_cmd.t_options.COUNT LOOP
            ...
            IF is_matching_prefix(r_option.name, 'h)elp') OR r_option.name = '?' THEN
               l_help := TRUE;
               EXIT search_help_option;
            END IF;
         END LOOP;
         IF pr_cmd.command IN ('set-current') AND NOT l_help THEN
            assert(r_ver.app_code IS NOT NULL, 'Application code is mandatory!');
         END IF;
         assert(l_help OR NOT (pr_cmd.command='set-current' AND r_ver.app_code IS NULL),'Application is mandatory for "set-current" command!');
         assert(l_help OR NOT (pr_cmd.command='set-current' AND NVL(r_ver.app_code,'all')='all'),'Application "all" not allowed for "set-current" command!');
         assert(l_help OR NOT (pr_cmd.command='make-inventory' AND r_ver.app_code IS NULL),'Application is mandatory for "make-inventory" command!');
         IF r_ver.app_code IS NOT NULL AND pr_cmd.t_params.COUNT >= 2 THEN
            ...
            ELSE
               dbms_output.put_line('Warning: invalid command argument "'||pr_cmd.t_params(2)||'" will be ignored!');
            END IF;
         END IF;
         assert(pr_cmd.t_params.COUNT<=2, 'Unexpected command parameter: '||CASE WHEN pr_cmd.t_params.COUNT>=3 THEN pr_cmd.t_params(3) END);
         assert(l_help OR NOT (pr_cmd.command='set-current' AND r_ver.ver_code IS NULL),'Version is mandatory for "set-current" command!');
         ...
         ELSIF pr_cmd.command IN ('export','import') THEN
            export_import(pr_cmd, r_ver.app_code, NULL);
         ...
         ELSIF pr_cmd.command = 'display' THEN
            display_application(pr_cmd, r_ver.app_code, r_ver.ver_code);
   ...
   IS
      ...
      l_cmd dbm_commands.command_line%TYPE;
      lr_cmd dbm_utility_var.gr_command_type;
      PROCEDURE dbm_cli_help IS 
      BEGIN
         FOR i IN 1..pr_cmd.t_options.COUNT LOOP
            r_option := pr_cmd.t_options(i);
         ...
         dbms_output.put_line('Copyright (C) '||TO_CHAR(SYSDATE,'YYYY')||' - European Commission');
         dbms_output.put_line('========================================');
         ...
         dbms_output.put_line('- expor[t] [<application>] [<path>]');
         dbms_output.put_line('- expos[e] [<application>] [<grantee>]');
         dbms_output.put_line('- flash[back-database] (CoP DB only!)');
         dbms_output.put_line('- guess[-current] [<application>]');
         dbms_output.put_line('- help [<commmand>]');
         dbms_output.put_line('- imp[ort] [<application>] [<path>]');
   ...
   BEGIN
      IF pr_cmd.t_params.COUNT > 0 THEN
         FOR i IN 1..pr_cmd.t_params.COUNT LOOP
            l_command := pr_cmd.t_params(i);
            l_command := get_full_cmd(l_command);
            IF l_command = 'all' THEN
               l_cmd := dbm_utility_var.ga_cmd.FIRST;
               WHILE l_cmd IS NOT NULL LOOP
                  IF l_cmd IN ('migrate','import','install','upgrade','precheck','export','expose','conceal','uninstall','rollback','validate')
                  OR l_cmd IN ('check-files','setup')
                  OR l_cmd IN ('configure','display','set-current','guess-current','show-current','make-inventory')
                  THEN
                        lr_cmd := pr_cmd;
                        lr_cmd.t_params.DELETE;
                        lr_cmd.t_params(1) := l_cmd;
                        help(lr_cmd);
                  END IF;
                  l_cmd := dbm_utility_var.ga_cmd.NEXT(l_cmd);
               END LOOP;
            ELSIF l_command = 'help' THEN
               dbm_cli_help;
            ELSIF dbm_utility_var.ga_cmd.EXISTS(l_command) THEN
               IF l_command IN ('migrate','import','install','upgrade','precheck','export','expose','conceal','uninstall','rollback','validate')
               OR l_command IN ('check-files','setup')
               OR l_command IN ('configure','display','set-current','guess-current','show-current','make-inventory')
               THEN
                  r_command := NULL;
                  r_command.cmd_id := pr_cmd.cmd_id;
                  r_command.command := l_command;
                  r_option := NULL;
                  r_option.name := 'help';
                  r_command.t_options(1) := r_option;
                  parse_op(r_command);
               ELSE
                  dbms_output.put_line('No help available for command "'||l_command||'"');
               END IF;
            ELSE
               dbms_output.put_line('Invalid command: "'||l_command||'"!');
               dbms_output.put_line('Hint: just type "help" with no argument to get the list of valid commands.');
            END IF;
         END LOOP;
      ELSE
         dbm_cli_help;
...
--         print_commands;
     END;
      ...
      BEGIN
         ...
         IF p_log THEN
            ...
            ;
            COMMIT;
            spool(r_cmd.cmd_id, 'IN', r_cmd.log_file_name); --||' append');
            l_spool := TRUE;
         END IF;
      END;
      ...
      BEGIN
         ...
         END IF;
         IF l_spool THEN
            spool(p_cmd_id=>r_cmd.cmd_id, p_type=>'IN', p_text=>'off', p_line=>999999900);
            l_spool := FALSE;
         END IF;
   ...
   BEGIN
      ...
      IF l_command IS NULL THEN
         ...
         dbms_output.put_line('Hint: use "help" command to get the list of available commands.');
      ...
      ELSIF l_command = 'noop' THEN
         insert_command(FALSE);
         noop;
      ELSIF l_command IN ('migrate','import','install','upgrade','precheck','export','expose','conceal','uninstall','rollback','validate') THEN
         -- Log and fork
         insert_command(TRUE);
         parse_op(r_command);
         fork_sqlplus;
      ELSIF l_command IN ('check-files','setup') THEN
         ...
         insert_command(TRUE);
         parse_op(r_command);
      ELSIF l_command IN ('configure','display','set-current','guess-current','show-current','make-inventory') THEN
         -- No log and no fork
         insert_command(FALSE);
         parse_op(r_command);
