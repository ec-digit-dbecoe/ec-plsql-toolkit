...
--#end public
   ...
   IS
   BEGIN
      RETURN get_par_value('dbm_utility', 'all', 'get_file_extensions','sql, pks, pkb, pls, plb, prc, fnc, java');
   END;
   ---
   ...
   BEGIN
      ...
      FOR r_app IN c_app(p_app_code) LOOP
         ...
         FOR r_fil IN c_fil LOOP
            ...
            IF LOWER(get_file_extension(r_fil2.path)) IN ('pkb','pks','plb','pls','prc','fnc') THEN
               show_errors(pr_cmd);
...
--         output_line(pr_cmd.cmd_id,'IN','prompt Migrating application "'||r_app.app_code||'"...');
         ...
         FOR r_ver IN c_ver(r_app.app_code, pr_ver.ver_status) LOOP
            ...
            FOR k IN 1..2 LOOP -- 1: app/ver, 2:app/0
               EXIT WHEN k = 2 AND NVL(r_ver.ver_code,'all') = 'all';
               l_ver_code := CASE k WHEN 1 THEN NVL(r_ver.ver_code,'all') ELSE 'all' END;
               FOR r_fil IN c_fil(r_ver.app_code, l_ver_code, 'PRECHECK') LOOP
                  IF r_fil.run_condition IS NOT NULL AND NOT file_condition_met(r_ver.app_code, r_ver.ver_nbr, r_fil.path, r_fil.run_condition) THEN
            ...
            FOR r_fil IN c_fil(r_ver.app_code, r_ver.ver_code, r_ver.next_op_type) LOOP
               ...
               ELSE
                  ...
                  ELSE
                     ...
                     output_line(pr_cmd.cmd_id,'IN','prompt Executing script "'||get_apps_dir(TRUE)||r_fil.path||'"...');
                     output_line(pr_cmd.cmd_id,'IN','@@'||get_apps_dir(TRUE)||r_fil.path);
                     IF LOWER(get_file_extension(r_fil.path)) IN ('pkb','pks','plb','pls','prc','fnc') THEN
                        show_errors(pr_cmd);
                        stop_on_warning(pr_cmd, r_app.app_code);
                     END IF;
                     output_line(pr_cmd.cmd_id,'IN',REPLACE('exec dbm_utility_krn.update_fil(p_path=>"'||r_fil.path||'", p_run_status=>"SUCCESS")','"',''''));
   ...
   BEGIN
      ...
      FOR r_app IN c_app LOOP
         ...
         l_accessible := CASE WHEN r_app.owner != USER THEN ', and is '|| CASE WHEN r_app.granted_flag = 'N' THEN 'NOT ' END||'accessible' END;
         IF r_app.ver_code IS NOT NULL THEN
            dbms_output.put_line('Application "'||r_app.app_code||'" '||l_owned_by||'has version "'||r_app.ver_code||'", status "'||NVL(r_app.ver_status,'UNKNOWN')||'"'||l_accessible);
         ELSE
            l_obj_name_pattern := get_par_value(r_app.app_code, 0, 'object_name_pattern');
            l_obj_name_anti_pattern := get_par_value(r_app.app_code, 0, 'object_name_anti_pattern', '^$');
            ...
            IF l_obj_count = 0 AND r_app.owner = USER THEN
               dbms_output.put_line('Application "'||r_app.app_code||'" is not installed locally.');
            ELSE
               dbms_output.put_line('Application "'||r_app.app_code||'" '||l_owned_by||'has version "UNKNOWN", status "'||NVL(r_app.ver_status,'UNKNOWN')||'"'||l_accessible);
            END IF;
         END IF;
      END LOOP;
   END;
...
--         END IF;
         ...
         output_whenever_sqlerror_exit(p_cmd_id=>r_cmd.cmd_id, p_type=>'IN', p_line=>-11); -- 2 lines
   ...
   BEGIN
      ...
      IF t_command.COUNT >= 2 AND l_command NOT IN ('startup','scan-files','read-config') THEN
         ...
         FOR i IN t_command.FIRST..t_command.LAST LOOP
            ...
            output_whenever_sqlerror_exit(p_cmd_id=>r_cmd.cmd_id, p_type=>'IN');
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ...
         IF l_sqlerrm != 'ORA-20000: ' THEN
            output_line(r_cmd.cmd_id, 'IN', 'prompt ERROR: '||REPLACE(l_sqlerrm,'ORA-20000: '));
         END IF;
         RETURN r_cmd.cmd_id;
   END;
--#begin public
