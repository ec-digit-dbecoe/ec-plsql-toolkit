...
--
   ...
   * v1.01; 2024-09-26; malmjea; execute post actions
   */
   ...
   */
   ...
   /**
   * Execute a post action. The placeholders are first decrypted.
   *
   * @param p_code: action SQL code
   * @throws -20012: invalid action code
   */
   PROCEDURE exec_post_action(
      p_code            IN  CLOB
   ) IS
   ...
   BEGIN
      ...
      IF code IS NULL THEN
         raise_error(-20012, 'invalid action code');
      END IF;

      -- Browse through the placeholders.
      ph_pos := INSTR(code, '{DPPCRYPT:');
      WHILE ph_pos >= 1 LOOP
         ...
         variable := SUBSTR(
            code
          , ph_pos + 10
          , INSTR(code, '}', ph_pos + 1) - (ph_pos + 10)
         );

         -- DEBUG
         show_debug('Decrypting "' || variable || '...');
   ...
   END exec_post_action;

   /**
   * Execute the monitoring post actions.
   *
   * @param p_sma_id: schema id
   * @throws -20009: invalid schema name
   * @throws -20011: executing post actions failure
   * @throws -20012: invalid action code
   */
   PROCEDURE exec_post_actions(
   ...
   BEGIN
      ...
      -- Check parameters.
      IF p_sma_id IS NULL THEN
         raise_error(-20009, 'invalid schema ID');
      END IF;
      ...
      FOR r_action IN (
         ...
         || TO_CHAR(r_action.execution_order)
         );
         exec_post_action(r_action.block_text);
   ...
   EXCEPTION
      WHEN OTHERS THEN
         -- DEBUG
         show_debug(
            'Execution monitoring post actions failure: '
         || NVL(SQLERRM, '/')
         );
         raise_error(
            -20011
          , 'executing monitoring post actions failure ('
          || NVL(SQLERRM, '/')
          || ')' 
         );

   END exec_post_actions;
   ...
   */
   ...
   BEGIN
      ...
      EXCEPTION
         WHEN OTHERS THEN
            -- Treatment must go on even if the status could not be uodated.
            NULL;
      ...
      <<exec_post>>
      BEGIN
         exec_post_actions(p_sma_id);
      EXCEPTION
         WHEN OTHERS THEN
            -- Treatment must go on even if the post actions could not be
            -- executed.
            NULL;
      END exec_post;
