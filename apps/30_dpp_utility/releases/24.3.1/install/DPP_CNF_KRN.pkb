...
*/
   ...
   */
   PROCEDURE insert_http_request_action(
    ...
    , p_user_modif            IN  dpp_actions.user_modif%TYPE        := NULL
      ...
      -- SQL code
      code                    dpp_actions.block_text%TYPE;
   ...
   BEGIN
      ...
      -- Build the SQL code.
      code := dpp_cnf_var.gk_actcode_http_request;
      code := REPLACE(
         code
       , '{url}'
       , '''' || REPLACE(TRIM(p_url), '''', '''''') || ''''
      );
      IF TRIM(p_wallet_path) IS NULL THEN
         code := REPLACE(code, '{wallet_path}', 'NULL');
      ELSE
         code := REPLACE(
            code
          , '{wallet_path}'
          , '''' || REPLACE(TRIM(p_wallet_path), '''', '''''') || ''''
         );
      END IF;
      IF TRIM(p_proxy) IS NULL THEN
         code := REPLACE(code, '{proxy}', 'NULL');
      ELSE
         code := REPLACE(
            code
          , '{proxy}'
          , '''' || REPLACE(TRIM(p_proxy), '''', '''''') || ''''
         );
      END IF;
      ...
      insert_action(
       ...
       , p_functional_name    => p_functional_name
       , p_atn_usage          => p_atn_usage
       , p_atn_type           => p_atn_type
       , p_exec_order         => p_exec_order
       , p_block_text         => code
   ...
   END insert_http_request_action;
   ...
   */
   PROCEDURE insert_http_request_action_enc(
    ...
    , p_ph_suffix             IN  VARCHAR2
      ...
      -- encrypted URL
      url_enc                 VARCHAR2(2000);

      -- encrypted wallet path
      wallet_path_enc         VARCHAR2(2000);

   BEGIN

      -- Check the suffix.
      IF TRIM(p_ph_suffix) IS NULL THEN
         raise_app_error(
            dpp_cnf_var.gk_errcode_inv_prm
          , 'invalid plcaholder suffix'
         );
      END IF;

      -- Encrypt the URL.
      IF TRIM(p_url) IS NOT NULL THEN
         INSERT INTO sec_crypto_credentials (
            username
          , password
         )
         VALUES (
            'URL_' || UPPER(TRIM(p_ph_suffix))
          , TRIM(p_url)
         );
         UPDATE sec_crypto_credentials
            SET password = sec_utility_krn.encrypt(
                              username
                            , password
                            , dpp_sec_var.gk_sec_util_key)
          WHERE username = 'URL_' || UPPER(TRIM(p_ph_suffix));
      ELSE
         raise_app_error(
            dpp_cnf_var.gk_errcode_inv_prm
          , 'invalid base URL'
      ...
      IF TRIM(p_wallet_path) IS NOT NULL THEN
         ...
         VALUES (
            'WALLET_PATH_' || UPPER(TRIM(p_ph_suffix))
          , TRIM(p_wallet_path)
         );
         UPDATE sec_crypto_credentials
            SET password = sec_utility_krn.encrypt(
                            ...
                            , password
                            , dpp_sec_var.gk_sec_util_key)
          WHERE username = 'WALLET_PATH_' || UPPER(TRIM(p_ph_suffix));
      END IF;

      -- Create the action.
      insert_http_request_action(
         p_schema_id          => p_schema_id
       , p_functional_name    => p_functional_name
       , p_atn_usage          => p_atn_usage
       , p_atn_type           => p_atn_type
       , p_exec_order         => p_exec_order
       , p_url                => '{DPPCRYPT:URL_' || UPPER(TRIM(p_ph_suffix)) || '}'
       , p_wallet_path        => '{DPPCRYPT:WALLET_PATH_' || UPPER(TRIM(p_ph_suffix)) || '}'
       , p_proxy              => p_proxy
       , p_active_flag        => p_active_flag
       , p_date_creat         => p_date_creat
       , p_user_creat         => p_user_creat
   ...
   END insert_http_request_action_enc;
   ...
   */
   ...
   BEGIN
      ...
      -- Build the SQL code.
      code := dpp_cnf_var.gk_actcode_gitlab_http_req;
      code := REPLACE(
         code
       , '{url}'
       , '''' || REPLACE(TRIM(p_url), '''', '''''') || ''''
      ...
      IF TRIM(p_token) IS NULL THEN
         code := REPLACE(code, '{token}', 'NULL');
      ELSE
         code := REPLACE(
            code
          , '{token}'
          , '''' || REPLACE(TRIM(p_token), '''', '''''') || ''''
         );
      END IF;
      IF TRIM(p_var_name) IS NULL THEN
         code := REPLACE(code, '{var_name}', 'NULL');
      ELSE
         code := REPLACE(
            code
          , '{var_name}'
          , '''' || REPLACE(TRIM(p_var_name), '''', '''''') || ''''
         );
      END IF;
      IF TRIM(p_var_value) IS NULL THEN
         code := REPLACE(code, '{var_value}', 'NULL');
      ELSE
         code := REPLACE(
            code
          , '{var_value}'
          , '''' || REPLACE(TRIM(p_var_value), '''', '''''') || ''''
         );
      END IF;
      IF TRIM(p_wallet_path) IS NULL THEN
         code := REPLACE(code, '{wallet_path}', 'NULL');
   ...
   END insert_http_gitlab_req_action;
   ...
   */
   ...
   BEGIN
      ...
      -- Encrypt the token.
      IF TRIM(p_token) IS NOT NULL THEN
         INSERT INTO sec_crypto_credentials (
            username
          , password
         )
         VALUES (
            'TOKEN_' || UPPER(TRIM(p_ph_suffix))
          , TRIM(p_token)
         );
         UPDATE sec_crypto_credentials
            SET password = sec_utility_krn.encrypt(
                            ...
                            , password
                            , dpp_sec_var.gk_sec_util_key)
          WHERE username = 'TOKEN_' || UPPER(TRIM(p_ph_suffix));
      END IF;

      -- Create the action.
      insert_http_gitlab_req_action(
         p_schema_id          => p_schema_id
       , p_functional_name    => p_functional_name
       , p_atn_usage          => p_atn_usage
       , p_atn_type           => p_atn_type
       ...
       , p_url                => '{DPPCRYPT:URL_' || UPPER(TRIM(p_ph_suffix)) || '}'
       , p_wallet_path        => '{DPPCRYPT:WALLET_PATH_' || UPPER(TRIM(p_ph_suffix)) || '}'
       , p_token              => '{DPPCRYPT:TOKEN_' || UPPER(TRIM(p_ph_suffix)) || '}'
   ...
   END insert_http_gitlab_req_action_enc;
