CREATE OR REPLACE PACKAGE DPP_JOB_VAR AUTHID DEFINER ACCESSIBLE BY (
   PACKAGE dpp_inj_krn
 , PACKAGE dpp_job_krn
 , PACKAGE dpp_job_mem
...
--
   ...
   PRAGMA EXCEPTION_INIT(ge_missing_hash_value, -20115);

   ge_illegal_argument EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_illegal_argument, -20109); -- was 21005!

   ge_selftest_failed EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_selftest_failed, -20107);

   ge_abort_export EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_abort_export, -20106);

   ge_no_imp_file_for_context EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_no_imp_file_for_context, -20108);

   ge_target_schema_doesnt_exist EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_target_schema_doesnt_exist, -20110);

   ge_abort_import EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_abort_import, -20111);
   ...
   PRAGMA EXCEPTION_INIT(ge_renaming_failed, -20112);

   ge_injection_failed EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_injection_failed, -20113);

   ge_illegal_arg_exec_sql_action   EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_illegal_arg_exec_sql_action, -20114);
   
   ge_illegal_arg_exec_sql_pre_post EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_illegal_arg_exec_sql_pre_post, -20116);
   
   ge_drop_obj_failed EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_drop_obj_failed, -20117);

   ge_inv_parameter EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_inv_parameter, -20220);

   ge_dir_not_exists EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_dir_not_exists, -20221);

   ge_abort_transfer EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_abort_transfer, -20222);

   ge_s3_not_implemented EXCEPTION;
   PRAGMA EXCEPTION_INIT(ge_s3_not_implemented, -20225);
   ...
   -- schema option: S3 bucket
   gk_schopt_s3_bucket           CONSTANT dpp_schema_options.otn_name%TYPE :=
      'S3_BUCKET';

   -- schema option: S3 prefix
   gk_schopt_s3_prefix           CONSTANT dpp_schema_options.otn_name%TYPE :=
      'S3_PREFIX';

   -- schema option: S3 compression level
   gk_schopt_s3_compr_level      CONSTANT dpp_schema_options.otn_name%TYPE :=
      'S3_COMPR_LEVEL';

   -- number of dump index digitss
   gk_dump_idx_digits            CONSTANT PLS_INTEGER := 3;

   -- trace message maximum length
   gk_trace_max_length           CONSTANT PLS_INTEGER := 900;

   -- S3 package name
   gk_s3_package_name            CONSTANT VARCHAR2(100)  := 'DPP_S3_KRN';
