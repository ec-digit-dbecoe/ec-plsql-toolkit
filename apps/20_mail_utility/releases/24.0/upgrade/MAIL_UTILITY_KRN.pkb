...
--
   ...
   */
   ...
   BEGIN
      ...
      l_conn := sys.UTL_SMTP.OPEN_CONNECTION(
         host        => mail_utility_var.g_smtp_host
       , port        => mail_utility_var.g_smtp_port
       , wallet_path => mail_utility_var.g_smtp_wallet_path
      );
      
      -- Set the domain
      IF mail_utility_var.g_smtp_domain IS NOT NULL THEN
         sys.UTL_SMTP.HELO(l_conn, mail_utility_var.g_smtp_domain);
      END IF;
      
      -- Perform authentication if needed.
      IF mail_utility_var.g_smtp_username IS NOT NULL AND
         mail_utility_var.g_smtp_password IS NOT NULL THEN
         SYS.UTL_SMTP.EHLO(l_conn, 'localhost');
         SYS.UTL_SMTP.STARTTLS(l_conn);
         SYS.UTL_SMTP.AUTH(
            c           => l_conn
         ,  username    => mail_utility_var.g_smtp_username
          , password    => mail_utility_var.g_smtp_password
          , schemes     => SYS.UTL_SMTP.ALL_SCHEMES
         );
      END IF;
  ...
  ------------------------------------------------------------------------
   ...
   /**
   * Set the SMTP server data.
   *
   * @param p_smtp_host: SMTP server host
   * @param p_smtp_port: SMTP server port
   * @param p_smtp_domain: SMTP server domain
   * @param p_smtp_wallet_path: path of the wallet containing the certificate
   * @param p_smtp_username: SMTP username
   * @param p_smtp_password: SMTP password
   */
   PROCEDURE set_smtp_data(
      p_smtp_host          IN mail_utility_var.g_short_string_type   := NULL
    , p_smtp_port          IN PLS_INTEGER                            := NULL
    , p_smtp_domain        IN mail_utility_var.g_short_string_type   := NULL
    , p_smtp_wallet_path   IN mail_utility_var.g_long_string_type    := NULL
    , p_smtp_username      IN mail_utility_var.g_short_string_type   := NULL
    , p_smtp_password      IN mail_utility_var.g_short_string_type   := NULL
   ) IS
   BEGIN
   
      -- Set SMTP values if they are defined.
      IF p_smtp_host IS NOT NULL THEN
         mail_utility_var.g_smtp_host := p_smtp_host;
      END IF;
      IF p_smtp_port IS NOT NULL THEN
         mail_utility_var.g_smtp_port := p_smtp_port;
      END IF;
      IF p_smtp_domain IS NOT NULL THEN
         mail_utility_var.g_smtp_domain := p_smtp_domain;
      END IF;
      IF p_smtp_wallet_path IS NOT NULL THEN
         mail_utility_var.g_smtp_wallet_path := p_smtp_wallet_path;
      END IF;
      IF p_smtp_username IS NOT NULL THEN
         mail_utility_var.g_smtp_username := p_smtp_username;
      END IF;
      IF p_smtp_password IS NOT NULL THEN
         mail_utility_var.g_smtp_password := p_smtp_password;
      END IF;
