   ...
   CURSOR c_prv IS
      SELECT * FROM (
         SELECT LISTAGG(my_privs.privilege, ', ') privilege
           FROM (SELECT DECODE(rownum,1,'DROP ANY SYNONYM') privilege FROM dual CONNECT BY rownum<=1) my_privs
          ...
          UNION ALL
         SELECT LISTAGG(my_privs.privilege, ', ') privilege
           FROM (SELECT DECODE(rownum,1,'DROP PUBLIC SYNONYM') privilege FROM dual CONNECT BY rownum<=1) my_privs
           LEFT OUTER JOIN user_sys_privs sys_privs
             ON sys_privs.privilege = my_privs.privilege
          WHERE sys_privs.privilege IS NULL
...
BEGIN
   ...
   IF l_privileges IS NOT NULL THEN
      RAISE_APPLICATION_ERROR(-20000,'Missing system privilege(s) for concealing: '||l_privileges);
   END IF;
   dbms_output.put_line('System privileges required for concealing are ok.');
END;
/