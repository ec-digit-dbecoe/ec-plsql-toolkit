...
--#define is_id(p_char) (is_letter(p_char) OR p_char='_')
   ...
   IS
      ...
      l_chr VARCHAR2(1);    -- current character
   BEGIN
   ...
   IS
      ...
      l_ch1 VARCHAR2(1);    -- current character
      l_ch2 VARCHAR2(1);    -- next character
      l_res VARCHAR2(4000); -- result string
   BEGIN
      --RETURN p_str; -- temporary deactivation to avoid a collateral issue, to further analyze
      WHILE l_pos <= l_len LOOP
